...
//  echo "<br> $sql <br><br>funcao --> $nfuncao  lotac --> $lotac  npad --> $npad ";exit; 
   $arquivo = "arq1";
   $nom = array();
   $tip = array();
   $tam = array();
   ...
   $retorno = pg_exec("create index work3_in on ".$arquivo." (w_regist,w_ano,w_mes,w_assent)") ;
...
//     echo "<BR> assenta --> $csql";
////db_criatabela(pg_query($csql." order by h16_dtconc, h16_assent"));  
...
*/
      ...
      //db_criatabela(pg_query("select * from " .$arquivo));  
...
////////// Comeca a impressao
           ...
           //$head5 = "PERÍODO : ".db_formatar($dataini,'d')." A ".db_formatar($datafin,'d');
	         ...
	         $head2 = "GRADE EFETIVIDADE";
           $head4 = "Matricula : ".db_str($pessoal[0]["r01_regist"],6)."   ".$pessoal[0]["z01_nome"];
           $head5 = "Lotação : ".$lotac;
           $head6 = "Admissão  : ".db_dtoc($pessoal[0]["r01_admiss"])."   Rescisão/Exoneração : ".db_dtoc($pessoal[0]["r01_recis"]);
           $head7 = "Função  : ".$nfuncao;
           $head8 = "Padrão    : ".$npad;
           if( $cparam == 0){
              if( !db_empty($num_proc)){
                  $head8 = "Processo: ".$num_proc;
                  $head9 = "Carga Horária :".db_str($pessoal[0]["r01_hrsmen"],6);
              }
           }else if($cparam == 3){
              // Quando chamado pela opção : Relatórios / Grade de Efevidade
              $pdf = new PDF();
...
//           for($Iwork3=0;$Iwork3< count($work3);$Iwork3++){
//echo "<BR> 1 w_ano --> ".$work3[$Iwork3]["w_ano"]." w_assent --> ".$work3[$Iwork3]["w_assent"]." w_dias --> ".$work3[$Iwork3]["w_dias"];
...
//	   }
              ...
              $pdf->cell(13,$alt,$ano_base,$lado,0,"C",0);
              for($k=1;$k <= $maior;$k++){
...
//              }
           ...
           if($total_munic > 0){ 
             ...
             $pdf->cell(40,$alt,"Total de Dias ",0,0,"L",0,'','.');
             $pdf->cell(20,$alt,$total_munic,0,1,"R",0);
           }
           ...
           while($Iwork3<count($work3)){
              ...
              if( !db_empty($tipo)){
                 ...
                 if(db_empty($posicao)){
//       echo "<BR> w_assent --> $assent h12efetiv --> $h12efetiv h12reltot --> $h12reltot h12graefe --> $h12graefe h12descr --> $h12descr";
                    $max += 1;
                    $posicao = $max;
...
//                 echo "<br>"."select * from tipoasse where trim(h12_assent) = ".db_sqlformat(trim($tempo_servico[$i]))."<br>" ;
                 //db_criatabela(pg_query("select * from tipoasse where trim(h12_assent) = ".db_sqlformat(trim($tempo_servico[$i]))));
...
//echo "<BR> function cria_assenta($tipo_contagem, $somatorio,$certinic,$datacert,$tipo_certd){";
...
if($tipo_contagem == "F"){
   ...
   $cond  = "select * from " . $arquivo . " left join tipoasse on h12_assent = w_assent::varchar(5) 
             where w_regist = " .db_sqlformat($pessoal[0]["r01_regist"]);
   $cond .= "     and w_ano = " .db_sqlformat(db_str($ano_inicio,4))." 
                  and w_mes = " .db_sqlformat(db_str($mes_inicio,2,0,"0"))." 
                  and h12_efetiv = '-' 
             order by w_regist,w_ano,w_mes ";
   global $work4;
