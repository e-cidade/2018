...
class cl_importaDadosColetor {
    ...
    for($i = 0; $i < count($arrayArquivos); $i++) {
      ...
      if($codFoto[0] == $iCodMatricula) {
        ...
        if($this->clACEDadosFoto->erro_status == "0") {
          $this->iErroStatus = 0;
          $this->sErroMsg    = "Inclusao na tabela aguacoletorexportadadosfoto não efetuada. Operação abortada. ERRO:{$this->clACEDadosFoto->erro_msg}";
          return false;
        }
  ...
  public function comparaRegistrosArquivo($iCodExportacao) {
    ...
    $rsAguaColetorExportaDados = $clAguaColetorExportaDados->sql_record($clAguaColetorExportaDados->sql_query_file(null, "x50_sequencial", null, "x50_aguacoletorexporta = $iCodExportacao"));
  ...
  public function mudaSituacaoExportacao($iCodExportacao, $iCodSituacao = 2) {
    ...
    if($this->clACExporta->numrows > 0) {
      ...
      if($this->clACExporta->erro_status == "0") {
        $this->iErroStatus = 0;
        $this->sErroMsg    = "Alteração na tabela aguacoletorexporta não efetuada. Operação abortada. ERRO:{$this->clACExporta->erro_msg}<br/>";
        return false;
      }
  ...
  public function geraSituacaoExportacao($iCodExportacao, $iUsuario, $dData, $sHora, $sMotivo, $iCodSituacao = 2) {
    ...
    if($this->clACESituacao->erro_status == "0") {
      $this->iErroStatus = 0;
      $this->sErroMsg    = "Inclusão na tabela aguacoletorexportasituação não efetuada. Operação abortada. ERRO:{$this->clACESituacao->erro_msg}<br/>";
      return false;
    }
  ...
  public function geraDadosImportacao($iCodColetorExportaDados, $iLinhaArquivo) {
    ...
    $rsACExportaDados = $this->clACExportaDados->sql_record($this->clACExportaDados->sql_query_file($this->iCodColetorExportaDados));
    ...
    if($this->clACExportaDados->numrows > 0) {
      ...
      if($this->clACExportaDados->erro_status == "0") {
        $this->iErroStatus = 0;
        $this->sErroMsg    = "Inclusão na tabela aguacoletorexporta não efetuada. Operação abortada. ERRO: {$this->clACExportaDados->erro_msg}.<br/>";
        return false;
      }
  ...
  public function getLeituraExportacao($iCodColetorExportaDados){
    ...
    $rsACExportaDadosLeitura = $this->clACExportaDadosLeitura->sql_record($this->clACExportaDadosLeitura->sql_query_file(null, "x51_agualeitura", null, "x51_aguacoletorexportadados = $this->iCodColetorExportaDados"));
  ...
  public function alteraLeitura($iCodLeitura) {
    ...
    if($this->clAguaLeitura->numrows > 0) {
      ...
      if($this->clAguaLeitura->erro_status == "0") {
        $this->iErroStatus = 0;
        $this->sErroMsg    = "Alteração na tabela agualeitura não informada. Operação abortada. ERRO: {$this->clAguaLeitura->erro_msg}.<br/>";
        return false;
      }
  ...
  public function cancelaLeitura($iCodLeitura) {
    ...
    if($this->clAguaLeituraCancela->erro_status == "0") {
      $this->iErroStatus = 0;
      $this->sErroMsg    = "Inclusão na tabela agualeituracancela não efetuada. Operação Abortada. ERRO: {$this->clAguaLeituraCancela->erro_msg}<br/>";
      return false;
    }
  ...
  public function geraOperacaoExcesso($iCodConfExcesso, $iCodRecExcesso) {
    ...
    $rsAguaCalc   = $this->clAguaCalc->sql_record($this->clAguaCalc->sql_query_file(null, "x22_codcalc, x22_numpre", null, "x22_exerc = $this->iAnoExportacao and x22_mes = $this->iMesExportacao and x22_matric = {$this->getCodMatricula($this->iLinhaArquivo)}"));
    ...
    if($this->clAguaCalc->numrows > 0) {
      ...
      if($this->clAguaCalcVal->erro_status == "0") {
        $this->iErroStatus = 0;
        $this->sErroMsg    = "Inclusão na tabela aguacalcval não efetuada. Operação abortada. ERRO: {$this->clAguaCalcVal->erro_msg}<br/>";
        return false;
      }
  ...
  public function geraReciboPaga($iCodColetorExportaDados) {
    ...
    $rsACExportaDados = $this->clACExportaDados->sql_record($this->clACExportaDados->getSqlArrecadRecibo($this->iCodColetorExportaDados));
    ...
    if($this->clACExportaDados->numrows > 0) {
      ...
      for($i = 0; $i < $this->clACExportaDados->numrows; $i++) {
        ...
        if($this->clReciboPaga->erro_status == "0") {
          $this->iErroStatus = 0;
          $this->sErroMsg    = "Inclusão na tabela recibopaga não efetuada. Operação Aboratada. ERRO:{$this->clReciboPaga->erro_msg}";
          return false;
        }
  ...
  public function geraReciboWeb($iCodColetorExportaDados) {
    ...
    $sCampos = "
       distinct
       x50_numpre  as k99_numpre_n,
       x52_numpre  as k99_numpre,
       x52_numpar  as k99_numpar, 
       0::integer  as k99_codbco,
       0::integer  as k99_codage,
       0::integer  as k99_numbco,
       0::integer  as k99_desconto,
       11::integer as k99_tipo,
       5::integer  as k99_origem";
    
    $rsACEDadosReceita = $this->clACEDadosReceita->sql_record($this->clACEDadosReceita->sql_query_dados(null, $sCampos, null, "x50_sequencial = $this->iCodColetorExportaDados"));
    ...
    if($this->clACEDadosReceita->numrows > 0) {
      ...
      for($i=0; $i < $this->clACEDadosReceita->numrows; $i++) {
        ...
        $this->clDBReciboWeb->incluir($oACEDadosReceita->k99_numpre, $oACEDadosReceita->k99_numpar, $oACEDadosReceita->k99_numpre_n);
        
        if($this->clDBReciboWeb->erro_status == "0") {
          $this->iErroStatus = 0;
          $this->sErroMsg    = "Inclusao na tabela db_reciboweb não efetuada. Operação abortada. ERRO: {$this->clDBReciboWeb->erro_msg}<br/>";
          return false;
  ...
  public function geraReciboCodBar() {
    ...
    if($this->clReciboCodBar->erro_status == "0") {
      $this->iErroStatus = 0;
      $this->sErroMsg    = "Inclusao na tabela recibocodbar não efetuada. Operação abortada. ERRO: {$this->clReciboCodBar->erro_msg}<br/>";
      return false;
  ...
  public function atualizaAguaCalc($iCodCalc) {
    ...
    if($this->clAguaCalc->numrows > 0) {
      ...
      if($this->clAguaCalc->erro_status == "0") {
        ...
        $this->iErroStatus = 0;
        $this->sErroMsg    = "Alteração na tabela aguacalc não efetuada. Operação abortada. ERRO:{$this->clAguaCalc->erro_msg}";
        return false;
  ...
  public function getCodReceita($iCodConfExcesso) {
    ...
    $rsAguaConsumoTipo = $this->clAguaConsumoTipo->sql_record($this->clAguaConsumoTipo->sql_query_file($this->iCodConfExcesso));
