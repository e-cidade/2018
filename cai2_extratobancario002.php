...
if($imprime_pdf == 'p'){	
	...
	foreach ($aContas as $oConta) {
		...
		imprimeConta($pdf,$oConta->k13_reduz,$oConta->k13_descr,$oConta->debito,$oConta->credito,$lImprimeSaldo);
		$lImprimeSaldo = false;
...
function imprimeConta($pdf,$codigo,$descricao,$debito,$credito,$lImprimeSaldo){
	$pdf->SetFont('Arial','b',8);		
	$pdf->Cell(12,5,"CONTA:"								,0,0,"L",0);
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(50,5,$codigo." - ".$descricao,0,0,"L",0);
	$pdf->SetFont('Arial','b',8);
	if($lImprimeSaldo){
...
function imprimeTotalMovConta($pdf,$saldo_debitado,$saldo_creditado,$saldo_atual){
	$pdf->SetFont('Arial','b',8);		
	$pdf->Cell(20,5,""																	,"TB",0,"R",1);
	$pdf->Cell(210,5,"TOTAIS DA MOVIMENTAÇÃO:"					,"TB",0,"R",1);
...
function imprimeTotalMovContaTxt($fp,$saldo_debitado,$saldo_creditado,$saldo_atual){
  ...
  $aLinhaTotalMovConta[6] = "TOTAIS DA MOVIMENTAÇÃO:";
