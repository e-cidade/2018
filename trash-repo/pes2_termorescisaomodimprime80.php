...
for ( $iIndice = 0; $iIndice < $iTotalRescisoes; $iIndice++ ) {
  ...
  $oServidor             = new Servidor($oDadosServidor->rh01_regist);
  ...
  /**
   * Busca nome da mae na tabela rhdepend  
   */
  $sSqlBuscaMae = $oDaoRhdepend->sql_query_file(null, "rh31_nome", null, "rh31_regist = {$oDadosServidor->rh01_regist} and rh31_gparen = 'M'");
  $rsBuscaMae   = $oDaoRhdepend->sql_record($sSqlBuscaMae);

  if ( $oDaoRhdepend->numrows > 0 ) {
    $sNomeMae = db_utils::fieldsMemory($rsBuscaMae, 0)->rh31_nome;
  } 

  $oServidorRelatorio->sNomeMae             = $sNomeMae;
