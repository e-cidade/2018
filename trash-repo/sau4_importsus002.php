...
function atualiza_cadsus($termometro = 0, $conn, $cod_cgs = null ,$DB_SERVIDOR ,$DB_BASE ,$DB_PORTA ,$DB_USUARIO ,$DB_SENHA){
	...
	if ($clsau_config->numrows > 0) {
		...
		//Estabelecer conexção com interbase
...
//$sWhere = " where tb_ms_usuario.NO_USUARIO in ('CLEIR MARTEGANHA RODRIGUES') ";
      ...
      //iniciar uma secção no postgres
      db_query ( 'begin' );
               ...
               //echo "<br> $sql03 <br>";
                    ...
                    //echo "<br> 336 <br> ";
							...
							if ((isset ( $result04 )) && (pg_num_rows ( @$result04 ) > 0) && (($row->ident != '') && ($row->ident != null))) {
								...
								//           1.procura pelos campos: 1. nro da certidão 2. nro do livro 3. nro da folha
								$result05 = null;
...
function cruza_dados($row, $cgs, $conn) {
	//O registro não será atualizado até que se prove nescesario
	$atualiza = false;
...
*/
		  ...
		  $atualiza = ( trim($objCGS->z01_v_uf)            != trim($row->uf) && $row->uf != null )?true:$atualiza;
			...
			for($x = 0; $x < 14; $x ++) {
				if (($row [$x + 2] != "") && ($row [$x + 2] != null)) {
					...
					//discartando endereço sem numero
					if ($x == 3) {
	...
	//retorna veredito
	//O registro deve ser atualizado? sim(true) não(false)
	return $atualiza;
}
