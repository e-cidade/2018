...
if($sqlerro==false) {
  ...
  $sqlorigem = "select * from issquant where q30_anousu =   $anoorigem limit 1";
  $resultorigem = db_query($sqlorigem);
  $linhasorigem = pg_num_rows($resultorigem);

  $sqldestino = "select * from issquant where q30_anousu =  $anodestino limit 1";
  $resultdestino = db_query($sqldestino);
  $linhasdestino = pg_num_rows($resultdestino);

  if (($linhasorigem > 0) && ($linhasdestino == 0 )) {
    
    $sqlrhlotaex = "select fc_duplica_exercicio('issquant', 'q30_anousu', ".$anoorigem.",".$anodestino.",null);";
    ...
    if ($linhasorigem == 0) {
      $cldb_viradaitemlog->c35_log = "Não existem dados de issqn (issquant) para o exercicio $anoorigem";
    }
    if ($linhasdestino >0) {
      $cldb_viradaitemlog->c35_log = "Ja existem dados de issqn (issquant) para ano de destino $anodestino";
    }
    $cldb_viradaitemlog->c35_codarq        = 47;
