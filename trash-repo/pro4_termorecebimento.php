...
include("classes/db_db_config_classe.php");
  ...
  $rsproc = pg_exec($sqlproc);
  $imptit = 0;
  $pdf = new scpdf();
  $pdf->open();
  $totregistros = (round(pg_num_rows($rsproc)/ 15) == 0?1:round(pg_num_rows($rsproc)/ 15)+1);
  
  for ($multiplo=1; $multiplo <= $totregistros; $multiplo++) {
     ...
     */
      ...
      $pdf->text(120,$xlin-9,'Processo nº '.$p58_codproc);
      if ($atendimento != "") {
    ...
    //  $pdf->text($xcol+17,$xlin+15,date('d/m/Y'));
      $pdf->text($xcol+40,$xlin+19,'Hora: '.$p62_hora);
    //  $pdf->text($xcol+40,$xlin+15,'Hora: '.date("H:i:s"));
      $pdf->Setfont('Arial','B',8);
      ...
      for ($ii = $multiplofor;$ii < ($multiplo * 15);$ii++) {
       ...
       if($ii == pg_num_rows($rsproc) - 1){
         break;
       }
    }
    $pdf->Setfont('Arial','',6);
                //$pdf->setdash(1,1);
//    $pdf->line(10,$xlin+100,85,$xlin+100);
                $pdf->setxy(10,$xlin+100);
                $pdf->multicell(75,2,str_repeat(".",75)."\n".$dusu,0,'C');
//    $pdf->line(125,$xlin+100,195,$xlin+100);
