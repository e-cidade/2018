...
try {
  ...
  switch ($oParam->exec) {
    ...
    case 'vincularEmpenhos' :
      ...
      $oDaoDesvincular = db_utils::getDao("empempenhocontrato");
      ...
      if ($oDaoDesvincular->numrows > 0) {

        for ($iDesvincular = 0; $iDesvincular < $oDaoDesvincular->numrows; $iDesvincular++) {

          $oDadosDesvincular = db_utils::fieldsMemory($rsEmpenhosDesvincular, $iDesvincular);
          ...
          if (USE_PCASP) {
            ...
            $sSqlBuscaEmpenhos = "select *
                             ...
                             inner join conhistdoc on c53_coddoc   = c71_coddoc
                                  where c75_numemp = {$oDadosDesvincular->e100_numemp}
                                    and c53_tipo in (900, 901)";
            $rsBuscaEmpenhoVinculado = db_query($sSqlBuscaEmpenhos);
            if (pg_num_rows($rsBuscaEmpenhoVinculado) > 0) {
          ...
          if ($oDaoAcordo->numrows > 0) {
            ...
            $aDesvincular = db_utils::getColectionByRecord($rsDesvincularEmpenhos);
      ...
      foreach ($aEmpenhosVincular as $oEmpenhosVincular) {
        ...
        $oDaoVerificaVinculo  = db_utils::getDao("empempenhocontrato");
        $sSqlVerificaVinculo  = $oDaoVerificaVinculo->sql_query_file(null, "*", null, "e100_acordo = {$iAcordo} and e100_numemp = {$oEmpenhosVincular} ");
        $rsVerificaVinculados = $oDaoVerificaVinculo->sql_record($sSqlVerificaVinculo);

        if ($oDaoVerificaVinculo->numrows == 0) {

          $oDaoEmpEmpenhoContrato->e100_numemp = $oEmpenhosVincular;
          $oDaoEmpEmpenhoContrato->e100_acordo = $iAcordo;
      ...
      //$oRetorno->sMessage = "Empenho(s) vinculado(s) com sucesso.";
    ...
    case "getEmpenhos":
      ...
      if (isset($iNumeroEmpenho) && $iNumeroEmpenho != null) {
        ...
        if ($iAcordo != null || $iAcordo != '') {
          ...
          $aVinculados   = db_utils::getColectionByRecord($rsJaVinculados);
