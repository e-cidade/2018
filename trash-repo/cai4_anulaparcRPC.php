...
switch ( $oParam->exec ) {
  ...
  case "getDadosParcelamento":
    ...
    $oTotal->valor_total_geral_retorno     = 0;
	  ...
	  try {
       ...
       $sSql .= "                                         {$sFormulaCalculoSaldo}::float8,                                    ";
		   ...
		   if ( pg_num_rows($rsSimulacao) == 0 ) {
			 ...
			 $aSimulacao = db_utils::getColectionByRecord($rsSimulacao,false,false,true);
      ...
      $aRetorno["sMensagem"] = $oErro->getMessage();
		  $aRetorno["iStatus"]   = 2;
  ...
  case "setConfirmaExclusao" :
    ...
    $sMensagem = "Parcelamento anulado com sucesso !";
	  ...
	  $rsAnulacao = pg_fetch_result(pg_query($sSqlAnulacao),0);
