...
for ($x=0; $x<$numrows01; $x++) {
  ...
  //echo "$y63_mes ----$y63_ano--------$codlev<br><br> ";
  ...
  //echo "<br>$numrows-------------------$sql<<br><br><br><br>    ";
  if ($numrows<1) {
    continue;
  }
  $tot_valor   = 0;
  $tot_imposto = 0;
  //if($y63_ano==2008 and $y63_mes==11) {
  ...
  for ($i=0; $i<$numrows; $i++) {
    ...
    if ($pdf->gety() > $pdf->h - 30 || $i==0) {
      if ($pdf->gety() > $pdf->h - 30) {
        ...
        $pdf->cell(20,$alt,"Valor pago","T",0,"L",0);
        $pdf->cell(20,$alt,"Valor à pagar","T",0,"L",0);
        $pdf->cell(20,$alt,"Valor corrigido","T",0,"L",0);
        $pdf->cell(20,$alt,"Multa","T",0,"L",0);
        $pdf->cell(20,$alt,"Juros","T",0,"L",0);
        $pdf->cell(20,$alt,"Valor total","T",1,"L",0);
        */
        $pdf->addpage("L");
      }
  ...
  if ($numrows>0) {
    ...
    $result = pg_query("select round(fc_corre(".($y60_espontaneo=='t'?$y32_receitexp:$y32_receit).",'".$y63_dtvenc."',".$y63_saldo.",'".$dtoper."',".db_getsession("DB_anousu").",'$y63_dtvenc'),2) as correcao");
    db_fieldsmemory($result,0);
    
    $result = pg_query("select fc_juros(".($y60_espontaneo=='t'?$y32_receitexp:$y32_receit).",'".$y63_dtvenc."','".$dtoper."','".$dDtOperData."','f',".db_getsession("DB_anousu").") as juro");
    db_fieldsmemory($result,0);
    $juro = round($correcao * $juro,2);
    
    /*if($y63_ano==2008 and $y63_mes==11) {
      echo "<br>$correcao ";
      echo "<br>select round(fc_multa(".($y60_espontaneo=='t'?$y32_receitexp:$y32_receit).",'".$y63_dtvenc."','".$dtoper."','".$dDtOperData."',".db_getsession("DB_anousu")."),2) as multa";
      $result = pg_query("select round(fc_multa(".($y60_espontaneo=='t'?$y32_receitexp:$y32_receit).",'".$y63_dtvenc."','".$dtoper."','".$dDtOperData."',".db_getsession("DB_anousu")."),2) as multa");
      db_fieldsmemory($result,0);
      $multa = round($correcao * $multa,2);
      echo "<br>$multa";
      die();
    }*/
    $result = pg_query("select fc_multa(".($y60_espontaneo=='t'?$y32_receitexp:$y32_receit).",'".$y63_dtvenc."','".$dtoper."','".$dDtOperData."',".db_getsession("DB_anousu").") as multa");
    db_fieldsmemory($result,0);
