  ...
  foreach ($oDadosRelatorio->aAlunosMatriculados as $oMatricula) {
    ...
    if ($oMatricula->getSituacao() != 'MATRICULADO') {
      ...
      foreach ($aDisciplinasPagina as $oRegencia) {
        ...
        $sAproveitamento        = ArredondamentoNota::formatar($sAproveitamento,
                                                              $oRegencia->getTurma()
                                                                        ->getCalendario()->getAnoExecucao());
        ...
        if ($oDadosRelatorio->iTipoPeriodo == 2) {
          ...
          if (!empty($oDadosRelatorio->iEnsino) && ($sResultadoFinal == 'A' || $sResultadoFinal == 'R')) {
            ...
            $aDadosTermo = DBEducacaoTermo::getTermoEncerramento($oDadosRelatorio->iEnsino, $sResultadoFinal);
         ...
         */
        if (   !$oAproveitamentoPeriodo->temAproveitamentoMinimo()
...
function cabecalhoPadrao($oPdf, $oDadosRelatorio) {
  ...
  $oDadosRelatorio->sEscola = $oEscola->getNome();
