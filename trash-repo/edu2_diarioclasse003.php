...
$result22 = $clperiodocalendario->sql_record($clperiodocalendario->sql_query("","ed52_i_codigo,ed52_c_aulasabado,ed53_d_inicio,ed53_d_fim",""," ed53_i_calendario = $ed57_i_calendario AND ed53_i_periodoavaliacao = $ed09_i_codigo"));
db_fieldsmemory($result22,0);
$dataperiodo = $ed09_c_descr." - ".db_formatar($ed53_d_inicio,'d')." à ".db_formatar($ed53_d_fim,'d');
$sql5 = $clregenciahorario->sql_query("","case when ed20_i_tiposervidor = 1 then cgmrh.z01_nome else cgmcgm.z01_nome end as regente",""," ed58_i_regencia = $ed59_i_codigo and ed58_ativo is true  ");
...
$result6 = $clregenciaperiodo->sql_record($clregenciaperiodo->sql_query("","ed78_i_aulasdadas as aulas",""," ed78_i_regencia = $ed59_i_codigo AND ed78_i_procavaliacao = $periodo"));
...
$sql1 = $clmatricula->sql_query("","ed60_i_aluno, ed60_i_codigo, ed60_i_numaluno, ed47_v_nome, ed60_c_situacao",
                                "ed60_i_numaluno,to_ascii(ed47_v_nome),ed60_c_ativa",
                                " ed60_i_turma = $ed57_i_codigo AND ed221_i_serie = $ed59_i_serie $condicao");
...
for($y=0;$y<$clmatricula->numrows;$y++) {
 ...
 if($amparo=="S"){
  ...
  if(trim($ed60_c_situacao)!="MATRICULADO"){
   $pdf->setfont('arial','b',11);
   $pdf->cell($larguraindiv*$colunas,4,trim(Situacao($ed60_c_situacao,$ed60_i_codigo)),1,0,"C",0);
   $pdf->setfont('arial','b',8);
  }else{
   $pdf->setfont('arial','b',8);
   $at = $pdf->getY();
 ...
 if(trim($ed60_c_situacao)=="MATRICULADO"){
  ...
  $pdf->setfont('arial','b',11);
  $pdf->cell($largura_pos_colunas,4,trim(Situacao($ed60_c_situacao,$ed60_i_codigo)),1,1,"C",0);
  $pdf->setfont('arial','b',8);
 }
