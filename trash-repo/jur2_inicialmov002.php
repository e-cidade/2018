...
if ($selecao=="final"){	   
	$sql .= " inner join inicialmov on v56_codmov = inicial.v50_codmov							                                    ";
}else{ 		   
	$sql .= " inner join inicialmov			on v56_inicial  = inicial.v50_inicial				                                    ";
}
	$sql .= " inner join situacao				     on v52_codsit                       = inicialmov.v56_codsit			          ";
...
if (isset($lista)){
  if($ver=="com") 
  $sql.=" where v50_instit = ".db_getsession('DB_instit')." and v56_codsit in $w ";
  else 
  $sql.=" where v50_instit = ".db_getsession('DB_instit')." and v56_codsit not in $w ";
} else {
...
switch($selOrdem){
   case "i":
     $sql .= " order by situacao , v50_inicial ";
     $cabOrdem = "Inicial";
   break;
   case "d":
     $sql .= " order by situacao, v50_data";
     $cabOrdem = "Data";
   break;
   case "f":
     $sql .= " order by situacao, v70_codforo";
     $cabOrdem = "Foro";
   break;
}
...
if ($auxiliar ->numrows ==0) {
  ...
  $sMsg = _M('tributario.juridico.jur2_inicialmov002.nao_existem_dados');
  db_redireciona("db_erros.php?fechar=true&db_erro= {$sMsg}");  
}

$head2 = "Iniciais por Tipo de Movimento";
  ...
  if (true){  
    ...
    for ($x=0; $x <$auxiliar->numrows;$x++){
        ...
        $imprime_header=true;
			...
			if (($imprime_header==true)||($sit!=$situacao))  {
        ...
        $pdf->setX(10);
        $pdf->Cell(20,$tam,"$v52_codsit",'B',0,"R",0);
        $pdf->Cell(80,$tam,"$situacao",  'B',1,"L",0); // <br>
        $pdf->SetFont('Arial','',7);
        $pdf->setX(10);
        $pdf->Cell(20,$tam,'INICIAL', '1',0,"C",1);
        $pdf->Cell(20,$tam,'DATA',    '1',0,"C",1); 
        $pdf->Cell(70,$tam,'ADVOGADO','1',0,"C",1); 
        $pdf->Cell(20,$tam,'FORO',    '1',0,"C",1); 
        $pdf->Cell(40,$tam,'VARA',    '1',0,"C",1); 
        $pdf->Cell(20,$tam,'ULT.AND', '1',1,"C",1); //<br> 
    		$pre = 1;
			} 		 
      ...
      $pdf->Cell(70,$tam,"$z01_nome",   0,0,"L",$pre); 
      $pdf->Cell(20,$tam,"$v70_codforo",0,0,"L",$pre); 
      $pdf->Cell(40,$tam,"$vara"			 ,0,0,"L",$pre); 
      $pdf->Cell(20,$tam,"$v56_data"	 ,0,1,"C",$pre);   // <br>
      $subTotalIni++;   
    }// end for
