	...
	for($Ipontofs=0;$Ipontofs<count($pontofs);$Ipontofs++){
		...
		$condicaoaux = " where rh27_instit = ". db_getsession("DB_instit") ." and rh27_rubric = " .db_sqlformat( $pontofs[$Ipontofs]["r10_rubric"] );
			...
			if( db_selectmax( "pontofr", "select * from pontofr ". bb_condicaosubpes("r19_").$condicaoaux )){
				...
				$matriz2[3] = round( $pontofr[0]["r19_valor"] + (!db_boolean($rubricas[0]["rh27_calcp"])?$pontofs[$Ipontofs]["r10_valor"]:($pontofs[$Ipontofs]["r10_valor"]/30)*$nsal),2);
				$matriz2[4] = round( $pontofr[0]["r19_quant"] + (!db_boolean($rubricas[0]["rh27_propq"])?$pontofs[$Ipontofs]["r10_quant"]:($pontofs[$Ipontofs]["r10_quant"]/30)*$nsal),2);

				db_update( "pontofr", $matriz1, $matriz2, bb_condicaosubpes("r19_").$condicaoaux );
			}else{

				$matriz2[3] = round( (!db_boolean($rubricas[0]["rh27_calcp"])?$pontofs[$Ipontofs]["r10_valor"]:($pontofs[$Ipontofs]["r10_valor"]/30)*$nsal),2);
				$matriz2[4] = round( (!db_boolean($rubricas[0]["rh27_propq"])?$pontofs[$Ipontofs]["r10_quant"]:($pontofs[$Ipontofs]["r10_quant"]/30)*$nsal),2);
