...
function dadosBasicos($oGet) {
  ...
  $iIsntituicao = db_getsession("DB_instit");
  ...
  $sSqlBasico .= "                 order by k97_dtsaldofinal desc limit 1)                ";
  ...
  $sSqlBasico .= "                          and k97_dtsaldofinal  = k68_data              ";
  $sSqlBasico .= "  inner join db_config     on codigo            = {$iIsntituicao}";
...
function listaDeCheques($iConta, $iConciliacao) {
  ...
  $sSqlCheques .= "  inner join fc_extratocaixa({$iIsntituicao}, {$iConta}, null, null, false ) on ricaixa  = k89_id     ";
  ...
  return db_utils::getColectionByRecord($rsCheques);
}
...
function movimentosTesouraria($iConta, $iConciliacao) {
  ...
  $sSqlTesouraria .= " inner join fc_extratocaixa({$iIsntituicao}, {$iConta}, null, null, false ) on ricaixa  = k89_id     ";
  ...
  return db_utils::getColectionByRecord($rsTesouraria);
}
...
function movimentosExtrato($iConciliacao) {
  ...
  return db_utils::getColectionByRecord($rsExtrato);
}



function saldoTesouraria($oGet, $sData, $iIntituicao) {
  
  $sSaldo  = " select sum(saldocontacaixa) as saldocontacaixa                                                                      ";
  $sSaldo .= " from ( select substr(fc_saltessaldo(c61_reduz, '{$sData}', '{$sData}', null, 1) ,43, 13)::float as saldocontacaixa  ";
  ...
  $sSaldo .= "         and conplanoreduz.c61_instit = {$iIntituicao}                                                               ";
