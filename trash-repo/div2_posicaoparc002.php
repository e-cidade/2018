 ...
 */
  ...
  if (($numini != "") && ($numfim != "")) {
    ...
    $where .= $and." arrecad.k00_numpar {$between} {$numini} and {$numfim} and arrecad.k00_dtvenc < '" . date("Y-m-d", db_getsession("DB_datausu")) . "'";
    $and = " and ";
    $head6 = "NUMERO DA PARCELA EM ATRASO: {$numini} a {$numfim}";
  } else if ($numini != "") {
    
    $where .= $and . " arrecad.k00_numpar {$oprini} {$numini} and arrecad.k00_dtvenc < '" . date("Y-m-d", db_getsession("DB_datausu")) . "'";
    $and = " and ";
    $head6 = "NUMERO DA PARCELA EM ATRASO - inicial: $numini";
  } else if ($numfim != "") {
    
    $where .= $and . " arrecad.k00_numpar {$oprfim} {$numini} and arrecad.k00_dtvenc < '" . date("Y-m-d", db_getsession("DB_datausu")) . "'";
    $and = " and ";
    $head6 = "NUMERO DA PARCELA EM ATRASO - final: {$numfim}";
  } else {
    
    $head6 = "NUMERO DA PARCELA EM ATRASO: nao especificado";
  }
  ...
  for ($x = 0; $x < pg_numrows($result); $x ++) {
    ...
    //if ($numpre == $numpre_ant && pg_numrows($result) > 1) {
    if (($numpre == $numpre_ant && pg_numrows($result) > 1) and !( $x == pg_numrows($result) - 1) ) {
      ...
      if (strpos($grafico, "R") > 0) {
        ...
        if ($ordem == "NP") {
          ...
          //$chave = $k00_dtvenc_one . str_pad($x,3,"0",STR_PAD_LEFT);
