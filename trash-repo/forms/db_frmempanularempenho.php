               ...
               <thead>
                   <tr> 
                     <th class='table_header'><input type='checkbox'  style='display:none'
                       ...
                       <th class='table_header'>Sequência</td>
                       <th class='table_header'>Item</td>
                       <th class='table_header'>Vlr Unitário</td>
                       <th class='table_header'>Quantidade</td>
                       <th class='table_header'>Valor Total</td>
                       <th class='table_header'>Quantidade</td>
                       <th class='table_header'>Valor Total</td>
                       <th class='table_header' width='18px'>&nbsp;</td>
                   </tr>    
               </thead>
               <tbody id='dados' style='height:100;width:95%;overflow:scroll;overflow-x:hidden;background-color:white'>
               <td colspan='10'>&nbsp;</td>
               </tbody>
               <tfoot>
                  <tr>
      ...
      <td>
        <fieldset><legend><b>Solicitacao de Anulação</b></legend> 
           <table style='border:2px inset white;backgorund-color:white;' width='100%' cellspacing='0'>
               <thead>
                   <tr> 
                       <th class='table_header'>&nbsp;</td>
                       <th class='table_header'>Sequência</td>
                       <th class='table_header'>Item</td>
                       <th class='table_header'>Quantidade solicitada</td>
                       <th class='table_header'>Valor Solicitado</td>
                       <th class='table_header' width='18px'>&nbsp;</td>
                   </tr>    
               </thead>
...
function js_saida(oAjax){
    ...
    if (obj.numnotas > 0){
      for (var i = 0; i < obj.data.length;i++){
         ...
         saida += "<td class='linhagrid' id='vlruni"+obj.data[i].e62_sequen+"' style='text-align:right'>"+obj.data[i].e62_vlrun+"</td>";
         saida += "<td class='linhagrid' id='saldo"+obj.data[i].e62_sequen+"'  style='text-align:right'>"+obj.data[i].saldo+"</td>";
         saida += "<td class='linhagrid' id='saldovlr"+obj.data[i].e62_sequen+"' style='text-align:right'>"+obj.data[i].e62_vlrtot+"</td>";
         ...
         saida += " value='"+sQuantidadeTotal+"' style='text-align:right' size='5'onkeypress='return js_teclas(event)' onblur='js_calculaValor("+obj.data[i].e62_sequen+",1)'></td>";
         saida += "<td class='linhagrid' style='text-align:center;width:10%'>";
         ...
         saida += " value='"+sValorTotal+"' size='5' class='valores' onkeypress='return js_teclas(event)' onblur='js_calculaValor("+obj.data[i].e62_sequen+",2)'></td></tr>";
     }
...
function setValoresItem(item){
   ...
   $('confirmar').disabled = false;
   nValor      = new Number($('vlrsolic'+item.value).innerHTML);
   nQtde       = new Number($('qtdsolic'+item.value).innerHTML);
   nItensDigi  = new Number($('qtdesol'+item.value).value);//valores digitados pelo usuario
   nValorDigi  = new Number($('vlrtot'+item.value).value); //valores digitados pelo usuario
   nItensEmp   = new Number($('saldo'+item.value).innerHTML);//saldo de itens do empenho
   nValorEmp   = new Number($('saldovlr'+item.value).innerHTML); //saldo do valor do item;
   nValorTotal = new Number(nValorDigi + nValor);
   nQtdeTotal  = new Number(nItensDigi + nQtde); 
   
   if (item.checked){

      /* marcamos o item do empenho e atualizamos os valores caso o valor nao seje maior que 
       * o saldo do empenho. 
      */
...
function js_calculaValor(id,tipo){
   ...
   $('confirmar').disabled = false;
   var nVlrUni        = new Number($('vlruni'+id).innerHTML); 
   var nQtde          = new Number($F('qtdesol'+id));
   var nVlrTotal      = new Number($F('vlrtot'+id));
   ...
   iSaldo         = (new Number($('saldo'+id).innerHTML)    - iSaldoSolic);
   iSaldovlr      = (new Number($('saldovlr'+id).innerHTML) - nSaldoVlrSolic);
   if (nQtde > 0){
...
function setTotal(){
    ...
    $('valortotal').innerHTML = nTotal.toFixed(2);
}
     ...
     for (i = 0;i < itens.length;i++){
       if (itens[i].checked == true){
         ...
         if ($F('vlrtot'+itens[i].value) != 0 && $F('vlrtot'+itens[i].value) !=''){
            ...
            * caso o total seje maior que o existente avisamos o usuário e nao deixamos 
           ...
           itensEmp   += '"vlruni":"'+$('vlruni'+itens[i].value).innerHTML+'"}';
           sV          = ",";
           valorTotal += eval($F('vlrtot'+itens[i].value)); 
         }
