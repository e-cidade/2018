...
function js_retornoGetDados(oAjax){
    ...
    if (oJson.isRestoPagar){
        ...
        /*
         * caso o empenho seje RP, so podemos deixa ro usuario anular a ordem, se solictar a anulaçao do 
         * empenho.
         */
    ...
    if (oJson.m51_tipo == 2){
       ...
       alert('Nota gerada automaticamente pelo sistema. nao poderá ser anulada.');
    ...
    if (oJson.itens.length > 0){//3
       ...
       if (iLinhasErro == oJson.itens.length) {
         ...
         sMsgErro += "\nNão sera possível efetuar a anulação da ordem.";
         alert(sMsgErro);
...
function js_helpanula(valor){
   ...
   switch (valor){
      ...
      case "1":
         $('helpanula').innerHTML  = 'Será solicitado a anulação dos valores do itens no empenho. Geralmente usado para descontos.';
      break;
      case "2":
         $('helpanula').innerHTML  = 'Será solicitado a anulação da quantidade do itens no empenho.';
