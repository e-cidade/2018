...
//MODULO: Farmacia
$clfar_listamodelo->rotulo->label();
...
<table border="0">
  <tr>
    <td nowrap title="<?=@$Tfa17_i_codigo?>">
       <?=@$Lfa17_i_codigo?>
    </td>
    <td> 
<?
...
<tr>
	<td>
		...
		$cliframe_alterar_excluir->legenda       ="ITENS DO MODELO LIVRO";
...
<script>
function js_pesquisafa17_i_listacontrolado(mostra){
  if(mostra==true){
    js_OpenJanelaIframe('','db_iframe_far_listamodelo','func_far_listacontrolado.php?funcao_js=parent.js_mostrafar_listamodelo1|fa15_i_codigo|fa15_c_listacontrolado','Pesquisa',true);
  }else{
     if(document.form1.fa17_i_listacontrolado.value != ''){ 
        js_OpenJanelaIframe('','db_iframe_far_listamodelo','func_far_listacontrolado.php?pesquisa_chave='+document.form1.fa17_i_listacontrolado.value+'&funcao_js=parent.js_mostrafar_listamodelo','Pesquisa',false);
     }else{
       document.form1.fa15_c_listacontrolado.value = ''; 
     }
  }
}
function js_mostrafar_listamodelo(chave,erro){
  document.form1.fa15_c_listacontrolado.value = chave; 
  if(erro==true){ 
    document.form1.fa17_i_listacontrolado.focus(); 
    document.form1.fa17_i_listacontrolado.value = ''; 
  }
}
function js_mostrafar_listamodelo1(chave1,chave2){
  document.form1.fa17_i_listacontrolado.value = chave1;
  document.form1.fa15_c_listacontrolado.value = chave2;
  db_iframe_far_listamodelo.hide();
}
function js_pesquisa(){
  js_OpenJanelaIframe('','db_iframe_far_listamodelo','func_far_listamodelo.php?funcao_js=parent.js_preenchepesquisa|fa17_i_codigo','Pesquisa',true);
}
