...
function js_mostraescolaproc(ed82_i_codigo, ed82_c_nome){
  ...
  if (ed82_i_codigo == undefined
    ...
    alert("Escola não encontrada, digite um código correto ou efetue uma busca!");
...
function js_submit() {
  ...
  if (js_checaEscolaExiste() == false) {
    ...
    alert('Escola de destino não encontrada, verifique a escola!');
    return false;
    $('ed104_i_escoladestino').focus();

  } else if ($('ed104_i_aluno').value == "") {

    alert('Selecione o Aluno para realizar a transferência antes de incluir.');
    return false;

  } else if ($('ed104_i_escoladestino').value == "") {

	  alert('Digite ou pesquisa o código da escola de destino antes de incluir.');
