...
class dadosEmpenhoFolha {
  ...
  */ 
  ...
  public function geraPlanilhaGeral( $sSigla='',$iAnoUsu='',$iMesUsu='',$iInstit='',$sSemestre='' ){
    ...
    if ( $lGeraRetencao == 't') {    
	  ...
	  //será gerada planilha para todos os registros de retenções dos empenhos da folha
	     	...
	     	try {
	     	  $iPlanilhaEmpenho = $this->geraPlanilhaEmpenhoFolha($sSigla, $iAnoUsu, $iMesUsu, $sSemestre, db_getsession("DB_instit"));
	     	} catch ( DBException $eException ){
          throw new DBException("{$sMsgErro}\n{$eException->getMessage()}");
	      } catch ( BusinessException $eException ){
          throw new BusinessException("{$sMsgErro}\n{$eException->getMessage()}");
  ...
  public function geraPlanilhaEmpenhoFolha( $sSigla, $iAnoUsu, $iMesUsu, $sSemestre, $iInstit) {
