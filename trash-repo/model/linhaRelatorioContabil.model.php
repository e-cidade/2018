...
class linhaRelatorioContabil {
   ...
   * 
   */
  function __construct($iCodRel, $iCodLinha) {
   ...
   */
  function getCols($iPeriodo = null) {
    ...
    $iNumCols  = pg_num_rows($rsCols);
    $aCols     = db_utils::getColectionByRecord($rsCols);
    return $aCols;
   ...
   */
  function getValoresColunas($iLinha=null,$iCol=null, $sListaInstit=null, $iAno = null) {
   ...
   * @return array 
   */
  function getValoresColunasInstituicoes($iLinha=null,$iCol=null, $sListaInstit=null, $iAno = null) {
   ...
   *
   * @param string $sListaInstit lista das instuicoes que serao calculadas
   * @param integer $iAno ano padrao para consulta  
   * @return array
   */
  function getValoresSomadosColunas($sListaInstit=null, $iAno = '') {
  ...
  function excluirLinha($iLinha, $iInstituicao = null, $iAno = null) {
    ...
    foreach ($aLinhas as $oLinha) {
      ...
      if ($this->iPeriodo != null) { 
        $sSqlLinhas .= "  and o117_periodo  = ".$this->iPeriodo; 
      }
   ...
   *
   * @param sdtClass $oFilter objeto com os filtos
   ...
   *
   * @return Objeto com os dados do filtro
   */
  function getParametrosPadrao($sInstit = '') {
        ...
        if (trim($oRecursoLinha->item(0)->getAttribute("numerolinha")) != "") {
          ...
          foreach ($aLinhasCadastradas as $iLinha) {
            ...
            foreach ($aContas as $oConta) {
              ...
              $sWhere = " c61_anousu = ".db_getsession("DB_anousu");
              if ($oConta->nivel != "") {
              ...
              if ($sInstit != "") {
                 $sWhere .= "and c61_instit in ({$sInstit})";             
              }
              $sSqlRecursos   = analiseQueryPlanoOrcamento($oDaoConplanoReduz->sql_query(null, null,"distinct c61_codigo", null, $sWhere));
              $rsRecursos     = $oDaoConplanoReduz->sql_record($sSqlRecursos);
              for ($i = 0; $i < $oDaoConplanoReduz->numrows; $i++ ) {
    
                $iRecurso = db_utils::fieldsMemory($rsRecursos, $i)->c61_codigo;
                if (!in_array($iRecurso, $aRecursosLinha)) {
                  array_push($aRecursosLinha, $iRecurso);
                }
              }
            }
            $oFiltro->recurso->operador = "in";
   ...
   * @return objeto com os dadso dos parametros configurados
   */
  function getParametros($iAno, $sInstituicoes=null) {
    ...
    $iCodigoInstit   = db_utils::fieldsMemory($rsCodigoInstit, 0)->codigo; 
   ...
   * @param dbUtils $oDadosOrigem dados com a origem dos valores que seram verificados
   ...
   * </code>
   */
  function match($oConta, $oOrcamento, $oDadosOrigem, $iOrigem) {
 ...
 /**
        ...
        if (trim($oRecursoLinha->item(0)->getAttribute("numerolinha")) != "") {
          ...
          foreach ($aLinhasCadastradas as $iLinha) {
            ...
            foreach ($aContas as $oConta) {
              ...
              $sWhere .= "and c61_instit in (".db_getsession("DB_instit").")";             
              $sSqlRecursos   = analiseQueryPlanoOrcamento($oDaoConplanoReduz->sql_query(null, null,"distinct c61_codigo", null, $sWhere));
              $rsRecursos     = $oDaoConplanoReduz->sql_record($sSqlRecursos);
              for ($i = 0; $i < $oDaoConplanoReduz->numrows; $i++ ) {
   ...
   *
   * @param integer $iAno ano do parametro
   */
  ...
  /**
   * Exclui os vinculos SIGAP
   *
   * @param integer_type $iAno
   */
