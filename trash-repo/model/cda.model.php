...
class cda {
  ...
  protected function getOrigemDebitoDivida() {
    ...
    $sqlOrigemMatric .= "         v01_numcgm as numcgm ";
    ...
    $aOrigem          = array();
    $aOrigem          = db_utils::getColectionByRecord($rsOrigemDebitos);
    return  $aOrigem;
  ...
  function getProcessoParcelamento() {
    ...
    $aDadosProcParc  = db_utils::getColectionByRecord($rsProcParc);
  ...
  function getObsNumpreParcelamento() {
    ...
    $aDadosObsNumpre  = db_utils::getColectionByRecord($rsObsNumpre);
  ...
  function getDevedoresEnvolvidos($sTipoEndereco = 'o') {
    ...
    $oRetorno->aDevedores = $aDevedoresEnvolvidos;
  ...
  protected function getDebitosParcelamento($lRemissao = false) {
    ...
    foreach ($aOrigens as $oOrigem) {
      ...
      $aDebitos  = db_utils::getColectionByRecord($rsDebitos);
      ...
      for ( $i = 0; $i < count($aDebitos); $i++) {
        ...
        if ( $this->isComposicao() ) {
          ...
          if ( pg_num_rows($rsArreold) == 0 ) {
	        ...
	        $sSqlVerificaArreold = "select * from arreold where k00_numpre = ".$aDebitos[$i]->numpre;
	        $rsArreold = db_query($sSqlVerificaArreold) or die($sSqlVerificaArreold);
  ...
  public function getProcedencias() {
    ...
    if ($this->getTipo() == 1) {
      ...
      foreach ($aOrigens as $oOrigem) {             
        ...
        $aProcedencias  = db_utils::getColectionByRecord($rsProcedencias);
      ...
      $rsDadosDivida   = $this->oDaoCertid->sql_record($sqlDadosDivida);
      $aProced         = db_utils::getColectionByRecord($rsDadosDivida);
    ...
    if (count($aProcedenciasAgrupadas) > 1) {
      ...
      $aProcedencias     = db_utils::getColectionByRecord($rsProcedAgrupa);
  ...
  public function geraCertidao($iNumpre='', $iNumpar='',$iTipoDebito='',$iCodCertidao='',$iMatric=''){
    ...
    $aDadosArrecad = db_utils::getColectionByRecord($rsArrecad);
