...
for ($xxx = 0; $xxx < $this->nvias; $xxx++) {
  ...
  $this->objpdf->Setfont('Arial','B',10);
  $this->objpdf->text(153,$xlin-7,'MOVIMENTO : ');
  $this->objpdf->text(180,$xlin-7,$this->sMovimento);
  $this->objpdf->text(142,$xlin-2,'DATA DE EMISSÃO : ');
  $this->objpdf->text(180,$xlin-2,$this->emissao);
  ...
  $rsNotas = db_query("select e69_numero, e69_dtnota 
    	     						        from empnota 
  ...
  if (is_array($this->aRetencoes) && count($this->aRetencoes) > 0 ) {
    ...
    for ($ii = 0; $ii < count($this->aRetencoes); $ii++) {
      ...
      if ($this->aRetencoes[$ii]->e23_recolhido == 't') {
      	$nTotalRetencaoRecolhido = $this->aRetencoes[$ii]->e23_valorretencao;
      }
    	if ($ii < 9) {
  ...
  $this->objpdf->cell(23,4,db_formatar($this->empenhado - $this->outrasordens - $total_emp + ($total_anu - $this->empenho_anulado),'f'),0,1,"R");
  ...
  $this->objpdf->Setfont('Arial','',7);
  $this->objpdf->multicell(200,4,substr($this->obs,0,1338));   
  ...
  if ($total_sal == 0) {
    ...
    //$nValorSaldo = ($this->valor_ordem != "") ? $this->valor_ordem : ($total_sal - ($total_ret - $nTotalRetencaoRecolhido));
