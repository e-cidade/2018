...
for ($i = 0; $i < 2; $i++) {
   ...
   if ( $this->tipoguia == "q" ) {
     ...
     if ($this->lRetificado) {
     	$this->objpdf->Cell(100,3,'Guia Retificadora N'.chr(176) .' SMF/'.db_formatar($this->itbi,'s','0',5).'/'.$this->ano,0,0,"L",0);
     } else {
   ...
   if ( $this->tipoguia == "q" ) {
     if ( $i == 0 ) {
       ...
       $this->objpdf->Cell(25,5,"Via Contribuinte",0,0,"R",0);
   	 ...
   	 if ($this->lRetificado) {
   	 	$this->objpdf->sety($iY1+10);
   	 	$this->objpdf->setx(30);
   	 	$this->objpdf->SetFont('Arial',false,7);
   	 	$sOrigemRetificativa  = "Guia Retificativa à Guia de Número SMF/";
   	 	$sOrigemRetificativa .= db_formatar($this->sOrigemRetificacaoNumero,'s','0',5);
   	 	$sOrigemRetificativa .="/". $this->sOrigemRetificacaoAno;
   	 	$this->objpdf->Cell(100,3, $sOrigemRetificativa ,0,0,"L",0);
   	 }
   }
...
//   $this->objpdf->ln(2);
    ...
    */
   $observacoes = substr( str_replace("\n"," ",$this->adquirintes." - ".$this->transmitentes." - ".$this->observacaoIncluido." ".$this->observacaoLiberado) . $this->sMsgSituacaoImovel ,0,400);
   $yy = $this->objpdf->gety();
   $this->objpdf->sety($y);   
   ...
   if ( $this->tipoguia == "n" ) {
     $this->objpdf->SetFont('Arial','',6);
     $this->objpdf->cell(60,$altura,'Pagável nos bancos conveniados HSBC, CEF, Itaú, Banco do Brasil e Lotéricas.',0,1,"C",0);
   }
   $this->objpdf->SetFont('Arial','B',10);
   ...
   if ( $this->tipoguia == "n" ) {
     ...
     $this->objpdf->SetFont('Arial','B',10);
     $this->objpdf->setY($pos-5);
     $this->objpdf->setX(130);
     if ($this->it14_valorpaga == 0) {
       $this->objpdf->cell(60,$altura,'Valor a Pagar : I S E N T O',0,1,"C",0);
     } else {
   	   $this->objpdf->cell(60,$altura,'Taxa de expediente: R$ '.db_formatar($this->tx_banc,'f','',1),0,1,"L",0);
       $this->objpdf->setX(130);
   	   $this->objpdf->cell(60,$altura,'Valor a Pagar : R$ '.db_formatar($this->it14_valorpaga+$this->tx_banc,'f','',1),0,1,"L",0);
     }

     $this->objpdf->SetFont('Arial','',6);
     $this->objpdf->sety($pos+4);
   /*
     $this->objpdf->setX(125);
     $this->objpdf->cell(60,2.5,'Atenção! 48 horas após o pagamento, retirar o comprovante de quitação fiscal',0,0,"L",0);
     $this->objpdf->sety($pos+6);
   
     $this->objpdf->setX(125);
     $this->objpdf->cell(60,2.5,'junto ao setor responsável.',0,0,"L",0);
  */
	   if ( $i == 0 ) {
       $this->objpdf->ln(15);
     }
     ...
     $this->objpdf->setY($iPosicaoY);
     $this->objpdf->ln(2);

     $this->objpdf->SetFont('Arial','B',7);
     $pos = $this->objpdf->gety();
     $this->objpdf->text(83,$pos-1.5,$this->linha_digitavel);

     $this->objpdf->SetFont('Arial','B',6);
     $pos = $this->objpdf->gety();
     
		 $this->objpdf->cell(60,2.5,'Informações do pagamento',0,1,"L",$iPreencheFundo);
		 $this->objpdf->cell(60,2.5,'Valor: ' . db_formatar($this->valorpagamento,'f'),0,1,"L",0);
		 $this->objpdf->cell(60,2.5,'Taxa de Expediente: ' . db_formatar($this->taxa_bancaria,'f'),0,1,"L",0);
		 $this->objpdf->cell(60,2.5,'Data: ' . db_formatar($this->datapagamento,'d'),0,1,"L",0);
		 $this->objpdf->cell(60,2.5,'Banco: ' . $this->bancopagamento . ' - Agencia: ' . $this->agenciapagamento,0,1,"L",0);

     $this->objpdf->sety($pos);   
   /*     
     $this->objpdf->SetFont('Arial','',6);
		 $this->objpdf->cell(65,2.5,'',0,0,"L",0);
		 $this->objpdf->cell(60,2.5,'ATENÇÃO: A PARTIR DO REGISTRO DA ESCRITURA DO IMÓVEL, O CONTRIBUINTE DEVERÁ NO PRAZO DE 90 (NOVENTA)',0,1,"L",0);
		 $this->objpdf->cell(65,2.5,'',0,0,"L",0);
		 $this->objpdf->cell(60,2.5,'DIAS A CONTAR DA DATA DO REGISTRO PARA REQUERER JUNTO A P.M.M. A TRANSFERÊNCIA E AVERBAÇÃO',0,1,"L",0);
   */
     $this->objpdf->ln(10);
