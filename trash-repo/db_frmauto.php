...
//MODULO: fiscal
include("classes/db_tipofiscaliza_classe.php");
...
/*    include("classes/db_cgm_classe.php");
 ...
 // $result = $clfiscal->sql_record($clfiscal->sql_query_ender($y30_codnoti,"y30_codnoti,y13_codigo,y13_codi,y13_numero,y13_compl,y12_codigo,y12_codi,y12_numero,y12_compl,bairroexec.j13_descr as bairroe,bairrolocal.j13_descr as bairrol,ruaexec.j14_nome as ruae,rualocal.j14_nome as rual"));
$sqlnot = "
			select 
			y13_codnoti,
 ...
 //die($sqlnot);
  ...
  $ruanomee=$ruae;
  $ruanomel=$rual;
  $bairrodescre=$bairroe;
  $bairrodescrl=$bairrol;
*/
  //$notific=true;
  include_once("classes/db_fiscalcgm_classe.php");
  $clfiscalcgm = new cl_fiscalcgm;
  $result = $clfiscalcgm->sql_record($clfiscalcgm->sql_query($y30_codnoti,"*",null," y30_instit = ".db_getsession('DB_instit') )); 
  if($clfiscalcgm->numrows > 0){
    db_fieldsmemory($result,0);
  }
  include_once("classes/db_fiscalinscr_classe.php");
  $clfiscalinscr = new cl_fiscalinscr;
  $result = $clfiscalinscr->sql_record($clfiscalinscr->sql_query($y30_codnoti,"*",null," y30_instit = ".db_getsession('DB_instit')));
  if($clfiscalinscr->numrows > 0){
    db_fieldsmemory($result,0);
  }
  include_once("classes/db_fiscalmatric_classe.php");
  $clfiscalmatric = new cl_fiscalmatric;
  $result = $clfiscalmatric->sql_record($clfiscalmatric->sql_query($y30_codnoti,"*",null," y30_instit = ".db_getsession('DB_instit')));
  if($clfiscalmatric->numrows > 0){
    db_fieldsmemory($result,0);
  }
  include_once("classes/db_fiscalsanitario_classe.php");
  $clfiscalsanitario = new cl_fiscalsanitario;
  $result = $clfiscalsanitario->sql_record($clfiscalsanitario->sql_query($y30_codnoti,"*",null," y30_instit = ".db_getsession('DB_instit')));
...
if ($db_opcao==2||$db_opcao==3){
  $result_ident=$clauto->sql_record($clauto->sql_query_busca($y50_codauto,"*",null," y50_instit = ".db_getsession('DB_instit')));
  if($clauto->numrows>0){
    db_fieldsmemory($result_ident,0);
    $cod=$dl_código;
    $inform=$dl_identificação;
    if ($dl_identificação=='Cgm'){
      $abre="prot3_conscgm002.php?fechar=func_nome&numcgm";
    }else if ($dl_identificação=='Inscrição'){
      $abre="iss3_consinscr003.php?numeroDaInscricao";
    }else if ($dl_identificação=='Matrícula'){
      $abre="cad3_conscadastro_002.php?cod_matricula";
    }else if ($dl_identificação=='Sanitário'){
      $abre="fis3_consultasani002.php?y80_codsani";
    }else if ($dl_identificação=='Notificação'){
       $abre="fis3_fiscal006.php?y30_codnoti";
    }
  }
}
if (isset($cod)&&$cod!=""){ 
 $dados = "<a style='text-decoration:none;color:#6699cc;background-color:yellow' onMouseOver='this.style.color=\"blue\"' onMouseOut='this.style.color=\"#6699cc\"'  onClick=\"js_abre('".$abre."=$cod');return false;\" href=''>".$inform.": ".$cod." &nbsp;|&nbsp;".@$z01_nome."</a>";
}
...
?>
  ...
  <tr>
    <td nowrap title="<?=@$Ty50_dtvenc?>" colspan=2>
       <?=@$Ly50_dtvenc?>
...
db_input('nome',40,$Iz01_nome,true,'text',3,'')
  ...
  //db_msgbox("y14_codigo $y14_codigo y14_codi $y14_codi y14_numero $y14_numero y14_compl $y14_compl y15_codigo $y15_codigo y15_codi $y15_codi y15_numero $y15_numero y15_compl $y15_compl j14_nome $j14_nome              j13_descr $j13_descr j14_nome_exec $j14_nome_exec j13_descr_exec $j13_descr_exec");
...
if($db_opcao==1||$db_opcao==2){
  if (@$j14_nome==""||$j13_descr==""||$y14_codigo==""||$y14_codi==""){
	...
	$result_munic = pg_query($sql_munic);
	db_fieldsmemory($result_munic,0);
	if(strtoupper($z01_munic) == strtoupper($munic) and 1 == 2){	  
	  echo "<script>alert('Atualize o endereço no CGM antes de prosseguir');
	  	for(i=0;i<document.form1.length;i++){
		  document.form1.elements[i].disabled = true
		}
	        if(confirm('Deseja Alterar o cgm agora?')){
                  js_OpenJanelaIframe('','db_iframe_altcgm','prot1_cadcgm002.php?chavepesquisa=$z01_numcgm&testanome=true&autoinfra=true','Altera Cgm',true,0);
	        }else{
		}
		</script>";
	}
...
if($db_opcao == 1){
  if(isset($j14_codigo) && $j14_codigo != ""){
    ...
    echo "<script>document.form1.y14_codigo.readOnly = true</script>";
  }else{
  ...
  if(isset($j13_codi) && $j13_codi != ""){
    ...
    echo "<script>document.form1.y14_codi.readOnly = true</script>";
  }else{
      echo "<script>alert('Atualize o endereço no CGM antes de prosseguir')</script>";
      echo "<script>
	    for(i=0;i<document.form1.length;i++){
  ...
  if(isset($z01_numero) && $z01_numero != ""){
    ...
    echo "<script>document.form1.y14_numero.readOnly = true</script>";
  }else{
      echo "<script>alert('Atualize o endereço no CGM antes de prosseguir')</script>";
      echo "<script>
	    for(i=0;i<document.form1.length;i++){
  ...
  if(isset($z01_compl) && $z01_compl != ""){
    ...
    echo "<script>document.form1.y14_compl.readOnly = true</script>";
  }
}*/
