...
switch ($objParam->exec){
	...
	case "getEspecialidade":
		...
		if( $clespecmedico->numrows > 0){
			$objRetorno->itens  = db_utils::getColectionByRecord($res_especmedico, true, false, true);
		}else{
			$objRetorno->status  = 2;
	...
	case "getGridProcedimentos":
		...
		if( $clprontproced->numrows > 0 ){
			$objRetorno->itens  = db_utils::getColectionByRecord($res_procedimentos, true, false, true);
		}
	...
	case "getCID":
		...
		if(($res_sau_proccid!=false) && (pg_num_rows($res_sau_proccid)  > 0)){
			$objRetorno->itens  = db_utils::getColectionByRecord($res_sau_proccid, true, false, true);		
		}else{
			$objRetorno->status  = 2;
