      ...
      $inicio_relatorio = 1;
		...
		if ($m81_tipo == 1) {
		  ...
		  $quant_sai  = $quantidade;
			$vlr_sai    = $preco_medio;
			$tot_sai    = $vlr_sai*$quantidade;
			$quant_est -= $quantidade;
  		$vlr_est    = $preco_medio;
			$tot_est   -= ($tot_sai);
			//$tot_est    = $vlr_est * $quant_est;
		...
		$pdf->cell(15, $alt, db_formatar($vlr_ent, 'f', ' ', 0, 'e', 4), 0, 0, "R", $p);
		$pdf->cell(20, $alt, db_formatar($tot_ent, 'f'), 0, 0, "R", $p);
		$pdf->cell(15, $alt, db_formatar(round($quant_sai,2), "p", " ", 2), 0, 0, "C", $p);
		$pdf->cell(15, $alt, db_formatar($vlr_sai, 'f', ' ', 0, 'e', 4), 0, 0, "R", $p);
		$pdf->cell(20, $alt, db_formatar($tot_sai, 'f'), 0, 0, "R", $p);
		$pdf->cell(15, $alt, db_formatar(round($quant_est,2), "p", " ", 2), 0, 0, "R", $p);
//		$pdf->cell(15, $alt, db_formatar(($quant_est==0?0:$tot_est/$quant_est), 'f'), 0, 0, "R", $p);
		$pdf->cell(15, $alt, db_formatar($preco_medio, 'f', ' ', 0, 'e', 4), 0, 0, "R", $p);
    ...
    $pdf->cell(20, $alt, db_formatar($nValorTotalSaida, "f"),"TB", 0, "R", 1);
		...
		$pdf->cell(15, $alt, db_formatar($preco_medio, 'f', ' ', 0, 'e', 4), "TB", 0, "R", $p);
