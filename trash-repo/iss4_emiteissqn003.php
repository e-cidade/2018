...
if ($tipo_debito == 3) {
  ...
  $whereescrito .= " and not exists ( select *";
  $whereescrito .= "                   from  isscadsimples";
  $whereescrito .= "     	                   left join isscadsimplesbaixa on q39_isscadsimples = q38_sequencial";
  $whereescrito .= "                   where q38_inscr = q02_inscr";
  $whereescrito .= "                     and q39_sequencial is null)";
  ...
  $sql = "
			select  q01_numpre,
    		 ...
    		 from	( select * from isscalc 
				  ...
				  left  join recibounica    on recibounica.k00_numpre     = arreinscr.k00_numpre
			where 1=1 $whereescrito $whereescrito_ativo
			          $sWhereUnicas 
			 order by $ordenaescrito
           		  $ordena 
                $limit";
...
$rsNumpres = pg_query($sql) or die($sql);
...
for($inti = 0; $inti < $intNumrowsNumpre; $inti ++) {
  ...
  for($volta = 0; $volta < sizeof($numpres); $volta ++) {
    ...
    $sqlIssBase = pg_exec("select q02_inscmu from issbase where q02_inscr = $origem");
	      ...
	      if ($resultfin != false) {
	        ...
	        for($unicont = 0; $unicont < pg_numrows($resultfin); $unicont ++) {
	          ...
	          if (! empty($HTTP_POST_VARS ["ver_matric"])) {
	            ...
	            $rsInfoEmpresa = pg_query($sqlInfoEmpresa);
	            ...
	            if ($impmodelo == 2) {
	              ...
	              $rsDetalhePgto = pg_query($sSqlDetalhe) or die($sSqlDetalhe);
	              ...
	              $resulttipo = pg_exec("select k03_tipo, k00_tipoagrup, k00_tercdigrecnormal,k00_msgrecibo  from arretipo where k00_tipo = $tipo_debito");
	              db_fieldsmemory($resulttipo, 0);
	          ...
	          if ($iTercDig == '7') {
	            ...
	            if ($tipo_debito == 3 && $emiteVal == 2) {
	              ...
	              $pdf1->descr15 = "R$ " . db_formatar($k00_valor, 'f'); //($ninfla==''?'R$'.db_formatar($k00_valor,'f'):$ninfla.''.$k00_valor);
	              $pdf1->valtotal = db_formatar($k00_valor, 'f');
	              $pdf1->descr7 = "R$ " . db_formatar($k00_valor, 'f'); //($ninfla==''?'R$'.db_formatar($k00_valor,'f'):$ninfla.''.$k00_valor); 
	            }
    ...
    $rsnumpres = pg_query($ssqlnumpres);
