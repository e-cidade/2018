...
/*
    $sqlTotalExtrato  = " select sum(k97_saldofinal) as  saldoextrato    ";
    $sqlTotalExtrato .= "	  from extratosaldo                            "; 
    $sqlTotalExtrato .= "	 where k97_contabancaria  = $k68_contabancaria ";
  //	$sqlTotalExtrato .= "	   and k97_dtsaldofinal   = '{$k68_data}'      ";
    $sqlTotalExtrato .= "	   and k97_dtsaldofinal   = ( $sqlData )      ";
    $sqlTotalExtrato .= "	 group by k97_dtsaldofinal                     ";
    $sqlTotalExtrato .= "	 order by k97_dtsaldofinal desc                ";
*/    
...
//  die($sqlTotalExtrato);
  ...
  $pdf->setfont($fonte,'',8);
  $pdf->cell(75,$alt,@$reduzido,0,1,"L",0);

  $pdf->setfont($fonte,'b',8);
  $pdf->cell(25,$alt,"AGÊNCIA : ",0,0,"L",0);
  $pdf->setfont($fonte,'',8);
  $pdf->cell(35,$alt,@$agencia,0,0,"L",0);
  $pdf->setfont($fonte,'b',8);
  $pdf->cell(40,$alt,"REDUZIDO CONTABIL: ",0,0,"L",0);
  $pdf->setfont($fonte,'',8);
  $pdf->cell(60,$alt,@$reduzido_contabil,0,1,"L",0);

  $pdf->setfont($fonte,'b',8);
  $pdf->cell(25,$alt,"CONTA : ",0,0,"L",0);
  $pdf->setfont($fonte,'',8);
  $pdf->cell(35,$alt,@$conta,0,0,"L",0);
  $pdf->setfont($fonte,'b',8);
  $pdf->cell(25,$alt,"DESCRIÇÃO : ",0,0,"L",0);
  $pdf->setfont($fonte,'',8);
  $pdf->cell(75,$alt,@$descricao,0,1,"L",0);
  ...
  $diferenca        = round($saldoConciliacao - $nTotContaCaixa,2) + 1 - 1;
