...
if(isset($e94_codanu)) {
  ...
  $re = pg_query("select distinct e94_numemp from empanulado where e94_codanu = $e94_codanu");
  if(pg_numrows($re) == 0 ){
  ...
  $re = pg_query("select distinct e94_numemp  from empanulado inner join empempenho on e94_numemp = e60_numemp $where");
  if(pg_numrows($re) == 0 ){
    db_redireciona('db_erros.php?fechar=true&db_erro=Anulação n'.chr(176).' '.$e94_codanu.' não encontrada. Verifique!');
  }
  db_fieldsmemory($re,0);
...
$result02 = pg_query("select * from empanulado $where1 ");
