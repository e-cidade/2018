...
if (isset($pc10_numero) && $pc10_numero != "") {
  $sOrigem = "solicitacao";
} else if (isset($pc80_codproc) && $pc80_codproc != "") {
  $sOrigem = "processo";
}
...
if ($sOrigem == "solicitacao") {
  ...
  $sSqlMater = $clpcorcamitem->sql_query_pcmatersol(null, "pc11_codigo, pc11_seq,pc22_codorc, 1 as l20_tipojulg", null, "pc11_numero=$pc10_numero");
  ...
  $sSqlMater = $clpcorcamitem->sql_query_pcmaterproc(null, "pc81_codproc, pc11_seq,pc22_codorc, 1 as l20_tipojulg", null, "pc81_codproc=$pc80_codproc");
  $sWhere = "  pc81_codproc =  $pc80_codproc  ";
...
if ($sOrigem == "solicitacao") {
  ...
  $head5 = "Processo de Compra: $pc80_codproc";
}
...
if ($modelo == 1) {
  ...
  for($x = 0; $x < $numrows_forne; $x ++) {
    ...
    if ($pdf->gety() > $pdf->h - 30 || $troca != 0) {
      ...
      if ($sOrigem == "solicitacao") {
        $sSqlMater = $clpcorcamitem->sql_query_pcmatersol(null, "distinct pc11_seq, pc22_orcamitem, pc01_descrmater",
                                                                "pc11_seq", "pc22_codorc=$orcamento");
      } else if ($sOrigem == "processo") {
        $sSqlMater = $clpcorcamitem->sql_query_pcmaterproc(null, "distinct pc11_seq, pc22_orcamitem, pc01_descrmater",
                                                                 "pc11_seq", "pc22_codorc=$orcamento");
      }
  ...
  for($x = 0; $x < $numrows_itens; $x ++) {
    ...
    if ($clpcorcamval->numrows > 0) {
      db_fieldsmemory($result_valor, 0);
      if ($pc24_pontuacao == 1) {
        $pdf->cell(20, $alt, db_formatar(@$pc23_valor, 'f'), 0, 1, "R", $p);
        $valor_tot += $pc23_valor;
      } else {
        $pdf->cell(20, $alt, db_formatar(@$pc23_valor, 'f'), 0, 1, "R", $p);
      }
    } else {
      $pdf->cell(20, $alt, db_formatar("0", 'f'), 0, 1, "R", $p);
    }
  ...
  for($x = 0; $x < $numrows_itens; $x ++) {
    ...
    $alt = 4;
    $pdf->setfont('arial', '', 7);
    $pdf->cell(20, $alt, $pc11_seq, 1, 0, "C", 0);
