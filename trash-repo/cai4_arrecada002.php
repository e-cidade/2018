...
if (! isset($HTTP_POST_VARS ["reduz"])) {
  $sqlconta = "select k13_reduz as c01_reduz, k13_descr as c01_descr 
    from cfautent 
    inner join cfautentconta on k16_id = k11_id 
    inner join saltes on k13_conta = k16_conta
    ...
    where k11_ipterm = '" . db_getsession("DB_ip") . "' and 
    k11_instit = " . db_getsession("DB_instit");
  $resultconta = db_query($sqlconta);
...
if (isset($HTTP_POST_VARS ["codrec"]) || isset($codrec)) {
  ...
  //echo "tam= $tamCodigoArrecadacao numpre = $codigoArrecadacao ";
  // exit;
  ...
  if ($naocalcula == true) {
    if (strlen(trim($codrec)) > 15) {
      ...
      if ( $banco == "816" || 
        ...
        if ($banco == "001") {
	  if ( $banco == "001" and strlen(trim($codrec)) == 44 ) { // BDL Banco do Brasil
	    $oSql   = db_utils::fieldsMemory($rsSql,0);
    ...
    if (isset($valor_variavel)) {
      ...
      $result = db_query("update issvar set q05_vlrinf = " . ($valor_variavel + 0) . " 
        where q05_numpre = " . substr($codrec, 0, 8) . " and q05_numpar = " . substr($codrec, 8, 3));
    }
    
    $result = db_query("select fc_calcula(substr('$codrec',1,8)::integer,substr('$codrec',9,3)::integer,0,'" . date("Y-m-d", db_getsession("DB_datausu")) . "'::date,'" . date("Y-m-d", db_getsession("DB_datausu")) . "'::date," . db_getsession("DB_anousu") . ")");
...
<?
        ...
        if (substr($fc_calcula, 0, 1) == '4' || substr($fc_calcula, 0, 1) == '2') {
          ...
          if (substr($fc_calcula, 0, 1) == '4') {
            if (! empty($vlrhist)) {
              //echo "<br>hist".$vlrhist;
              //echo "<br>corrigido".$vlrcor;
        ...
        //	 echo("vlrcor: $vlrcor - juros: $vlrjuros - multa: $vlrmulta - desconto: $vlrdesconto - vlrimp: $vlrimp\n");
        //	 exit;
        ?>
<script>
...
// echo "select fc_calcula(substr('$codrec',1,8),substr('$codrec',9,3),0,'2003-04-08','2003-04-08',2003)";
// $result = db_query("select fc_calcula(substr('$codrec',1,8),substr('$codrec',9,3),0,'2003-04-08','2003-04-08',2003)");
// db_fieldsmemory($result,0);
//  echo $fc_calcula;*/
...
function js_autenticar(lMostra) {
  ...
  if(lista.rows.length == 1)
    ...
    if(system_os.system =='Windows'){
      ...
      parent.js_OpenJanelaIframe("top.corpo",'db_autent_iframe','cai4_arrecada003.php?reduz='+document.form1.reduz.value+'&system=linux','Autenticação',false);  
    }
  }
}
...
<body bgcolor=#CCCCCC bgcolor="#AAB7D5" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"
  onload="document.form1.codrec.focus()";>
...
$sql = "select k13_reduz as c01_reduz,
  ...
  k13_conta
  from saltes
  inner join conplanoreduz on k13_reduz  = c61_reduz and 
...
<table width="99%">
  <tr>
    ...
    <td align="left" valign="middle"><select onChange="js_atualizaconta(this.name)" name="reduz" id="reduz">
<?
...
?>
</select> &nbsp; <select onChange="js_atualizaconta(this.name)" name="descr" id="descr">
<?
...
?>
    ...
    <td align="center" valign="middle"><input name="anteriores" type="button" id="anteriores" onClick="js_anteriores();"
      value="Anteriores"></td>
    <td align="right" valign="middle"><input name="banco" type="button" id="banco" onClick="js_baixabanco();" value="Baixa Banco"></td>
  </tr>
  <tr>
    <td width="13%" height="26" align="left" valign="middle"><b>Autenticar:</b></td>
    <td width="65%" align="left" valign="middle"><input name="codrec" type="text" id="codrec" size="30" maxlength="100"
      value="<?=(isset($valor_variavel) && ! isset($calcula) ? $codrec : "")?>" onchange="return js_quantdig(this.value);"></td>
    <td width="11%" align="center" valign="middle"><input name="calcula"
      type="<?=(isset($valor_variavel) && ! isset($calcula) ? "hidden" : "submit")?>" id="calcula" value="  Calcula "
      onclick="return js_quantdig(0);"></td>
    <td width="11%" align="right" valign="middle"><input name="autenticar" type="button" id="autenticar"
      onClick="js_autenticar(false)" value="Autenticar"><a onclick="js_autenticar(true)">&nbsp</a></td>
  </tr>
</table>
</form>
</body>
