...
class cl_turnoreferente { 
   ...
   var $campos = "
                 ed231_i_codigo = int8 = Código 
                 ed231_i_turno = int8 = Turno 
                 ed231_i_referencia = int4 = Referência do Turno 
                 ";
   ...
   function incluir ($ed231_i_codigo){ 
     ...
     if($this->ed231_i_referencia == null ){ 
       $this->erro_sql = " Campo Referência do Turno nao Informado.";
     ...
     if($ed231_i_codigo == "" || $ed231_i_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $ed231_i_codigo)){
         $this->erro_sql = " Campo ed231_i_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Referencias do Turno ($this->ed231_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "Referencias do Turno já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "Referencias do Turno ($this->ed231_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($ed231_i_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Referencias do Turno nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Referencias do Turno nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$ed231_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
