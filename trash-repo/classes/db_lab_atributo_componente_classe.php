...
require_once ("classes/db_lab_atributo_classe.php");
require_once ("classes/db_lab_exameatributoligacao_classe.php");
...
require_once ("classes/db_lab_examedisp_classe.php");
...
//fun��es de interface da classe
    ...
    * @param $irequiitem   - se for informado o irequiitem vai carregar os dados do banco referente ao requiitem
    * @param $db_opcao     -  modod de exibiçao dos dados
    * @param $fieldset     - define se vai exibir o fieldset 1 para sim 2 para não
    * @param $ibordas      - 0 sem bordas de 1 acima grossura da borda
    */
   function atributos($iExame,$iAtributo_pai,$iRequiitem = "",$db_opcao = 1,$fieldset = 1,$iBordas = 1){ 
...
//fun��es internas da classe
    ...
    * @param $iQuant - Qauntidade de espaço que ser dada para dividir cada nivel
    */
    ...
    * @return Objeto com com parametros codigo,unidade,alfanumerino,numerico,etc... 
    */
   function dadosRef($iAtributo_codigo){
        
   	    $sCampos=" la30_i_codigo,
   	               la30_f_normalmin,
                   la30_f_normalmax,
                   la30_f_absurdomin,
                   la30_f_absurdomax,
   ...
   /**
    * carrega o dado ja lançado na referentecia para determinado intem da requisicao
    * @param $iAtributo_codigo
    * @param $itipo define qual o tipo de dados 1 numerico, 2 alfanumerico, 3 selecionave 
    * @return inteiro com o valor do referencial numerico
    */
   function loadRef($iAtributo_codigo,$iTipo = 1){
        ...
        $sSql = cl_lab_resultado::sql_query_item("",$sCampos,""," la39_i_atributo=$iAtributo_codigo and la52_i_requiitem=$this->irequiitem ");
        $rResult = cl_lab_resultado::sql_record($sSql);
        if ($rResult != false) {
                
        	$oValor = db_utils::getColectionByRecord($rResult);
    ...
    * @param $iAtributo_codigo
    * @return Objeto com com parametros codigo,nome,estrutural,tipo,nivel
    */
   ...
   function levantamentoFilhos($iPai_atributo){
        ...
        $sSql = cl_lab_exameatributoligacao::sql_query_file("",$sCampos," la26_i_exameatributofilho "," la26_i_exameatributopai=$iPai_atributo ");
        $rResult = cl_lab_atributo::sql_record($sSql);
        if($rResult!=false){
        	if(pg_num_rows($rResult)>0){
           	  
              $aitens = db_utils::getColectionByRecord($rResult);
	      ...
	      if ($oValorRef->alfanumerico != "" ) {
	      	  if ($oValorRef->fixo > 0) {
		          ...
		          $this->aRepositorios[]="inp_A$iAtributo_codigo#1";
	      	 	  $sStr="<input type='text' tabindex=\"".count($this->aRepositorios)."\"  onkeydown=\"return js_controla_tecla_enter(this,event);\" name='inp_A$iAtributo_codigo' id='inp_A$iAtributo_codigo' value='$sValorRefLoad' size=\"20\"  >";
		          ...
		          $sStr="<select tabindex=\"".count($this->aRepositorios)."\" onkeydown=\"return js_controla_tecla_enter(this,event);\" name=\"sel_A$iAtributo_codigo\" id=\"sel_A$iAtributo_codigo\" >";
		         ...
		         $this->aRepositorios[]="inp_A$iAtributo_codigo#3";
		   	     $sStr="$oValorRef->calculavel<input type='text' onkeydown=\"return js_controla_tecla_enter(this,event);\" tabindex=\"".count($this->aRepositorios)."\" name='inp_A$iAtributo_codigo' id='inp_A$iAtributo_codigo' value='$iValorRefLoad' onchange=\"js_ValidaValor$iAtributo_codigo(this.value)\" size=\"5\"  > $oValorRef->unidade";
                 $sStr.=" <script>
                            function js_ValidaValor$iAtributo_codigo(valor){
	                            if((valor<$oValorRef->absurdomin)||(valor>$oValorRef->absurdomax)){
	                                alert('O Valor deve estar entre: \\n       $oValorRef->absurdomin at� $oValorRef->absurdomax');
   ...
   function infoValorRef($iAtributo_codigo){
	   ...
	   if($oValorRef != false){   
	      if ($oValorRef->alfanumerico == "" ) {
		   	     $sStr= $oValorRef->normalmin." at�  ".$oValorRef->normalmax;
          }
	   }
	   return $sStr;  	
   }   
   ...
   /**
    * Verifica se o atributo é valido para este exame
    * @param $iPai_atributo
   ...
   /**
    * Fun��o recursiva que monta as linhas da tabela de edição e exibição dos atributos
    * @param $iPai_atributo - codigo do atributo pai
    * @param $cPai_estrutural - estrutural do atributo pai
    * @param $cPai_nome - descrição do atributo pai
    * @param $cPai_nivel - nivel do atributo pai
