...
//MODULO: atendimento
//CLASSE DA ENTIDADE db_projetosgrupos
class cl_db_projetosgrupos { 
   ...
   function cl_db_projetosgrupos() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("db_projetosgrupos"); 
   ...
   function incluir ($at63_sequencial){ 
     ...
     $sql = "insert into db_projetosgrupos(
                                       at63_sequencial 
                                      ,at63_projeto 
                                      ,at63_grupo 
                       )
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Grupos de projetos ($this->at63_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "Grupos de projetos já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "Grupos de projetos ($this->at63_sequencial) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function alterar ($at63_sequencial=null) { 
      $this->atualizacampos();
     $sql = " update db_projetosgrupos set ";
     $virgula = "";
   ...
   function excluir ($at63_sequencial=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Grupos de projetos nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$at63_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:db_projetosgrupos";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   // funcao do sql 
   function sql_query ( $at63_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
     ...
     $sql .= " from db_projetosgrupos ";
     $sql .= "      inner join db_projetos  on  db_projetos.at60_codigo = db_projetosgrupos.at63_projeto";
     $sql .= "      inner join db_projetoscadgrupos  on  db_projetoscadgrupos.at62_codigo = db_projetosgrupos.at63_grupo";
     $sql .= "      inner join db_projetosituacao  on  db_projetosituacao.at61_codigo = db_projetos.at60_situacao";
     $sql2 = "";
     if($dbwhere==""){
       if($at63_sequencial!=null ){
         $sql2 .= " where db_projetosgrupos.at63_sequencial = $at63_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
   ...
   // funcao do sql 
   function sql_query_file ( $at63_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = split("#",$campos);
       $virgula = "";
