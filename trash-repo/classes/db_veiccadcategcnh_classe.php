...
class cl_veiccadcategcnh { 
   ...
   function incluir ($ve30_codigo){ 
     ...
     if($this->ve30_descr == null ){ 
       $this->erro_sql = " Campo Descrição Categoria CNH nao Informado.";
     ...
     if($ve30_codigo == "" || $ve30_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $ve30_codigo)){
         $this->erro_sql = " Campo ve30_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Cadastro de Categorias CNH ($this->ve30_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "Cadastro de Categorias CNH já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "Cadastro de Categorias CNH ($this->ve30_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($ve30_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Cadastro de Categorias CNH nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "Cadastro de Categorias CNH nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$ve30_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
