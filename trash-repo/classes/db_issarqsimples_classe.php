	...
	function validaArquivo($iCodigoArquivo) {
		...
		foreach ($aRegistros as $iIndiceRegistros => $oRegSimples) {
			...
			if ($oRegSimples->q23_acao == 1 && $oRegSimples->quantidade_cgm == 1) {
				...
				$sCamposIssBase.= "     and (q02_dtbaix is null or q02_dtbaix > '".date("Y-m-d",db_getsession("DB_datausu"))."') \n";
        ...
        $sWhereIssBase .= " and (q02_dtbaix is null or q02_dtbaix > '".date("Y-m-d",db_getsession("DB_datausu"))."') \n";
        ...
        $rsBase         = $oDaoIssBase->sql_record($sSqlIssBase);
          ...
          $oIssBase       = db_utils::fieldsMemory($rsBase, 0);
				...
				if (empty($oIssBase->inscricoes)) {
					...
					$oErros->iRegistro = $oRegSimples->q23_sequencial;
					$oErros->sCnpj     = $oRegSimples->q23_cnpj;
					$oErros->sDetalhe  = urlencode("CNPJ - {$oRegSimples->q23_cnpj}, CGM -{$oRegSimples->cgm_valido}: Cadastro com Mais de um álvara ({$oIssBase->inscricoes}) ou sem vínculo.");
		      ...
		      $sSqlIssCalc = $oDaoIssCalc->sql_query(null, null, null, null, null, "*", null, $sWhereIssCalc);
					...
					if ( pg_num_rows($rsIssCalc) == 0 ) {
						...
						$oErros->sTipo     = "AVISO";
						$oErros->sCnpj     = $oRegSimples->q23_cnpj;
						$oErros->sDetalhe  = urlencode("CNPJ - {$oRegSimples->q23_cnpj}, CGM - {$oRegSimples->cgm_valido}, Álvara - {$oIssBase->inscricoes}: ");
					...
					$sWhereIssVar  = "     q05_ano    = {$oRegSimples->q23_anousu} ";
					$sWhereIssVar .= " and q05_mes    = {$oRegSimples->q23_mesusu} ";
					$sWhereIssVar .= " and k00_inscr  in ({$oIssBase->inscricoes}) "; 
					$sSqlIssvar = $oDaoIssvar->sql_query_arreinscr(null, "*", null, $sWhereIssVar);
	...
	function processaArquivo($iRegistro, $sArquivo, $iBanco, $iAgencia, $iConta) {
		...
		foreach ($aRegistros as $iIndiceRegistros => $oRegSimples) {
			...
			if ($oRegSimples->q23_acao == 1 && $oRegSimples->quantidade_cgm == 1) {
   		  ...
   		  if (pg_num_rows($rsIncricaoVinculada) > 0 ) {
  	 		 ...
  	 		 */
				...
				$sWhereIssBase  = "exists(select 1                                                                           ";
				$sWhereIssBase .= "        from issarqsimplesregissbase                                                      ";
				$sWhereIssBase .= "       where q134_inscr = issbase.q02_inscr                                               ";
				$sWhereIssBase .= "         and q134_issarqsimplesreg = {$oRegSimples->q23_sequencial})                      ";
				$sWhereIssBase .= "and z01_numcgm = {$oRegSimples->cgm_valido}                                               ";
				$sWhereIssBase .= "and (q02_dtbaix is null or q02_dtbaix > '".date("Y-m-d",db_getsession("DB_datausu"))."')  ";
					
				$oDaoIssBase    = db_utils::getDao("issbase");
				$sSqlIssBase    = $oDaoIssBase->sql_query(null, "q02_inscr,z01_numcgm,z01_cgccpf", null, $sWhereIssBase);
				$rsBase         = $oDaoIssBase->sql_record($sSqlIssBase);
				...
				if ($oDaoIssBase->numrows == 0) {
					...
					throw new Exception(urldecode("CNPJ - {$oRegSimples->q23_cnpj}, CGM -{$oRegSimples->cgm_valido}:  Cadastro com Mais de um álvara ({$sInscr}), Registro: {$oRegSimples->q23_sequencial} "));
				  ...
				  $oInscr      = $aIssBase[0];
					$oDaoIssCalc = db_utils::getDao("isscalc");
					$oDaoIssvar  = db_utils::getDao("issvar");
					$sSqlIssCalc = $oDaoIssCalc->sql_query($oRegSimples->q23_anousu,$oInscr->q02_inscr,3);
					
					
					$rsIssCalc   = db_query($oDaoIssCalc->sql_query($oRegSimples->q23_anousu,$oInscr->q02_inscr,3));

					$rsVar = db_query($oDaoIssvar->sql_query_arreinscr(null,
							"*",
							null,
			...
			if ($oDaoIssArqSimplesRegIssVar->erro_status == 0){
				...
				$oDaoIssvar             = db_utils::getDao('issvar');
				$oDaoIssvar->q05_vlrinf = $dVlrReg;
				$oDaoIssvar->q05_codigo = $q05_codigo;
				$oDaoIssvar->alterar($q05_codigo);

				if ($oDaoIssvar->erro_status == 0){
					throw new Exception( $oDaoIssvar->erro_msg . "{$dVlrReg}");
				}
