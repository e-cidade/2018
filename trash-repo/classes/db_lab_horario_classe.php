...
class cl_lab_horario { 
   ...
   function incluir ($la35_i_codigo){ 
     ...
     if($la35_i_codigo == "" || $la35_i_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $la35_i_codigo)){
         $this->erro_sql = " Campo la35_i_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "lab_horario ($this->la35_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "lab_horario já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "lab_horario ($this->la35_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($la35_i_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "lab_horario nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "lab_horario nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$la35_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
