...
if(@$q02_inscr!=""){
  $sqliss = "select * from issbase where q02_inscr = $q02_inscr";
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" >
  <tr>
    <td >
    <br>
       <fieldset><Legend align="left"><b>Endereço no Município</b></Legend>
        <table border="0" cellspacing="0" cellpadding="0" width="100%" >
<?
...
?>  
  ...
  if($db_opcao!=33 && $db_opcao!=22 && strtoupper($munic)!=strtoupper($z01_munic)){
    ...
    db_input('q02_numero',10,@$q02_numero,true,'text',$db_opcao);
    ?>
    </td>
    <td align="right">
  	 <?=@$Lq02_compl?>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" >
  <tr>
    <td nowrap title="<?=@$Tq02_fanta?>" width='200'>
       <?=@$Lq02_fanta?>
    </td>
    <td colspan="2"> 
<?
!isset($q02_fanta)&&$db_opcao!=33&&$db_opcao!=22?$q02_fanta=$z01_nome:"";
db_input('q02_fanta',50,$Iq02_fanta,true,'text',$db_opcao,"")
?>
  ...
  db_input('z01_nome',53,$Iz01_nome,true,'text',3,'','z01_nome_escrito',"E6E4F1");
  ?>
      <td>
    </tr>
  <tr>
...
?>
  <input name="<?=($db_opcao==1?"incluir":($db_opcao==2 || $db_opcao==22?"alterar":"excluir"))?>" type="<?=($db_opcao==1?"button":($db_opcao==2 || $db_opcao==22?"button":"submit"))?>" id="db_opcao" value="<?=($db_opcao==1?"Incluir":($db_opcao==2 || $db_opcao==22?"Alterar":"Excluir"))?>" <?=($db_botao==false?"disabled":"")?>   <?=$db_opcao==1?"onclick='js_testaproc(1);'":($db_opcao==2?"onclick='js_testaproc(2);'":"") ?> >
...
function js_testaproc(opc){
  if (opc==1) {
    if (document.form1.j14_codigo.value=="") {
      alert("Codigo do Logradouro não está preenchido!!");
      document.form1.j14_codigo.focus();
    }
    if (document.form1.j13_codi.value=="") {
      //alert("Codigo do bairro não está preenchido!!");
