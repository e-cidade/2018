...
select rh02_regist as r01_regist,
       ...
       case when r14_rubric in ('R918','R920') then r14_valor else 0 end as ded
from gerfsal 
...
where r14_anousu = $ano 
  ...
  and r14_rubric in ('R992')
  and rh02_tbprev in (2)
...
select rh02_regist as r01_regist,
       ...
       case when r48_rubric in ('R918','R920') then r48_valor else 0 end as ded
from gerfcom
...
select rh02_regist as r01_regist,
       ...
       case when r20_rubric in ('R918','R920') then r20_valor else 0 end as ded
from gerfres
...
for($x = 0; $x < pg_numrows($result);$x++){
   ...
   if ($pdf->gety() > $pdf->h - 30 || $troca != 0 ){
      ...
      $pdf->cell(15,$alt,'BASE',1,0,"R",0);
      $pdf->cell(15,$alt,'PATR.',1,0,"R",0);
      $pdf->cell(15,$alt,'DED.',1,0,"R",0);
      $pdf->cell(15,$alt,'DIVIDA',1,0,"R",0);
      $pdf->cell(15,$alt,'TOTAL',1,1,"R",0);
      $troca = 0;
   }
   $pdf->setfont('arial','B',8);
   if($orgao != $rh26_orgao){
...
//     }
     ...
     $pdf->cell(15,$alt,db_formatar($inss,'f'),0,0,"R",0);
     $pdf->cell(15,$alt,db_formatar($pat,'f'),0,0,"R",0);
     $pdf->cell(15,$alt,db_formatar($ded,'f'),0,0,"R",0);
     $pdf->cell(15,$alt,db_formatar($pat_div,'f'),0,0,"R",0);
     $pdf->cell(15,$alt,db_formatar(($pat + $pat_div - $ded),'f'),0,1,"R",0);
//   }
//   if(db_formatar($rh26_orgao,'orgao').db_formatar($rh26_unidade,'orgao') == '0203'){ 
//     $val_pat      += (($inss+$sub)/100)*20;
//   }else{
   $val_pat      += (($inss+$sub)/100)*11;
   $val_pat_div  += (($inss+$sub)/100)*9;
   $val_fgts     += $inss+$sub;
   $val_ded      += $ded;
   }
...
//echo $teste;exit;
   ...
   $pdf->cell(15,$alt,db_formatar($val_fgts,'f'),0,0,"R",0);
   $pdf->cell(15,$alt,db_formatar($val_pat,'f'),0,0,"R",0);
   $pdf->cell(15,$alt,db_formatar($val_ded,'f'),0,0,"R",0);
   $pdf->cell(15,$alt,db_formatar($val_pat_div,'f'),0,0,"R",0);
   $pdf->cell(15,$alt,db_formatar($val_pat + $val_pat_div -$val_ded,'f'),0,1,"R",0);
