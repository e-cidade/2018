...
if (isset($tipo_usuario) && trim(@$tipo_usuario) != ""){
     if (trim(@$tipo_usuario) == "T"){
          $cabec_usuarios_situacao = "SITUACAO: TODOS";
     }

     if (trim(@$tipo_usuario) == "1"){
          $cabec_usuarios_situacao = "SITUACAO: ATIVOS";
     }
     
     if (trim(@$tipo_usuario) == "0"){
          $cabec_usuarios_situacao = "SITUACAO: INATIVOS";
     }
}
...
for($a=0; $a<$numrows_executa; $a++){
  ...
  if($pdf->gety() > $pdf->h - 35 || $a == 0 || $testaUsuario != $id_usuario){
    ...
    $pdf->cell(0,$alt,$nome . " [Login: " . $login . "] - [Status: ".trim($status)."]" . " - [Tipo de usuario: $descr_tipo_usuario]","T",1,"L",1);
  }
