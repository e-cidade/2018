...
$rsResultpar = pg_query("select * from parissqn");
...
if (isset($q60_modalvara) && $q60_modalvara == "3") {
  ...
  if ($q07_perman == 'f') {
    ...
    $sql_tipoalvara = "select db_paragrafo.*
      ...
      order by db02_descr
      ";
    $res_tipoalvara = pg_query($sql_tipoalvara);
  ...
  /*$sql = "select munic,nomeinst from db_config where codigo = ".db_getsession();
      $result = pg_query($sql);
      if(pg_numrows($result) > 0){
      db_fieldsmemory($result, 0);
      }*/
...
$rsCodCnae = pg_query($sSqlCodCnae);
