...
class calendario {
  ...
  function cria($dia,$mes,$ano,$marca=0,$str_where, $result_diassemana,$fechar=false,$iQuantidade) {
    ...
    $oDaoLabAusencia    = db_utils::getdao('lab_ausencia');
    $oDaoLabParalizacao = db_utils::getdao('lab_paralizacao');
        ...
        $booMostradiv     = false;
          $str_query = "select la35_i_codigo,
                ...
                and ( la35_d_valinicio is null or 
                     ...
                     ";
          //echo" SQL = [ $str_query ] ";
          $rsHorario = pg_query( $str_query ) or die( "ERRO: <p> $str_query ");
        ...
        <tr>
         <td colspan='4'>
           ...
           <tr align=\"center\">
             <td width=\"100%\" colspan=\"7\" nowrap>
              <FONT SIZE='1' FACE='Verdana' COLOR='black'>
               <a href=\"func_calendariolaboratorio.php?".($this->shutdown_function!=""?"shutdown_function=".$this->shutdown_function."&":"")."nome_objeto_data=".$this->nome_objeto_data."&mes_solicitado=".($mes)."&ano_solicitado=".($ano-1)."&".$str_where." \"> << </a>
                      $ano
             <a href=\"func_calendariolaboratorio.php?".($this->shutdown_function!=""?"shutdown_function=".$this->shutdown_function."&":"")."nome_objeto_data=".$this->nome_objeto_data."&mes_solicitado=".($mes)."&ano_solicitado=".($ano+1)."&".$str_where." \"> >> </a>   
              </font>
             </td>
           ...
           <tr align=\"center\">
             <td width=\"100%\" colspan=\"7\" nowrap>
              <FONT SIZE='1' FACE='Verdana' COLOR='black'>
               <a href=\"func_calendariolaboratorio.php?".($this->shutdown_function!=""?"shutdown_function=".$this->shutdown_function."&":"")."nome_objeto_data=".$this->nome_objeto_data."&ano_solicitado=".($ano)."&mes_solicitado=".($mes-1)."&".$str_where."\"> << </a>
               ".$this->mes[$mes]."
               <a href=\"func_calendariolaboratorio.php?".($this->shutdown_function!=""?"shutdown_function=".$this->shutdown_function."&":"")."nome_objeto_data=".$this->nome_objeto_data."&ano_solicitado=".($ano)."&mes_solicitado=".($mes+1)."&".$str_where." \"> >> </a>
...
if (  isset($la09_i_codigo) && (int)@$la09_i_codigo != 0  ) {
  ...
  $str_query = "select distinct la35_i_diasemana
                  from lab_horario 
                 inner join lab_setorexame on la09_i_codigo = la35_i_setorexame
                 where $str_where
                 order by la35_i_diasemana   
                ";

  $result     = pg_query($str_query) or die("ERRO: nos horÃ¡rios para o exame.");
