...
$sCampos   .= ", ed52_i_ano as anocalendario, trim(ed57_c_descr) as turma, ed15_c_nome as turno ";
...
foreach ($aMatricula as $iMatricula) {
  ...
  $oPdf->Cell(7,   $iHeigth, "{$oDadosTurma->escola}" , 0, 1);
  ...
  //$oPdf->Cell($iTamanhoColunaPeriodo,  8, $sFormaAvaliacao, 1, 0, "C");
  ...
  foreach ($aGradeAluno as $oGradeAluno) {
    ...
    foreach ($oGradeAluno->aAproveitamento as $oAproveitamento) {
      ...
      if ($oAproveitamento->sFormaAvaliacao == "PARECER") {
    ...
    foreach ($oGradeAluno->aAproveitamento as $oAproveitamento) {
      ...
      $aTotalFaltas[$oAproveitamento->sDescricao][] = $oAproveitamento->oAproveitamento->iFaltas;
    }
    /**
