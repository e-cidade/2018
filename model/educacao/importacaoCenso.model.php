...
abstract class importacaoCenso {
   ...
   */
  function getCartorio($iCodigoCensoCartorio = null, $sNomeCartorio = null) {
  ...
  if ($oDaoRechumano->numrows > 0) {      
    return $aDadosRechumano = db_utils::getColectionByRecord($rsRechumano, false, false, false);           
  } else {
    return null;
  } //fecha o else 
...
function validaArquivo() {
  ...
  if ($this->lLayoutComPipe) {
    ...
    $aData  = explode("/", $dDta);
  	...
  	$aLinha[0] = substr($sLinha, 0,  2);
  	$aLinha[1] = substr($sLinha, 12, 8);
  	$aLinha[2] = substr($sLinha, 25, 4);
  ...
  } elseif ($this->lInepEscola) {
    
  	if ($aLinha[1] != $iInepBanco) {
  ...
  } elseif ($this->iAnoEscolhido != $aData[2]) {
  ...
  while (!feof($pArquivoCenso)) {
    ...
    if ($aLinha[0] != "00" && $aLinha[0] != "10" && $aLinha[0] != "20" 
        && $aLinha[0] != "30" && $aLinha[0] != "40" && $aLinha[0] != "50"
        && $aLinha[0] != "51" && $aLinha[0] != "60" && $aLinha[0] != "70" 
        && $aLinha[0] != "80") {
  ...
  * @param object $oLinha com os campos contidos em uma linha de importacao (conforme seu tipo de registro)
  ...
  function importarArquivo() {            
    ...
    foreach ($aLinhasArquivo as $iIndLinha => $oLinha) {   
      ...
      }//fecha o if que $this->lImportarEscola
      ...
      }//fecha o if que $this->lImportarTurma
      ...
      }//fecha o if que $this->lImportarDocente
      ...
      if ($this->lImportarAluno) {
       ...
       }//fecha o if $this->lImportarAluno
