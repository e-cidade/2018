...
class CadvencdescIPTU implements iViradaIPTU {
  ...
  function __construct() {
    ...
    $sSqlIptuTabelasConfig  = $oDaoIptuTabelasConfig->sql_query(null, "iptutabelasconfig.j122_sequencial", 
                                                                null, "db_sysarquivo.nomearq = 'cadvencdesc'");
    ...
    $oArretipoTipo = db_utils::fieldsMemory($rsSqlArretipoTipo, 0);
    $this->setTipo($oArretipoTipo->k00_tipo);
  ...
  public function vira() {
    ...
    $sSqlCadVencDesc  = $oDaoCadVencDesc->sql_query_file($this->getCodigo(), "cadvencdesc.*", null, "");
    $rsSqlCadVencDesc = $oDaoCadVencDesc->sql_record($sSqlCadVencDesc);
    $iNumRows = $oDaoCadVencDesc->numrows;
    if ($iNumRows > 0) {
    	
    	for ( $iInd=0; $iInd < $iNumRows; $iInd++ ) {
    		
	      $oDadosCadVencDesc  = db_utils::fieldsMemory($rsSqlCadVencDesc,$iInd);
	      $aCamposCadVencDesc = get_object_vars($oDadosCadVencDesc);
	      
	      foreach ($aCamposCadVencDesc as $sNomeCampoCadVencDesc => $sValorCampoCadVencDesc ) {
	      	
	        if (in_array($sNomeCampoCadVencDesc, $this->getCampoCorrecao())) {
	        	
            $nPercentual     = $this->getPercentual();
            $nSomaPercentual = ($oDadosCadVencDesc->$sNomeCampoCadVencDesc + ($oDadosCadVencDesc->$sNomeCampoCadVencDesc * ($nPercentual / 100)));
	      ...
	      if ($oDaoCadVencDesc->erro_status == 0) {
	        throw new Exception($oDaoCadVencDesc->erro_msg);
	      }
	      
	      /*
	       * Altera o código vencimento para o ano de exercicío novo na tabela cfiptu
	       */
	      $oDaoCfIptu             = db_utils::getDao('cfiptu');
	      $oDaoCfIptu->j18_anousu = $this->getAnoNovo();
	      $oDaoCfIptu->j18_vencim = $oDaoCadVencDesc->q92_codigo;
	      $oDaoCfIptu->alterar($oDaoCfIptu->j18_anousu);
    	  if ($oDaoCfIptu->erro_status == 0) {
          throw new Exception($oDaoCfIptu->erro_msg);
        }
    	}
    }
