...
class IptutaxaIPTU implements iViradaIPTU {
  ...
  function __construct() {
    ...
    $sSqlIptuTabelasConfig  = $oDaoIptuTabelasConfig->sql_query(null, "iptutabelasconfig.j122_sequencial", 
                                                                null, "db_sysarquivo.nomearq = 'iptutaxa'");
    ...
    if ($oDaoIptuTabelasConfig->numrows > 0) {
      ...
      if ($oDaoIptuTabelasConfigVirada->numrows > 0) {
        ...
        $sMensagem = "ERRO: Tabela iptutaxa já foi feito virada anual para exercicío {$this->getAnoNovo()}!";
    ...
    $sWhere                           = "db_sysarquivo.nomearq = 'iptutaxa'";
  ...
  public function vira() {
    ...
    $oDaoIptuTaxa  = db_utils::getDao('iptutaxa');
    
    $oDaoIptuTaxa->excluir(null, null, "j19_anousu = {$this->getAnoNovo()}");
    if ($oDaoIptuTaxa->erro_status == 0) {
      throw new Exception($oDaoIptuTaxa->erro_msg);
    }
    
    $sWhere       = " j19_anousu = {$this->getAnoAtual()} ";
    $sSqlIptuTaxa  = $oDaoIptuTaxa->sql_query_file(null, null, "*", null, $sWhere);
    $rsSqlIptuTaxa = $oDaoIptuTaxa->sql_record($sSqlIptuTaxa);
    $iNumRows = $oDaoIptuTaxa->numrows;
    if ($iNumRows > 0) {

    	for ( $iInd=0; $iInd < $iNumRows; $iInd++ ) {
    		
    		$oDadosIptuTaxa  = db_utils::fieldsMemory($rsSqlIptuTaxa, $iInd);
        $aCamposIptuTaxa = get_object_vars($oDadosIptuTaxa);
        
        foreach ($aCamposIptuTaxa as $sNomeCampoIptuTaxa => $sValorCampoIptuTaxa ) {
        	
          if (in_array($sNomeCampoIptuTaxa, $this->getCampoCorrecao())) {
            
            $nPercentual     = $this->getPercentual();
            $nSomaPercentual = ($oDadosIptuTaxa->$sNomeCampoIptuTaxa + ($oDadosIptuTaxa->$sNomeCampoIptuTaxa * ($nPercentual / 100)));
            ...
            $oDaoIptuTaxa->$sNomeCampoIptuTaxa = $oDadosIptuTaxa->$sNomeCampoIptuTaxa;
          }
        }
        
        $oDaoIptuTaxa->incluir($iAnoUso, $iReceit);
        if ($oDaoIptuTaxa->erro_status == 0) {
          throw new Exception($oDaoIptuTaxa->erro_msg);
        }
    	}
