...
class ProgramaObjetivo {
   ...
   * @param  string $sDescricao - Descrição sucinta da meta
   * @param  string $sMeta      - Descrição completa da meta
   * @throws DBException
   */
  public function adicionaMeta($sDescricao, $sMeta) {
    
   if (empty($this->aMetas)) {
      $this->buscaMetas();
    }
    
    $oMeta = new ProgramaMeta();
    $oMeta->setMeta($sMeta);
    $oMeta->setDescricao($sDescricao);
    $this->aMetas[] = $oMeta;
  }
