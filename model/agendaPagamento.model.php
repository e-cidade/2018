 ...
 */
  ...
  function getContasRecurso($iNumEmp,$lRetornaContasVinculadas = true) {
    ...
    $sWhere         = " e50_codord = {$iNumEmp} AND (k13_limite is null or k13_limite >= '{$sDataAtual}') ";
    ...
    if ($oDaoEmpAgeTipo->numrows > 0 ) {
      ...
      $aContas = db_utils::getColectionByRecord($rsContas,false,false, $this->getUrlEncode());
    }
    return $aContas;
  }
...
//        }
  ...
  function getContasFornecedor($iNumCgm) {
    ...
    if ($oDaoPcForne->numrows > 0) {
      return db_utils::getColectionByRecord($rsContas,false,false, $this->getUrlEncode());
    }
  }
  ...
  function configurarPagamentos($dtPagamento, stdClass $oMovimento, $iCodigoOrdemAuxiliar = null, $lGerarOpAuxiliar=true) {
    ...
    if ($oMovimento->iCodForma == 0) {
       ...
       */
      if ($oMovimento->iCodForma == 3 && $oMovimento->iContaFornecedor != "con") {

        if ($oMovimento->iContaFornecedor == "") {
          throw new Exception("Movimento ({$oMovimento->iCodMov}) da Nota {$oMovimento->iCodNota} sem conta do Fornecedor");
        }
  ...
  function getMovimentosSlip($dtInicial, $dtFinal, $iAgrupar = 1, $iOrdemPagamento = null,
  ...
  $sSqlMovimentos   .= "   and k107_data between '{$sDataIni}' and '{$sDataFim}'";
    ...
    if ($oDaoEmpagemovslip->numrows > 0) {
      $aMovimentos = db_utils::getColectionByRecord($rsMovimentos,false,false, $this->getUrlEncode());
    }
    return $aMovimentos;
  }
   ...
   * @return integer
   */
  function gerarSlip($iCtaDebito, $iCtaCredito, $nValor, $dtIni, $dtFim, $lAgendar = false, $iAgrupar=1,
                     $iCodigoOrdem = null) {
