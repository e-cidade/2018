 ...
 */
  ...
  if ( isset($referenciaanterior) ) {
    if ( $iNumRowsIncricao == 1 ) {
      ...
      $result = db_query($sql);
      $iNumRowsIncricao = pg_numrows($result);
  ...
  $nPercentual    = 0;
  if($iNumRowsSocios > 0){
    $aPercentual = db_utils::getColectionByRecord($rsSqlSocios);
...
function js_Imprimebaix() {
  ...
  if (iBloqueioCertidao == 2){
    alert("usuário:\n\nSistema não permite emitir certidão de baixa para inscricões em débito!\n\n");
