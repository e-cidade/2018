...
$oAgata           = new cl_dbagata("social/declaracao_bolsafamilia.agt");
$oApiAgata        = $oAgata->api;
$sCaminhoSalvoSxw = "tmp/__declaracaobolsafamilia" . date('YmdHis') . "_" . db_getsession("DB_id_usuario") . ".sxw";

$oApiAgata->setOutputPath($sCaminhoSalvoSxw);
$oApiAgata->setParameter ('$iCodigoFamilia', $oGet->iCodigoFamilia);

try {
  $oDocumentoTemplate = new documentoTemplate(29, $oGet->iModeloImpressao);
...
if ( $oApiAgata->parseOpenOffice( $oDocumentoTemplate->getArquivoTemplate() ) ) {
  ...
  $sNomeRelatorio   = "tmp/declaracaobolsafamilia" . date('YmdHis') . "_" . db_getsession("DB_id_usuario") . ".pdf";
