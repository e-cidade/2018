...
$this->objpdf->Line($x,     $y+45, $xx+$x+182, $y+45); //horiz  5
...
if ($this->codigo_barras != null) {
  $this->objpdf->int25(10, $y+89, $this->codigo_barras, 12, 0.3);
}
...
if ($this->linhadigitavel != null) {
  $this->objpdf->Text($x+61,  $y+7,$this->linhadigitavel);
}
...
$this->objpdf->text($x+103, $y+19, "CNPJ");
...
if (isset($this->sTituloInstrucoes) && trim($this->sTituloInstrucoes) != '') {
  $this->objpdf->Text($x+3, $y+47, $this->sTituloInstrucoes);
} else {
  $this->objpdf->Text($x+3, $y+47, "Instruções");
}
...
$this->objpdf->Text($x+3,   $y+86, "Pagador/Avalista");
...
if (isset($this->sCedenteBoleto) && trim($this->sCedenteBoleto) != '') {
  $this->objpdf->Text($x+3, $y+22, $this->sCedenteBoleto);              // cedente
} else {
  $this->objpdf->Text($x+3, $y+22, $this->prefeitura);                  // cedente
}

$this->objpdf->SetFont('Arial', '', 6);
$this->objpdf->Text($x+103,  $y+22, db_formatar($this->cgcpref,'cnpj')); // cnpj
$this->objpdf->SetFont('Arial', 'b', 10);
$this->objpdf->Text($x+125, $y+22, $this->tipo_convenio);               // tipo_convenio
$this->objpdf->SetFont('Arial', '', 10);
$this->objpdf->Text($x+138, $y+23, $this->agencia_cedente);             // agencia do cedente

$sEnderecoFixaComp = $sEnderPref;

if (!empty($sMunicPref)) {
  $sEnderecoFixaComp .= " - ".$sMunicPref;
}

$this->objpdf->SetFont('Arial', '', 6);
$this->objpdf->Text($x+3, $y+29, $sEnderecoFixaComp);                   // endereco
$this->objpdf->SetFont('Arial', '', 10);
...
$this->objpdf->Text($x+100, $y+78,"CPF/CNPJ: ".db_formatar(@$this->cgccpfcomprador,(strlen(@$this->cgccpfcomprador)<12?'cpf':'cnpj'))); // CPF/CNPJ
$this->objpdf->Text($x+3,   $y+80.5,$this->descr11_2);                   // sacado 2
if (!isset($this->ufcgm)) {
  $this->ufcgm = $this->uf_config;
}
$this->objpdf->Text($x+3, $y+83,$this->munic."/".$this->ufcgm." - CEP: ".$this->cep);  // sacado 3