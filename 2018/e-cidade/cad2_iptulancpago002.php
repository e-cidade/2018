      ...
      coalesce((
        ...
        where k00_numpre = j20_numpre
          ...
          and not exists(
            ...
            where arrepaga.k00_numpre = k170_numpre
              and arrepaga.k00_numpar = k170_numpar
              and arrepaga.k00_receit = k170_receit
...
$resultporparcela = db_query($sqlporparcela) or die($sqlporparcela);

$sqlmaxparcarrecad = "select max(k00_numpar) as max_arrecad from ($sqlporparcela) as x";
$resultmaxparcarrecad = db_query($sqlmaxparcarrecad) or die($sqlmaxparcarrecad);
db_fieldsmemory($resultmaxparcarrecad, 0);

$sqlmaxparcdivold = "select max(k10_numpar) as max_divold from ($sqlporparcela) as x";
$resultmaxparcdivold = db_query($sqlmaxparcdivold) or die($sqlmaxparcdivold);
...
$pdf->addHeaderDescription($head1);
$pdf->addHeaderDescription($head2);
$pdf->addHeaderDescription($head3);
$pdf->addHeaderDescription($head4);
...
foreach ($array_parcs as $a => $b) {
  ...
  $resultreceita = db_query($sqlreceita) or die($sqlreceita);
...
$pdf->showPDF("iptu_lancamento_pagamento_" . time());