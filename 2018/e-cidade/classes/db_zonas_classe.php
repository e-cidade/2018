...
class cl_zonas {
   ...
   function incluir ($j50_zona){
     ...
     if($j50_zona == "" || $j50_zona == null ){
       $result = db_query("select nextval('zonas_j50_zona_seq')");
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: zonas_j50_zona_seq do campo: j50_zona";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false;
     ...
     if(($this->j50_zona == null) || ($this->j50_zona == "") ){
       $this->erro_sql = " Campo j50_zona não declarado.";
   ...
   public function alterar ($j50_zona=null) {
      $this->atualizacampos();
     ...
     if (!$result) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Zonas fiscais não Alterado. Alteração Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->j50_zona;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Zonas fiscais não foi Alterado. Alteração Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->j50_zona;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   public function excluir ($j50_zona=null,$dbwhere=null) {
     ...
     if ($result == false) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Zonas fiscais não Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Zonas fiscais não Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$j50_zona;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   // funcao do sql
   public function sql_query ($j50_zona = null,$campos = "*", $ordem = null, $dbwhere = "") {
   ...
   // funcao do sql
   public function sql_query_file ($j50_zona = null, $campos = "*", $ordem = null, $dbwhere = "") {
  ...
  /**
   * Verifica vinculo da zona fiscal
   * @param  integer $j50_zona código da zona
   * @return string
   */
  public function sql_verificaZona( $j50_zona ){

    $sSql = "select sum(count) as quantidade_registros
               from (select count(*) from isszona      where q35_zona  = {$j50_zona}
                       union
                     select count(*) from zonafator    where j110_zona = {$j50_zona}
                       union
                     select count(*) from carzonafator where j96_zona  = {$j50_zona}
                       union
                     select count(*) from aguacorte    where x40_zona  = {$j50_zona}) as quantidade";
    return $sSql;
  }
