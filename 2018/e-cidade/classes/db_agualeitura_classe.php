...
class cl_agualeitura {
   ...
   public $x21_codleitura = 0;
   public $x21_codhidrometro = 0;
   public $x21_exerc = 0;
   public $x21_mes = 0;
   public $x21_situacao = 0;
   public $x21_numcgm = 0;
   public $x21_dtleitura_dia = null;
   public $x21_dtleitura_mes = null;
   public $x21_dtleitura_ano = null;
   public $x21_dtleitura = null;
   public $x21_usuario = 0;
   public $x21_dtinc_dia = null;
   public $x21_dtinc_mes = null;
   public $x21_dtinc_ano = null;
   public $x21_dtinc = null;
   public $x21_leitura = 0;
   public $x21_consumo = 0;
   public $x21_excesso = 0;
   public $x21_virou = 'f';
   public $x21_tipo = 0;
   public $x21_status = 0;
   public $x21_saldo = 0;
   public $x21_aguacontrato = 'null';
   ...
   public $campos = "
                 ...
                 x21_aguacontrato = int4 = Contrato
                 ";
   ...
   function incluir ($x21_codleitura){
     ...
     if($this->x21_numcgm == null ){
       $this->erro_sql = " Campo Leiturista não informado.";
     ...
     if($this->x21_dtinc == null ){
       $this->erro_sql = " Campo Data Inclusao não informado.";
     ...
     if($this->x21_leitura == null ){
       $this->erro_sql = " Campo Leitura não informado.";
     ...
     if($this->x21_virou == null ){
       $this->erro_sql = " Campo Hidrômetro Virou não informado.";
     ...
     if($this->x21_tipo == null ){
       $this->erro_sql = " Campo Tipo de Leitura não informado.";
     ...
     if($this->x21_status == null ){
       $this->erro_sql = " Campo Status da Leitura não informado.";
     ...
     if(($this->x21_codleitura == null) || ($this->x21_codleitura == "") ){
       $this->erro_sql = " Campo x21_codleitura não declarado.";
     ...
     $sql = "insert into agualeitura(
                ...
                values (
                               ...
                               ,$this->x21_status
                               ,$this->x21_saldo
                               ,$this->x21_aguacontrato
                      )";
     $result = db_query($sql);
     if($result==false){
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Leitura ($this->x21_codleitura) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,1439,1009335,'','".AddSlashes(pg_result($resaco,0,'x21_aguacontrato'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   }
   // funcao para alteracao
   public function alterar ($x21_codleitura=null) {
      $this->atualizacampos();
     ...
     if(trim($this->x21_aguacontrato)!="" || isset($GLOBALS["HTTP_POST_VARS"]["x21_aguacontrato"])){
        if(trim($this->x21_aguacontrato)=="" && isset($GLOBALS["HTTP_POST_VARS"]["x21_aguacontrato"])){
           $this->x21_aguacontrato = "0" ;
        }
       $sql  .= $virgula." x21_aguacontrato = $this->x21_aguacontrato ";
       $virgula = ",";
     }
     $sql .= " where ";
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows > 0) {
         ...
         for ($conresaco = 0; $conresaco < $this->numrows; $conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["x21_aguacontrato"]) || $this->x21_aguacontrato != "")
             $resac = db_query("insert into db_acount values($acount,1439,1009335,'".AddSlashes(pg_result($resaco,$conresaco,'x21_aguacontrato'))."','$this->x21_aguacontrato',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
     if (!$result) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Leitura não Alterado. Alteração Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->x21_codleitura;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Leitura não foi Alterado. Alteração Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->x21_codleitura;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   public function excluir ($x21_codleitura=null,$dbwhere=null) {
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,1439,1009335,'','".AddSlashes(pg_result($resaco,$iresaco,'x21_aguacontrato'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     ...
     if ($result == false) {
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Leitura não Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Leitura não Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$x21_codleitura;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   // funcao do sql
   public function sql_query ($x21_codleitura = null,$campos = "*", $ordem = null, $dbwhere = "") {
     ...
     $sql .= "      left  join aguacontrato  on  aguacontrato.x54_sequencial = agualeitura.x21_aguacontrato";
     $sql2 = "";
     if (empty($dbwhere)) {
       if (!empty($x21_codleitura)) {
  ...
  public function sql_query_ultimas_leituras($sCampos = null, $sWhere = null, $sOrdem = null, $sGroupBy = null, $iLimit = null, $iOffset = null) {
    ...
    $aSql[] = "select {$sCampos}";
    $aSql[] = "from agualeitura";
    $aSql[] = "inner join aguacontratoligacao on aguacontratoligacao.x55_aguahidromatric = agualeitura.x21_codhidrometro";
    $aSql[] = "inner join aguasitleitura      on aguasitleitura.x17_codigo = agualeitura.x21_situacao";
    $aSql[] = "left  join aguacontrato        on aguacontrato.x54_sequencial = agualeitura.x21_aguacontrato";
   ...
   // funcao do sql
   public function sql_query_file ($x21_codleitura = null, $campos = "*", $ordem = null, $dbwhere = "") {
   ...
   * @return string
   */
	public function sql_verifica_emissao_coletor($iCodigoLeitura) {
    ...
    $sSqlDadosExportados = "
      ...
      where
        x21_codleitura = {$iCodigoLeitura} and
        x50_contaimpressa = 1
      limit 1
    ";

    return $sSqlDadosExportados;
  }
