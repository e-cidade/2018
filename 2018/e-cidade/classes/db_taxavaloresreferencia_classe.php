...
class cl_taxavaloresreferencia { 
   ...
   function incluir ($y121_sequencial){ 
      $this->atualizacampos();
     ...
     if($y121_sequencial == "" || $y121_sequencial == null ){
       $result = db_query("select nextval('taxavaloresreferencia_y121_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: taxavaloresreferencia_y121_sequencial_seq do campo: y121_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       if(($result != false) && (pg_result($result,0,0) < $y121_sequencial)){
         ...
         $this->y121_sequencial = $y121_sequencial; 
       }
     }
     if(($this->y121_sequencial == null) || ($this->y121_sequencial == "") ){ 
       $this->erro_sql = " Campo y121_sequencial não declarado.";
     ...
     $sql = "insert into taxavaloresreferencia(
                                       y121_sequencial 
                                      ,y121_descricao 
                                      ,y121_valor 
                                      ,y121_data_base 
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       $resaco = $this->sql_record($this->sql_query_file($this->y121_sequencial  ));
   ...
   public function excluir ($y121_sequencial=null,$dbwhere=null) { 
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (empty($dbwhere)) {
         ...
         $resaco = $this->sql_record($this->sql_query_file($y121_sequencial));
       } else { 
     ...
     $sql = " delete from taxavaloresreferencia
                    where ";
     $sql2 = "";
     if (empty($dbwhere)) {
   ...
   public function sql_query ($y121_sequencial = null,$campos = "*", $ordem = null, $dbwhere = "") { 
     ...
     $sql  = "select {$campos}";
     $sql .= "  from taxavaloresreferencia ";
     $sql2 = "";
     if (empty($dbwhere)) {
       if (!empty($y121_sequencial)) {
         $sql2 .= " where taxavaloresreferencia.y121_sequencial = $y121_sequencial "; 
       } 
