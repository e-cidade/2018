<?
//MODULO: material
//CLASSE DA ENTIDADE matmatercontrolefracionamento
class cl_matmatercontrolefracionamento { 
   ...
   function cl_matmatercontrolefracionamento() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("matmatercontrolefracionamento"); 
   ...
   function incluir ($m08_codigo){ 
     ...
     if($m08_codigo == "" || $m08_codigo == null ){
       $result = db_query("select nextval('matmatercontrolefracionamento_m08_codigo_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: matmatercontrolefracionamento_m08_codigo_seq do campo: m08_codigo"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       $result = db_query("select last_value from matmatercontrolefracionamento_m08_codigo_seq");
     ...
     $sql = "insert into matmatercontrolefracionamento(
                                       m08_codigo 
                                      ,m08_matmater 
                                      ,m08_unidade 
                                      ,m08_quantidade 
   ...
   public function alterar ($m08_codigo=null) { 
      $this->atualizacampos();
     $sql = " update matmatercontrolefracionamento set ";
     $virgula = "";
   ...
   public function excluir ($m08_codigo=null,$dbwhere=null) { 
     ...
     $sql = " delete from matmatercontrolefracionamento
                    where ";
     $sql2 = "";
     if (empty($dbwhere)) {
        if (!empty($m08_codigo)){
   ...
   public function sql_record($sql) { 
     ...
     $this->numrows = pg_num_rows($result);
      if ($this->numrows == 0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:matmatercontrolefracionamento";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   public function sql_query ($m08_codigo = null,$campos = "*", $ordem = null, $dbwhere = "") { 
     ...
     $sql  = "select {$campos}";
     $sql .= "  from matmatercontrolefracionamento ";
     $sql .= "      inner join matmater  on  matmater.m60_codmater = matmatercontrolefracionamento.m08_matmater";
     $sql .= "      inner join matunid  on  matunid.m61_codmatunid = matmatercontrolefracionamento.m08_unidade";
     $sql .= "      inner join matunid  as a on   a.m61_codmatunid = matmater.m60_codmatunid";
     $sql2 = "";
     if (empty($dbwhere)) {
       if (!empty($m08_codigo)) {
         $sql2 .= " where matmatercontrolefracionamento.m08_codigo = $m08_codigo "; 
       } 
