...
//		 $sql .= " inner join arreinstit on arreinstit.k00_numpre = arrecad.k00_numpre";
  ...
  function sql_query_arrecad ( $oid = null,$campos="*",$ordem=null,$dbwhere=""){
    $sql = "select ";
    if($campos != "*" ){
      $campos_sql = split("#",$campos);
      $virgula = "";
   ...
   *
   * @param integer $codigoTipo
   * @param boolean $cadtipo
   * @return string
   */
  function sql_query_getReceitasTipo($codigoTipo, $cadtipo = true){

    $sSql = null;

    if (!empty($codigoTipo)){

      $tabela = $cadtipo ? "cadtipo.k03_tipo" : "arrecad.k00_tipo";
  ...
  /**
   * @param array $aCampos
   * @param int $iCodigo - Código de quem está sendo pesquisado, podendo ser um CGM, Matrícula ou Inscrição Municipal
   * @param int $iOrigemCodigo - Informa a origem do código informado:
   *                             1 - CGM
   *                             2 - Matrícula
   ...
   * @param array $aWhere
   * @param int $iCodigo - Código de quem está sendo pesquisado, podendo ser um CGM, Matrícula ou Inscrição Municipal
   * @param int $iTipoContribuinte - Informa o tipo de contribuinte:
   *                                 1 - CGM
   *                                 2 - Matrícula
   *                                 3 - Inscrição Municipal
   * @param array $aOrdenacao
   * @return string
   */
  public function sqlDebitosPorTipoContribuinte(
    $aCampos = array(),
    $aWhere = array(),
    $iCodigo,
    $iTipoContribuinte,
    ...
    if(!in_array($iTipoContribuinte, array(1, 2, 3))) {
      return null;
    }

    $sInnerContribuinte = "";

    $sInnerPadrao  = " inner join arreinstit on arreinstit.k00_numpre = arrecad.k00_numpre         ";
    $sInnerPadrao .= "	                    and arreinstit.k00_instit = ".db_getsession("DB_instit");
    $sInnerPadrao .= " inner join arretipo on arretipo.k00_tipo = arrecad.k00_tipo                 ";
    $sInnerPadrao .= " inner join cadtipo on cadtipo.k03_tipo = arretipo.k03_tipo                  ";
    $sInnerPadrao .= " left join issvar on issvar.q05_numpre = arrecad.k00_numpre                  ";
    $sInnerPadrao .= "                 and issvar.q05_numpar = arrecad.k00_numpar                  ";

    $sWhereValor  = " case                                                                                     ";
    $sWhereValor .= "  when cadtipo.k03_tipo = 3 and (issvar.q05_vlrinf > 0 or issvar.q05_valor > 0) then true ";
    $sWhereValor .= "  when arrecad.k00_valor > 0 then true                                                    ";
    $sWhereValor .= " end                                                                                      ";

    $aWhere[] = $sWhereValor;

    /**
     * CONDIÇÕES QUANDO TRATA-SE DE UM CGM
     */
    if($iTipoContribuinte == 1) {

      $sInnerContribuinte = " inner join arrenumcgm on arrenumcgm.k00_numpre = arrecad.k00_numpre ";
      $aWhere[] = " arrenumcgm.k00_numcgm = {$iCodigo} ";
    }

    /**
     * CONDIÇÕES QUANDO TRATA-SE DE UMA MATRÍCULA
     */
    if($iTipoContribuinte == 2) {

      $sInnerContribuinte = " inner join arrematric on arrematric.k00_numpre = arrecad.k00_numpre ";
      $aWhere[] = " arrematric.k00_matric = {$iCodigo} ";
    }

    /**
     * CONDIÇÕES QUANDO TRATA-SE DE UMA INSCRIÇÃO MUNICIPAL
     */
    if($iTipoContribuinte == 3) {

      $sInnerContribuinte = " inner join arreinscr on arreinscr.k00_numpre = arrecad.k00_numpre ";
      $aWhere[] = " arreinscr.k00_inscr = {$iCodigo} ";
    }

    $sCampos = count($aCampos) > 0 ? implode(", ", $aCampos) : "*";
    $sOrdenacao = count($aOrdenacao) > 0 ? implode(", ", $aOrdenacao) : "arretipo.k00_descr";
    $sWhere = implode(" and ", $aWhere);

    $sSql  = "select {$sCampos}            ";
    $sSql .= "  from arrecad               ";
    $sSql .= "       {$sInnerContribuinte} ";
    $sSql .= "       {$sInnerPadrao}       ";
    $sSql .= " where {$sWhere}             ";
  ...
  /**
   * Retorna os tipos de débitos do exercício
   * @param array $aCampos
   * @param array $aWhere
   * @return string
   */
  public function sqlTiposDebitosGeral($aCampos = array(), $aWhere = array()) {

    $sCampos = count($aCampos) > 0 ? implode(", ", $aCampos) : "*";

    $aWhere[] = "k00_instit = ".db_getsession("DB_instit");
    ...
    $sSql .= "  order by arretipo.k00_descr ";
