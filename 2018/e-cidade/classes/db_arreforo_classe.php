...
class cl_arreforo { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->k00_certidao = ($this->k00_certidao == ""?@$GLOBALS["HTTP_POST_VARS"]["k00_certidao"]:$this->k00_certidao);
     }else{
     }
   }
   ...
   function incluir (){ 
     ...
     if($this->k00_certidao == null ){ 
       $this->erro_sql = " Campo numero da certidao nao Informado.";
       $this->erro_campo = "k00_certidao";
       $this->erro_banco = "";
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
   ...
   function alterar ( $oid=null ) { 
     ...
     if(trim($this->k00_certidao)!="" || isset($GLOBALS["HTTP_POST_VARS"]["k00_certidao"])){ 
       $sql  .= $virgula." k00_certidao = $this->k00_certidao ";
       $virgula = ",";
       if(trim($this->k00_certidao) == null ){ 
         $this->erro_sql = " Campo numero da certidao nao Informado.";
         $this->erro_campo = "k00_certidao";
         $this->erro_banco = "";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
...
$sql .= "oid = '$oid'";     $result = db_query($sql);
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:arreforo";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $oid = null,$campos="arreforo.oid,*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
     ...
     $sql .= " from arreforo ";
     $sql .= "      inner join cgm  on  cgm.z01_numcgm = arreforo.k00_numcgm";
     $sql .= "      inner join histcalc  on  histcalc.k01_codigo = arreforo.k00_hist";
     $sql .= "      inner join tabrec  on  tabrec.k02_codigo = arreforo.k00_receit";
     $sql .= "      inner join arretipo  on  arretipo.k00_tipo = arreforo.k00_tipo";
     $sql .= "      inner join tabrecjm  on  tabrecjm.k02_codjm = tabrec.k02_codjm";
     $sql .= "      inner join cadtipo  on  cadtipo.k03_tipo = arretipo.k03_tipo";
     $sql2 = "";
     if($dbwhere==""){
       if( $oid != "" && $oid != null){
          $sql2 = " where arreforo.oid = '$oid'";
       }
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
