...
//MODULO: ISSQN
//CLASSE DA ENTIDADE meiimportamei
class cl_meiimportamei { 
   ...
   function cl_meiimportamei() { 
     //classes dos rotulos dos campos
     $this->rotulo = new rotulo("meiimportamei"); 
   ...
   function incluir ($q105_sequencial){ 
     ...
     if($q105_sequencial == "" || $q105_sequencial == null ){
       $result = db_query("select nextval('meiimportamei_q105_sequencial_seq')"); 
       if($result==false){
         $this->erro_banco = str_replace("\n","",@pg_last_error());
         $this->erro_sql   = "Verifique o cadastro da sequencia: meiimportamei_q105_sequencial_seq do campo: q105_sequencial"; 
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "0";
         return false; 
       ...
       $result = db_query("select last_value from meiimportamei_q105_sequencial_seq");
   ...
   function sql_record($sql) { 
     ...
     $this->numrows = pg_numrows($result);
      if($this->numrows==0){
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:meiimportamei";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   function sql_query ( $q105_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= " from meiimportamei ";
     $sql .= "      inner join meiimporta  on  meiimporta.q104_sequencial = meiimportamei.q105_meiimporta";
     $sql .= "      inner join db_usuarios  on  db_usuarios.id_usuario = meiimporta.q104_id_usuario";
     $sql2 = "";
     if($dbwhere==""){
       if($q105_sequencial!=null ){
         $sql2 .= " where meiimportamei.q105_sequencial = $q105_sequencial "; 
       } 
     }else if($dbwhere != ""){
       $sql2 = " where $dbwhere";
     }
  ...
  function sql_query_nomemei( $q105_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){ 
     $sql = "select ";
     if($campos != "*" ){
       $campos_sql = split("#",$campos);
       $virgula = "";
     ...
     $sql .= " from meiimportamei ";
     $sql .= "      left join meiimportameireg        on meiimportameireg.q111_meiimportamei     = meiimportamei.q105_sequencial                 ";
     ...
     $sql .= "      left join cgm                     on cgm.z01_cgccpf                          = meiimportamei.q105_cnpj                       ";
     $sql2 = "";
     if($dbwhere==""){
       if($q105_sequencial!=null ){
         $sql2 .= " where meiimportamei.q105_sequencial = $q105_sequencial "; 
