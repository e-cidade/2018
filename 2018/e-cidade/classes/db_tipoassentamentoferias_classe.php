...
class cl_tipoassentamentoferias { 
   ...
   function incluir ($rh168_sequencial){ 
     ...
     $sql = "insert into tipoassentamentoferias(
                                       rh168_sequencial 
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "Tipos de Assentamentos para Férias ($this->rh168_sequencial) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "Tipos de Assentamentos para Férias já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "Tipos de Assentamentos para Férias ($this->rh168_sequencial) não Incluído. Inclusão Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   public function alterar ($rh168_sequencial=null) { 
      $this->atualizacampos();
     $sql = " update tipoassentamentoferias set ";
     $virgula = "";
     ...
     if (!$result) { 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Tipos de Assentamentos para Férias não Alterado. Alteração Abortada.\\n";
         $this->erro_sql .= "Valores : ".$this->rh168_sequencial;
       $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
       $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       $this->erro_status = "0";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Tipos de Assentamentos para Férias não foi Alterado. Alteração Executada.\\n";
         $this->erro_sql .= "Valores : ".$this->rh168_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   public function excluir ($rh168_sequencial=null,$dbwhere=null) { 
     ...
     $sql = " delete from tipoassentamentoferias";
     $sql2 = "";
     if (empty($dbwhere)) {
   ...
   public function sql_record($sql) { 
     ...
     $this->numrows = pg_num_rows($result);
      if ($this->numrows == 0) {
        $this->erro_banco = "";
        $this->erro_sql   = "Record Vazio na Tabela:tipoassentamentoferias";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
        $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
        $this->erro_status = "0";
        return false;
   ...
   public function sql_query ($rh168_sequencial = null,$campos = "*", $ordem = null, $dbwhere = "") { 
     ...
     $sql  = "select {$campos}";
     $sql .= "  from tipoassentamentoferias ";
     $sql .= "      left  join tipoasse  on  tipoasse.h12_codigo = tipoassentamentoferias.rh168_tipoassentamentoferias";
     $sql .= "      inner join naturezatipoassentamento  on  naturezatipoassentamento.rh159_sequencial = tipoasse.h12_natureza";
     $sql2 = "";
     if (empty($dbwhere)) {
       if (!empty($rh168_sequencial)) {
         $sql2 .= " where tipoassentamentoferias.rh168_sequencial = $rh168_sequencial "; 
       } 
  ...
  public function sql_query_tipos ($rh168_sequencial = null,$campos = "*", $ordem = null, $dbwhere = "") {
    ...
    $sql  = "select {$campos}";
    $sql .= "  from tipoassentamentoferias ";
    $sql .= "      left  join tipoasse  as assentamento_ferias on  assentamento_ferias.h12_codigo = tipoassentamentoferias.rh168_tipoassentamentoferias";
    $sql .= "      left  join tipoasse  as assentamento_abono  on  assentamento_abono.h12_codigo  = tipoassentamentoferias.rh168_tipoassentamentoabono";
    $sql2 = "";
    if (empty($dbwhere)) {
      if (!empty($rh168_sequencial)) {
        $sql2 .= " where tipoassentamentoferias.rh168_sequencial = $rh168_sequencial ";
