...
if ( $clturma->numrows > 0 ) {
 ...
 $sql3 = "SELECT round(sum(ed72_i_valornota)/count(ed72_i_valornota),$decimais) as media,
           ...
           WHERE ed72_i_procavaliacao = $periodo
           ...
           GROUP BY ed232_c_abrev,ed232_c_descr,ed09_c_descr,ed47_v_nome,ed59_i_ordenacao, ed232_i_codigo
 ...
 for ( $iContador = 0; $iContador < $linhas3; $iContador++ ) {
   ...
   $oDadosAluno      = db_utils::fieldsMemory( $result3, $iContador );
 ...
 $sql1 = "SELECT round(sum(ed72_i_valornota)/count(ed72_i_valornota),$decimais) as media,
          ...
          GROUP BY ed232_c_abrev,ed232_c_descr,ed09_c_descr,ed59_i_ordenacao, ed232_i_codigo
          ORDER BY ed59_i_ordenacao
         ";
 ...
 if($linhas1==0){?>
  ...
  for ( $x = 0; $x < $linhas3; $x++ ) {
    ...
    $oDados1       = db_utils::fieldsMemory( $result3, $x );
  ...
  for ( $x = 0; $x < $linhas1; $x++ ) {
    ...
    $oDados2      = db_utils::fieldsMemory( $result1, $x );
    $media_turma .= $sep.$oDados2->media;
    $sep          = ",";
    ...
    $sql2 = "SELECT round(sum(ed72_i_valornota)/count(ed72_i_valornota),$decimais) as media,
               ...
               FROM matricula
                    ...
                    left  join caddisciplina   on ed232_i_codigo    = ed12_i_caddisciplina
              WHERE ed72_i_procavaliacao = $periodo
                AND ed57_i_calendario    = $ed57_i_calendario
                AND ed221_i_serie       in ($ed223_i_serie)
                AND ed72_c_amparo        = 'N'
                AND ed59_c_freqglob     != 'F'
                AND ed59_i_turma         = ed60_i_turma
                AND ed60_c_ativa         = 'S'
              ...
              GROUP BY ed232_c_abrev,ed232_c_descr,ed59_i_ordenacao, ed232_i_codigo
              ORDER BY ed59_i_ordenacao";
    ...
    for($y = 0; $y < pg_num_rows($rsSql2); $y++) {
      ...
      $oDados3                                  = db_utils::fieldsMemory($rsSql2,$y);
      $aDisciplinas[$oDados2->ed232_i_codigo][] = $oDados3;
    }
  }
  ...
  $titulo = "Gráfico de Aproveitamento por Período  Aluno: $aluno - {$oDados1->ed47_v_nome}";
  $subtitulo = "Turma: {$ed57_c_descr}  {$oDados1->ed09_c_descr}";
  $largura = $larg_pagina;
  $altura = 400;
  ...
  $texto_linha = array("$aluno - {$oDados1->ed47_v_nome}","Média Turma $ed57_c_descr","Média da Etapa $ed11_c_descr");
  $cores_linha = array ($vermelho,$azul,$verde);
  ...
  for($i=0 ; $i<$numero_colunas; $i++)
  {
      ...
      for($j=$i ; $j<$numero_valores; $j+=$numero_colunas)
      {
          ...
          ImageStringUp($imagem, 2,$posx+($largura_barra/10) ,$inicio_grafico_y-$altura_barra-5,$valores[$j], $vermelho);
  ...
  if($exibir_legenda=="sim")
  {
      ...
      for($i=0 ; $i<$numero_linhas; $i++)
          if(strlen(substr($texto_linha[$i],0,29))>$maior_tamanho)
              $maior_tamanho = strlen(substr($texto_linha[$i],0,29));
      ...
      for($i=0 ; $i<$numero_linhas; $i++)
      {
          ...
          ImageString($imagem, $fonte, $x_pos, $y_pos, substr($texto_linha[$i],0,29), $preto);
      ...
      ImageRectangle($imagem, $x_inicio_legenda, 90+$y_inicio_legenda,$x_fim_legenda, 90+$y_fim_legenda, $preto);

      // começa a desenhar os dados
      for($i=0 ; $i<$numero_colunas; $i++)
      {
          $x_pos = $x_inicio_legenda + $largura_fonte;
          $y_pos = 90+$y_inicio_legenda + $i * ($altura_fonte + $espaco_entre_linhas) + $margem_vertical;
          $legenda = explode("|",$texto_coluna[$i]);
  ...
  echo "<form name='form1'>
        ...
        <script>
         function js_imprimir(){
          jan = window.open('edu2_gfcoaprovaluno003.php?aluno=$aluno - {$oDados1->ed47_v_nome}&turma=$turma&nome_arquivo=$nome_arquivo','','width='+(screen.availWidth-5)+',height='+(screen.availHeight-40)+',scrollbars=1,location=0 ');
          jan.moveTo(0,0);
          parent.location.href='edu2_gfcoaprovaluno001.php?calendario=$ed57_i_calendario&turma=$turma';
         }
        </script>
       ";
  ImageDestroy($imagem);
