 ...
 */
class RegraClassificacaoCredorElementos extends RegraClassificacaoCredor {
  ...
  private function comparaContaOrcamento(ContaOrcamento $oContaComparacao) {
    ...
    foreach ($this->oListaClassificacaoCredor->getContas() as $oConta) {
      ...
      $sEstruturalMascaraContaLista = $oConta->getContaOrcamento()->getEstruturalComMascara();
      $iNivelBusca = $oConta->getContaOrcamento()->getNivelEstrutura($sEstruturalMascaraContaLista);

      $sEstruturalBuscaContaAtributosEmpenho = $oContaComparacao->getEstruturaAteNivel($oContaComparacao->getEstruturalComMascara(), $iNivelBusca);
      $sEstruturalBuscaContaLista            = $oConta->getContaOrcamento()->getEstruturaAteNivel($sEstruturalMascaraContaLista, $iNivelBusca);

      if ($sEstruturalBuscaContaAtributosEmpenho == $sEstruturalBuscaContaLista) {
        return true;
      }
    }
