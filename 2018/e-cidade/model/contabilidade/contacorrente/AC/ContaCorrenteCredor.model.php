 ...
 * @version 1.0
 */
class ContaCorrenteCredor extends ContacorrenteBase implements IContaCorrente {

  /**
   * Registro na tabela contacorrente
   * @var integer
   */
  const CONTA_CORRENTE = 104;
  ...
  public function salvar($dtLancamento = null) {
    ...
    $sWhere .= " and c19_numcgm = {$oCredor->getCodigo()}";
    ...
    if ($oDaoContaCorrenteDetalhe->numrows == 1) {
      ...
      $oDaoContaCorrenteDetalhe->c19_numcgm              = $oCredor->getCodigo();
   ...
   */
  private function getCredor() {

    $oCredor = null;
    if ($this->oContaCorrenteDetalhe->getEmpenho() instanceof EmpenhoFinanceiro) {
      $oCredor = $this->oContaCorrenteDetalhe->getEmpenho()->getCgm();
    } else if ($this->oContaCorrenteDetalhe->getCredor() instanceof CgmBase) {
      $oCredor = $this->oContaCorrenteDetalhe->getCredor();
    } else {
      throw new ParameterException("Credor não informado para o conta corrente: {$this->oContaCorrente->getDescricao()}.");
    }
    return $oCredor;
  }
  ...
  private function validarInformacoes() {
    ...
    if ( ! $this->oContaCorrenteDetalhe instanceof ContacorrenteDetalhe) {
      throw new Exception("Erro técnico: ContaCorrenteDetalhe não definida para a conta {$this->getContaPlano()->getReduzido()}");
    }
    if ( ! $this->oContaCorrenteDetalhe->getEmpenho() instanceof EmpenhoFinanceiro &&
         ! $this->oContaCorrenteDetalhe->getCredor() instanceof CgmBase) {
      throw new ParameterException("Empenho/Credor não informado para inclusão do conta corrente {$this->oContaCorrente->getDescricao()}.");
    }
    return true;
  }
}
