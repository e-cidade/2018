 ...
 */
class RegraInscricaoPassivoSemSuporteOrcamentario implements IRegraLancamentoContabil {

	public function __construct(){
	}

	/**
	 * Este método deve retornar UMA instancia da Conta Contábil
	 * Para isso devemos validar se a inscrição é válida (não estar anulada)
    ...
    if ($iContaPcasp != "") {
      ...
      if (pg_num_rows($rsConta) > 0) {
		...
		for ($i = 0; $i < $oDaoTransacao->numrows; $i++) {
			...
			if ($oDadosTransacao->c47_compara == 1 && $oDadosTransacao->c47_debito == $iContaPcasp) {
