...
class EscrituracaoRestosAPagar {
  ...
  /**
   * EscrituracaoRestosAPagar constructor.
   *
   * @param $iAno
   * @param $iInstituicao
   */
   ...
   * @param $iTipo
   */
  public function setTipoRestoAPagar($iTipo) {
    $this->iTipo = $iTipo;
  }
   ...
   * @internal param int $iTipo
   *
   * @internal param bool $lIncluir
   */
  private function salvar($lProcessar = true) {
    ...
    $sWhere       .= " and c107_tipo   = {$this->iTipo}";

    $oDaoCabecalhoInscricaoRP = new cl_inscricaorestosapagar();
    ...
    if ($oDaoCabecalhoInscricaoRP->numrows == 0 && $lProcessar) {
      ...
      $sTipo = " ";
      if ($this->iTipo == RestosAPagar::TIPO_RP_NAO_PROCESSADO) {
        $sTipo = " Não ";
      }

      $sMsgErro  = "Não é possível desprocessar a inscrição de RPs{$sTipo}Processados pois não foi realizado ";
      $sMsgErro .= "o processamento para o ano {$this->iAno} para a Instituição {$sInstituicao}.";
      throw new BusinessException($sMsgErro);
    }
    ...
    $oDaoCabecalhoInscricaoRP->c107_tipo       = $this->iTipo;
   ...
   * @param LancamentoAuxiliarInscricaoRestosAPagar $oLancamentoAuxiliar
  ...
  public static function getValorLancamento($iAno, $iInstituicao, $iTipo) {
    ...
    $sWhere .= " and c107_processado is true                ";
  ...
  /**
   * Retorna o valor de exercícios anteriores do último lançamento processado
   *
   * @param $iAno
   * @param $iInstituicao
   * @param $iTipo
  ...
  public static function getValorLancamentoExerciciosAnteriores($iAno, $iInstituicao, $iTipo) {
    ...
    $sOrder  = " c108_sequencial desc limit 1                ";
    $sSqlRP  = $oDaoRP->sql_query(null, 'c70_valor, c71_coddoc', $sOrder, $sWhere);
    $rsRP    = db_query($sSqlRP);
    if (!$rsRP) {
      throw new DBException("Ocorreu um erro ao buscar os valores do lançamento para o documento 2007 e 2011.");
    }

    return (pg_num_rows($rsRP) == 0) ? 0 : db_utils::fieldsMemory($rsRP, 0)->c70_valor;
  }
