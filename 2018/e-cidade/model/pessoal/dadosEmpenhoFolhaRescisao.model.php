...
class dadosEmpenhoFolhaRescisao {
  ...
  public function getExcessoesEmpenhoFolha($sRubric='',$iAnoUsu='',$iInstit=''){
    ...
    $sWhereExcecao .= " and rh74_tipofolha = 3           "; // Retorna somente a regra para o tipo de folha de recisao
  ...
  */ 
  ...
  public function excluiDadosFolha($sSigla='',$iAnoUsu='',$iMesUsu='',$iInstit='',$aListaRescisoes= null){
    ...
    try {
	    ...
	    $this->excluiDadosPlanilhaFolha($sSigla,$iAnoUsu,$iMesUsu,$iInstit,$aListaRescisoes);
  ...
  public function excluiDadosPlanilhaFolha($sSigla='',$iAnoUsu='',$iMesUsu='',$iInstit='',$aListaRescisoes=null){
    ...
    $sWhereRubricas .= " and ( rh72_siglaarq = '{$sSigla}' or rh79_siglaarq = '{$sSigla}' ) "; 
	  ...
	  $sWhereRubricas .= " and rh73_seqpes in ({$sListaRescisoes})";
  ...
  public function getEstrututal($iAnoUsu,$iLotacao,$sVinculo,$iElemento){
    ...
    if ( $oDaorhLotaExe->numrows == 0 ) {
      throw new BusinessException("{$oDaorhLotaExe->erro_msg}\nverifique órgão e unidade da lotação {$iLotacao}");
    } else {
   ...
   * @return boolean
   */
  public function isLiberada($sSigla='',$sTipoEmp='',$iAnoUsu='',$iMesUsu='',$aListaRescisoes = null, $sListaTabPrev = "") {
   ...
   */
  public function geraPlanilhaGeral($sSigla='', $iAnoUsu='', $iMesUsu='', $iInstit='', $iCgm='', $aListaRescisoes=null) {
    ...
    /**
     * Verificamos se o parâmetro r11_geraretencaoempenho é true ou false
    ...
    if ( $lGeraRetencao == 't') {    
	  ...
	  //será gerada planilha para todos os registros de retenções dos empenhos da folha
	     	...
	     	try {
	     	  $iPlanilhaEmpenho = $this->geraPlanilhaEmpenhoFolha($sSigla, $iAnoUsu, $iMesUsu, $aListaRescisoes, db_getsession("DB_instit"), $iCgm );
	     	} catch ( DBException $eException ){
          throw new DBException("{$sMsgErro}\n{$eException->getMessage()}");
	      } catch ( BusinessException $eException ){
          throw new BusinessException("{$sMsgErro}\n{$eException->getMessage()}");
  ...
  public function geraPlanilhaEmpenhoFolha( $sSigla, $iAnoUsu, $iMesUsu, $aListaRescisoes, $iInstit, $iCgm = '') {
  ...
  public function getRescisoesNaoEmpenhadas($iMesUsu, $iAnoUsu, $sDataInicial, $sDataFinal) {
    ...
    if (pg_num_rows($rsRescisoes) > 0) {
      $aRescisoes = db_utils::getCollectionByRecord($rsRescisoes, false, false, true);
    }
    return $aRescisoes;
  }
  ...
  * @return unknown
  */ 
 public function getRescisoesEmpenhadas($iMesUsu, $iAnoUsu, $sDataInicial, $sDataFinal) {
    require_once('std/DBDate.php');
 ...
 public function getRescisoesPlanilhas($iMesUsu, $iAnoUsu) {
    ...
    if ( $lGeraRetencao == 't' ) {    
      ...
      $sSqlrescisoes .= "                                   and rh79_mesusu                = {$iMesUsu}       ";
      $sSqlrescisoes .= "                                   and rh79_anousu                = {$iAnoUsu}       ";
      $sSqlrescisoes .= "                                   and rh79_siglaarq              = 'r20'            ";
      $sSqlrescisoes .= " left  join rhslipfolhaslip         on rh82_rhslipfolha           = rh79_sequencial  ";
      ...
      $sSqlrescisoes .= " inner join rhempenhofolharhemprubrica on rh81_rhempenhofolharubrica  = rh73_sequencial     ";
      $sSqlrescisoes .= " inner join rhempenhofolha             on rh72_sequencial             = rh81_rhempenhofolha ";
      $sSqlrescisoes .= "                                      and rh72_anousu                 = {$iAnoUsu}          ";
      $sSqlrescisoes .= "                                      and rh72_mesusu                 = {$iMesUsu}          ";
      $sSqlrescisoes .= "                                      and rh72_siglaarq               = 'r20'               ";

      /**
       * Retenção e pagamento-extra
       */
      $sSqlrescisoes .= " and rh73_tiporubrica in (2,3) ";
    }
