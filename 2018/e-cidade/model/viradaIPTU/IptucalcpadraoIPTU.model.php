...
class IptucalcpadraoIPTU implements iViradaIPTU {
  ...
  function __construct() {
    ...
    $sSqlIptuTabelasConfig  = $oDaoIptuTabelasConfig->sql_query(null, "iptutabelasconfig.j122_sequencial", 
                                                                null, "db_sysarquivo.nomearq = 'iptucalcpadrao'");
    ...
    if ($oDaoIptuTabelasConfig->numrows > 0) {
      ...
      if ($oDaoIptuTabelasConfigVirada->numrows > 0) {
        ...
        $sMensagem = "ERRO: Tabela iptucalcpadrao já foi feito virada anual para exercicío {$this->getAnoNovo()}!";
    ...
    $sWhere                           = "db_sysarquivo.nomearq = 'iptucalcpadrao'";
  ...
  public function vira() {
    ...
    $oDaoIptuCalcPadrao        = db_utils::getDao('iptucalcpadrao');
    $oDaoIptuCalcPadraoConstr  = db_utils::getDao('iptucalcpadraoconstr');
    $oDaoIptuCalcPadraoLog     = db_utils::getDao('iptucalcpadraolog');
    
    $nPercentual         = $this->getPercentual();
    
    $oDaoIptuCalcPadraoConstr->excluir(null, "j11_iptucalcpadrao in (select j10_sequencial from iptucalcpadrao where j10_anousu = {$this->getAnoNovo()})");
    if ($oDaoIptuCalcPadraoConstr->erro_status == 0) {
      throw new Exception($oDaoIptuCalcPadraoConstr->erro_msg);
    }
    
    $oDaoIptuCalcPadraoLog->excluir(null, "j19_iptucalcpadrao in (select j10_sequencial from iptucalcpadrao where j10_anousu = {$this->getAnoNovo()})");
    if ($oDaoIptuCalcPadraoLog->erro_status == 0) {
      throw new Exception($oDaoIptuCalcPadraoLog->erro_msg);
    }
   
    $oDaoIptuCalcPadrao->excluir(null, "j10_anousu = {$this->getAnoNovo()}");
    if ($oDaoIptuCalcPadrao->erro_status == 0) {
      throw new Exception($oDaoIptuCalcPadrao->erro_msg);
    }
    
    $sWhere       = " j10_anousu = {$this->getAnoAtual()}";
    $sSqlIptuCalcPadrao  = $oDaoIptuCalcPadrao->sql_query_file(null, "*", null, $sWhere);
    $rsSqlIptuCalcPadrao = $oDaoIptuCalcPadrao->sql_record($sSqlIptuCalcPadrao);
    $iNumRows            = $oDaoIptuCalcPadrao->numrows;
    if ($iNumRows > 0) {

    	for ( $iInd=0; $iInd < $iNumRows; $iInd++ ) {
    		
    		$oDadosIptuCalcPadrao  = db_utils::fieldsMemory($rsSqlIptuCalcPadrao, $iInd);
        $aCamposIptuCalcPadrao = get_object_vars($oDadosIptuCalcPadrao);
        
        foreach ($aCamposIptuCalcPadrao as $sNomeCampoIptuCalcPadrao => $sValorCampoIptuCalcPadrao ) {
        	
          if (in_array($sNomeCampoIptuCalcPadrao, $this->getCampoCorrecao())) {
            $nSomaPercentual = ($oDadosIptuCalcPadrao->$sNomeCampoIptuCalcPadrao + ($oDadosIptuCalcPadrao->$sNomeCampoIptuCalcPadrao * ($nPercentual / 100)));
            $oDaoIptuCalcPadrao->$sNomeCampoIptuCalcPadrao = "{$nSomaPercentual}";
        ...
        $oDaoIptuCalcPadrao->incluir(null);
        if ($oDaoIptuCalcPadrao->erro_status == 0) {
          throw new Exception($oDaoIptuCalcPadrao->erro_msg);
        }
        
        $sWhereConstr  = " j11_iptucalcpadrao = {$oDadosIptuCalcPadrao->j10_sequencial} "; 
        $sWhereConstr .= " and j39_dtdemo is null";
        $sSqlIptuCalcPadraoConstr  = $oDaoIptuCalcPadraoConstr->sql_query(null, "*", null, $sWhereConstr);
        $rsSqlIptuCalcPadraoConstr = $oDaoIptuCalcPadraoConstr->sql_record($sSqlIptuCalcPadraoConstr);
        $iNumRowsConstr = $oDaoIptuCalcPadraoConstr->numrows;
        if ($iNumRowsConstr > 0) {
          
          for ($iConstr = 0; $iConstr < $iNumRowsConstr; $iConstr++) {
            
            $oDadosIptuCalcPadraoConstr = db_utils::fieldsMemory($rsSqlIptuCalcPadraoConstr, $iConstr);
            $oDaoIptuCalcPadraoConstr->j11_iptucalcpadrao = $oDaoIptuCalcPadrao->j10_sequencial;
            $oDaoIptuCalcPadraoConstr->j11_matric         = $oDadosIptuCalcPadraoConstr->j11_matric;
            $oDaoIptuCalcPadraoConstr->j11_idcons         = $oDadosIptuCalcPadraoConstr->j11_idcons;
            $nVlrConstrucao = ($oDadosIptuCalcPadraoConstr->j11_vlrcons + ($oDadosIptuCalcPadraoConstr->j11_vlrcons * ($nPercentual / 100)));
            $oDaoIptuCalcPadraoConstr->j11_vlrcons        = "{$nVlrConstrucao}";
            $oDaoIptuCalcPadraoConstr->incluir(null);
            if ($oDaoIptuCalcPadraoConstr->erro_status == "0") {
              throw new Exception($oDaoIptuCalcPadraoConstr->erro_msg);
            }
