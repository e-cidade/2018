...
class M9647Parcelamento extends PostgresMigration
{
    public function up()
    {
        $sSql = "
insert into arretipo (
    k00_codbco, k00_codage, k00_tipo, k00_descr, k00_emrec, k00_agnum, k00_agpar, k00_msguni, k00_msguni2,
    k00_msgparc, k00_msgparc2, k00_msgparcvenc, k00_msgparcvenc2, k00_msgrecibo, k00_tercdigcarneunica, k00_tercdigcarnenormal,
    k00_tercdigrecunica, k00_tercdigrecnormal, k00_txban, k00_rectx, codmodelo, k00_impval, k00_vlrmin, k03_tipo,
    k00_marcado, k00_hist1, k00_hist2, k00_hist3, k00_hist4, k00_hist5, k00_hist6, k00_hist7, k00_hist8, k00_tipoagrup,
    k00_recibodbpref, k00_instit, k00_formemissao, k00_receitacredito, k00_exercicioscarne
) values (
    1, 99999, nextval('arretipo_k00_tipo_seq'), 'PACELAMENTO COB ADM - FAZENDA', true, true, false, ' ', ' ', 
    ' ', ' ', ' ', ' ', ' ', 6, 6,
    6, 6, 0, 0, 0, true, 0, 7, 
    't', ' ', ' ', '.', '.', '.', '.', '.', '.', 1, 
    1, 1, 2, 995, 1
), (
    1, 99999, nextval('arretipo_k00_tipo_seq'), 'COBRANÇA ADM - FAZENDA', true, true, false, ' ', ' ', 
    ' ', ' ', ' ', ' ', ' ', 6, 6, 
    6, 6, 0, 0, 0, true, 0, 16, 
    't', ' ', ' ', '.', '.', '.', '.', '.', '.', 1, 
    1, 1, 2, 995, 1
        ...
        $this->execute("alter table termotipoconfig add column k42_tipoorigem integer default null");
        $this->execute("drop index termotipoconfig_cadtipo_tiponovo_in");

        $this->execute("insert into termotipoconfig (k42_sequencial, k42_cadtipo, k42_tiponovo, k42_instit, k42_tipoorigem) values ((select max(k42_sequencial) + 1 from termotipoconfig), 7, currval('arretipo_k00_tipo_seq'), 1, currval('arretipo_k00_tipo_seq') - 1)");
...
declare
  ...
  begin
    ...
    -- seleciona o tipo de debito de origem que esta recebendo o parcelamento
    select distinct arrecad.k00_tipo 
      into v_tipoorigem
      from arrecad 
     where arrecad.k00_numpre in (select NUMPRES_PARC.k00_numpre 
                                    from NUMPRES_PARC)
     limit 1;

    -- Seleciona arretipo.k00_tipo a ser incluido no debito de parcelamento.
    select k42_tiponovo
      into v_tiponovo
      from termotipoconfig
     where k42_sequencial = case
                              -- Verifica a existencia de k42_tiponovo(arretipo.k00_tipo).
                              -- Expecifico para o cadtipo.k09_tipo e arretipo.k00_tipo do debito.
                              when (select true
                                      from termotipoconfig
                                     where k42_cadtipo = v_tipo       
                                       and k42_tipoorigem = v_tipoorigem
                                       and k42_instit = iInstit)
                              -- Seleciona k42_tiponovo expecifico para cadtipo.k09_tipo e arretipo.k00_tipo de origem do debito.
                              -- Tem expecificacao de arretipo destipo conforme cadtipo e arretipo origem.
                              then (select k42_sequencial
                                      from termotipoconfig
                                     where k42_cadtipo = v_tipo       
                                       and k42_tipoorigem = v_tipoorigem
                                       and k42_instit = iInstit)
                              -- Seleciona k42_tiponovo expecifico para cadtipo.k09_tipo de origem do debito.
                              -- Nao tem expecificacao de arretipo destino para arretipo origem.
                              else (select k42_sequencial
                                      from termotipoconfig
                                     where k42_cadtipo = v_tipo       
                                       and k42_tipoorigem is null
                                       and k42_instit = iInstit)
                            end;
    ...
    loop
      ...
      select case
       ...
       if v_recdestino is not null or v_recdestino <> 0 then
         ...
         execute ' update w_testando set receita = ' || v_recdestino || ' where ' ||
                 ' receitaori = ' || v_record_recpar.receitaori || ';';
       end if;
      ...
      if v_record_recpar.parcela = v_totalparcelas or v_record_recpar.parcela = ultimaparcelareceita then
        ...
        if v_juronaultima is true then
          ...
          -- se total ate agora for menor ao total do parcelamento * valor proporcional dessa receita em relacao ao total do parcelamento
            ...
            -- total que deve ser gerado para a receita de destino
            select sum(valor)
              into nValorMaximoReceita
              from w_testando
             where receita = v_record_recpar.receit;

            -- valor da ultima parcela da receita é composta por ((x) - (y))
            -- ((x) - (y))
            -- (total que deve ser gerado para a receita) - (total que já foi gerado para a receita)
            -- (sum de valor em w_base_parcelas da receita) - (sum de valor em totrec da receita)
            v_valparc = nValorMaximoReceita - v_totateagora;
...
SQL;
    ...
    {
        $this->execute("delete from termotipoconfig where k42_tipoorigem is not null");

        $this->execute("alter table termotipoconfig drop column k42_tipoorigem");
        $this->execute("create unique index termotipoconfig_cadtipo_tiponovo_in on termotipoconfig (k42_cadtipo, k42_tiponovo)");
