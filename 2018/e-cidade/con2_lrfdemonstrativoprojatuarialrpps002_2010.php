...
try {
  ...
  if (!isset($oGet->instituicao) || empty($oGet->instituicao)) {
    throw new ParameterException('Campo Instituições é de preenchimento obrigatório.');
  }

  $aInstituicoes = explode(',', $oGet->instituicao);
  ...
  $oRelatorio = AnexoRREOFactory::getAnexoRREO(AnexoRREOFactory::ANEXO_X, $iAno, $oPeriodo);
  $oRelatorio->setInstituicoes($oGet->instituicao);
  $oRelatorio->emitir();
