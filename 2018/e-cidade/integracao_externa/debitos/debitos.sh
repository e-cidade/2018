...
function _debitos() {
  for iInstit in $DEBITOS_INSTITUICOES; do
    ...
    echo "[INFO] [ ] Limpando debitos da instituicao ${iInstit}, mantendo os ultimos ${DEBITOS_DIAS_MANTER:-14} dias"
    /usr/bin/php -q cleanup_debitos.php ${iInstit} ${DEBITOS_DIAS_MANTER:-14} > log/cleanup_debitos.log 2>&1
  done
}
