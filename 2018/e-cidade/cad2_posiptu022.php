...
include(modification("classes/db_debitos_classe.php"));
...
if ($setores!=""){
   $txt_where.=" and j34_setor in ('" . implode("', '", explode(',', $setores)) . "') ";
}

if ($quadras!=""){
   $txt_where.=" and j34_quadra in ('" . implode("', '", explode(',', $quadras)) . "') ";
}
...
for($x = 0; $x < $numrows;$x++){
  ...
  ";
  $result_pago = $clarrepaga->sql_record($clarrepaga->sql_query_file('','sum(k00_valor) as pago','',$sWhere));
