...
for ($s = 0; $s < $linhas; $s ++) {
  ...
  $fpdf->addpage('P');
  $fpdf->ln(5);
  $dados = db_query($conn,"select nomeinst,trim(ender)||','||trim(numero) as ender,munic,uf,telef,email,url,logo from 
                          db_config where codigo = ".db_getsession("DB_instit"));
  ...
  $fpdf->cell(80, 4, "Disciplinas", "LRT", 0, "R", 0);
  ...
  $fpdf->cell(8, $altura, "RF", 1, 1, "C", 0);
  $fpdf->setY($inicio + $alturainicio);
  $fpdf->cell(5, 4, "", "LRB", 0, "C", 0);
  if ($ed57_c_medfreq == "PERÌODOS") {
    $fpdf->cell(80, 4, $descr_num, "LRB", 0, "R", 0);
  } else {
   	$fpdf->cell(80, 4, "", "LRB", 0, "R", 0);
  }
  $cont1 = 0;
  for ($y = 0; $y < $clregencia->numrows; $y ++) {
  ...
  $fpdf->cell(80, 4, "Nome do Aluno", 1, 0, "C", 0);
  $cont2 = 0;
  for ($y = 0; $y < $clregencia->numrows; $y ++) {
  ...
  for ($z = 0; $z < $clmatricula->numrows; $z ++) {
    ...
    if ($datasaida == "" or ($datasaida != "" and $comp_datasaida > $comp_datalimitemov)) {
      ...
      $fpdf->setfont('arial', 'b', 6);
      $fpdf->cell(80, 4, $ed47_v_nome, "LR", 0, "L", $cor);
    ...
    if ($datasaida == "" or ($datasaida != "" and $comp_datasaida > $comp_datalimitemov)) {
      ...
      $fpdf->cell ( 8, 4, $rf, "LR", 1, "C", $cor );     
      
    }
    
    if ($cont4 == $limite && ($cont_geral + 1) < $clmatricula->numrows) {
    	
      $fpdf->cell(103, 4, "Observações", 1, 0, "C", 0);
      $fpdf->cell(100, 4, "Convenções", 1, 1, "C", 0);
      ...
      $fpdf->setY($alt_conv);
      $fpdf->setX(110);
      $fpdf->rect(110, $alt_conv, 90, 24);
      for ($y = 0; $y < $clregencia->numrows; $y ++) {
      	
        db_fieldsmemory($result2, $y);
        $cont5 ++;
        $fpdf->setfont('arial', 'b', 6);
        $fpdf->cell(80, 4, $ed232_c_abrev . " - " . $ed232_c_descr, $borda, $quebra, "L", 0);
      ...
      if ($quebra == "1") {
        ...
        $fpdf->cell(100, 4, "", "R", 1, "L", 0);
        $cont5 ++;
        
      }
      
      for ($y = ($cont5 / 2); $y < 12; $y ++) {
        $fpdf->cell(100, 4, "", "L", 1, "L", 0);
      }        
      ...
      $sCampos    .= "ed59_i_turma = $ed57_i_codigo AND ed59_i_serie = $ed223_i_serie";
      ...
      $fpdf->setY($alt_conv);
      $fpdf->rect(10, $alt_conv, 100, 48);
      $fpdf->setfont('arial', 'b', 6);
      $fpdf->multicell(100, 4,($sObs != "" ? $sObs . "\n" : "") , "L", "J", 0, 0);  
      $fpdf->setY($alt_conv+25 );
      $fpdf->multicell(90, 4, ($ed57_t_obs != "" ? $ed57_t_obs . "\n" : "") . 
                              ($transf_pagina != "" ? $transf_pagina . "\n" : "") . 
                              ($obs_cons != "" ? $obs_cons . "\n" : ""), "L", "J", 0, 0);
      $fpdf->setY($alt_conv + 23);
      $fpdf->setX(110);
      $fpdf->cell(90, 4, "E, para constar, foi lavrada esta ata.", "LTR", 2, "C", 0 );
      $fpdf->cell(90, 2, "$ed261_c_nome, " . date("d", db_getsession("DB_datausu")) . " de " . 
                                             db_mes(date("m", db_getsession("DB_datausu"))) . " de " . 
                                             date("Y", db_getsession("DB_datausu")), "LR", 2, "C", 0);
      $fpdf->cell(90, 4, "", "LR", 1, "L", 0);
      $fpdf->cell(45, 4, "_______________________", "L", 0, "C", 0);
      $fpdf->cell(45, 4, "_______________________", "R", 1, "C", 0);
      $fpdf->setX(110);
      $fpdf->cell(45, 4, "SECRETÁRIO(A)", "L", 0, "C", 0);      
      $fpdf->cell(45, 4, "DIRETOR(A)", "R", 1, "C", 0);
      $fpdf->setX(110);
      $fpdf->cell(45, 4, $nomesecretario, 0, 0, "C", 0);
      $fpdf->cell(45, 4, $nomediretor, "R", 1, "C", 0);
      $fpdf->setX(110);
      $fpdf->cell(45, 4, $funcaosecretario, 0, 0, "C", 0);
      $fpdf->cell(45, 4, $funcaodiretor, "R", 1, "C", 0);
      $fpdf->cell(190, 1, "", 1, 1, "C", 0);      
      $fpdf->addpage('P');
      $fpdf->ln(5);      
      $fpdf->setfont('arial', 'b', 7);
      $inicio = $fpdf->getY();
  ...
  for ($z = $cont4; $z < $limite + 1; $z ++) {
    ...
    $fpdf->cell(80, 4, "", "LR", 0, "L", 0);
    
    for ($t = 0; $t < 10; $t ++) {
      $fpdf->cell(11, 4, "", "LR", 0, "C", 0);
    }
    $fpdf->cell(8, 4, "", "LR", 1, "C", 0);
  ...
  if ($claprovconselho->numrows > 0) {
    ...
    for ($g = 0; $g < $claprovconselho->numrows; $g ++) {
      ...
      db_fieldsmemory($result_cons, $g);
      $obs_cons .= $sepobs . "-Aluno(a) $ed47_v_nome na Disciplina $disc_conselho foi aprovado pelo Conselho de ";
      $obs_cons .= " Classe. Justificativa: $ed253_t_obs";
      $sepobs    = "\n";
      
    }
  }
  $fpdf->setY($alt_conv);
  $fpdf->rect(5, $alt_conv, 103, 48);
  $fpdf->setfont('arial', 'b', 6);
  $fpdf->multicell(100, 4,($sObs != "" ? $sObs . "\n" : "") , "L", "J", 0, 0);  
  $fpdf->setY($alt_conv+25 );
  $fpdf->multicell(100, 4,($ed57_t_obs != "" ? $ed57_t_obs . "\n" : "") . 
                          ($transf_pagina != "" ? $transf_pagina . "\n" : "") . 
                          ($obs_cons != "" ? $obs_cons . "\n" : ""), "L", "J", 0, 0);
  $fpdf->setY($alt_conv + 22);
  $fpdf->setX(108);
  $fpdf->cell(100, 4, "E, para constar, foi lavrada esta ata.", "LTR", 2, "C", 0);
  $fpdf->cell(100, 2, "$ed261_c_nome, " . date("d", db_getsession("DB_datausu")) . " de " . 
                                         db_mes(date("m", db_getsession("DB_datausu"))) . " de " . 
                                         date("Y", db_getsession("DB_datausu")), "LR", 2, "C", 0);
  $fpdf->cell(100, 4, "", "LR", 2, "L", 0);
  $fpdf->cell(50, 4, "_______________________", "L", 0, "C", 0);
  $fpdf->cell(50, 4, "_______________________", "R", 1, "C", 0);
  $fpdf->setX(112);
  $fpdf->cell(48, 4, "SECRETÁRIO(A)", 0, 0, "C", 0);      
  $fpdf->cell(48, 4, "DIRETOR(A)", "R", 1, "C", 0);
  $fpdf->setX(112);
  $fpdf->cell(48, 4, $nomesecretario, 0, 0, "C", 0);
  $fpdf->cell(48, 4, $nomediretor, "R", 1, "C", 0);
  $fpdf->setX(112);
  $fpdf->cell(48, 4, $funcaosecretario, 0, 0, "C", 0);
  $fpdf->cell(48, 4, $funcaodiretor, "R", 1, "C", 0); 
  $fpdf->line ( 5, $fpdf->getY(), 208, $fpdf->getY() );
