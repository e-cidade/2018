...
class bal_desp  {
  ...
  function processa($instit=1,$data_ini="",$data_fim="",$orgaotrib=null,$subelemento="") {
    ...
    $result_teste = pg_query($sql_teste) or die($sql_teste);
    ...
    pg_exec("begin");
    pg_exec("create temp table t as select * from orcdotacao where o58_anousu = ".db_getsession("DB_anousu"));
...
//    db_criatabela($result);exit;
    ...
    for($i=0;$i<pg_numrows($result);$i++){
      ...
      if($o58_codigo > 0){
        ...
        $sql_desdobramento = "
          select sum(case when c71_coddoc in (7,52,53,54,55) then c70_valor else 0 end ) as sup ,
                 ...
                 sum(case when c71_coddoc in (62,63) then c70_valor else 0 end ) as esp
                   ...
                   if ($subelemento=="sim"){
                     $resultsup = pg_query($sql_desdobramento);	    
                   } else {
                     $resultsup = pg_query($sql);	   
                   }  
        ...
        $totalcre += $cre;
        $totalesp += $esp;
        $line .= formatar($sup,13,'v'); // creditos suple
        $line .= formatar($cre,13,'v'); // creditos especial
        $line .= formatar($esp,13,'v'); // creditos extraordinarios

        $line .= formatar(abs($reduzido_acumulado),13,'v'); // reducoes
        $line .= formatar(0,13,'v'); // suple recurso vinculado
        $line .= formatar(0,13,'v'); // reducao recurso vinculado
        $line .= formatar(abs($empenhado-$anulado),13,'v');
        $line .= formatar(abs($liquidado),13,'v'); // liquidado
        $line .= formatar(abs($pago),13,'v'); // pago
