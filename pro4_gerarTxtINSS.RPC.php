...
try {
  ...
  switch ($oParam->exec) {
    ...
    case "gerarTXT" :
      ...
      $oHeader->departamento_secretaria = validaDados($iDepartamento,       $sErroLinha1, "Departamento da secretária inválido", 55);  
      ...
      $oHeader->codigo_municipio        = validaDados($iCodInpasInstit,     $sErroLinha1, "Código do município inválido", 5);    
      ...
      foreach ($aDadosSisobra as $iIndiceSisobra => $oObra ) {
        ...
        if (!empty($oObra->areaexistente)) {        	
          $iAreaExistente  = validaDados(str_replace($aSubstituir , "",db_formatar($oObra->areaexistente, 'f')),  $iCodigoObra, "Área da obra inválida", 8, false, "L");        	
        } else {        	
        ...
        $oLinha2->responsavel_endereco_correspondencia = validaDados($oObra->enderecoresponsavel,    $iCodigoObra, "Endereço Responsável inválido",         55, false);
				$oLinha2->responsavel_bairro_correspondencia   = validaDados($oObra->bairroresponsavel,      $iCodigoObra, "Bairro Responsável inválido",           20, false);
				$oLinha2->responsavel_cep_correspondencia      = validaDados($oObra->cpfresponsavel,         $iCodigoObra, "CEP Responsável inválido",               8, false);
	      $oLinha2->responsavel_codigo_sinpas            = validaDados($iCodigoSinpasResponsavel,      $iCodigoObra, "Código SINPAS do município para Correspondencia do responsável não Encontrado", 6, true, "LS");
        ...
        $oLinha2->obra_numero_alvara                   = validaDados($oObra->alvaraobra,             $iCodigoObra, "Número alvará inválido",                12,  true, "L"); 
        ...
        $oLinha2->obra_cep                             = validaDados($sCepObra,                      $iCodigoObra, "CEP da obra inválido. Confira cadastro de Logradouro.", 8);
        $oLinha2->obra_uf                              = validaDados($aHeader->uf,                   $iCodigoObra, "UF da obra inválido",                    2);
        $oLinha2->obra_codigo_sinpas                   = validaDados($iCodInpasInstit,               $iCodigoObra, "Código SINPAS do município da obra inválido", 6, true, "LS");
