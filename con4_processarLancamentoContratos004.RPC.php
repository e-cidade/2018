...
switch ($oParam->exec) {
    ...
    try {
      ...
      foreach ($aEmpenhosEncontrados as $oStdCodigoEmpenho) {
        ...
        for ($iRowAcordo = 0; $iRowAcordo < $iTotalAcordosEmpenho; $iRowAcordo++) {
          ...
          $oLancamentoAuxiliarAcordo->setAcordo(new Acordo($oStdDados->ac16_sequencial));
          $oLancamentoAuxiliarAcordo->setEmpenho(new EmpenhoFinanceiro($oStdDados->e60_numemp));
  ...
  case "desprocessarLancamentosContabeisContratos":
    ...
    try {
      ...
      $sMsgVinculo = "";
      if (isset($oParam->lDesvinculaEmpenho) && $oParam->lDesvinculaEmpenho == "t") {

        $oDaoEmpEmpenhoContrato = db_utils::getDao('empempenhocontrato');
        $sWhereExcluirVinculo = "e100_numemp in ({$oParam->sEmpenhos})";
        $oDaoEmpEmpenhoContrato->excluir(null, $sWhereExcluirVinculo);
        if ($oDaoEmpEmpenhoContrato->erro_status == "0") {
          throw new BusinessException("Não foi possível excluir o vínculo entre contrato e empenho.");
        }
        $sMsgVinculo = "Empenho(s) desvinculado(s) com sucesso.\n\n";
        $iTotalRegistrosAlterados += $oDaoEmpEmpenhoContrato->numrows_excluir;
      }

      $sMsgRetorno       = "Processamento concluído com sucesso.\n\n";
      $sMsgRetorno      .= $sMsgVinculo;
      $sMsgRetorno      .= "Total de registros alterados: {$iTotalRegistrosAlterados}";
      $oRetorno->message = urlencode($sMsgRetorno);
      db_fim_transacao(false);
