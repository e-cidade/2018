...
try {
  ...
  switch ($oParam->exec) {
    ...
    case 'getBensFiltroManutencao':
      ...
      if ( isset($oParam->iOrgao) &&  !empty($oParam->iOrgao)){
      ...
      if ( isset($oParam->iDepartamento) &&  !empty($oParam->iDepartamento)){
        $aWhere[] = "t52_depart= {$oParam->iDepartamento}";
      }

      if ( isset($oParam->iClassificacaoInicial) &&  !empty($oParam->iClassificacaoInicial)){
        $aWhere[] = "t64_class >= '{$oParam->iClassificacaoInicial}'";
      }

      if ( isset($oParam->iClassificacaoFinal) &&  !empty($oParam->iClassificacaoFinal)){
        $aWhere[] = "t64_class <= '{$oParam->iClassificacaoFinal}'";
      }

      if ( isset($oParam->iBemInicial) &&  !empty($oParam->iBemInicial)){
        $aWhere[] = "t52_bem >= '{$oParam->iBemInicial}'";
      }

      if ( isset($oParam->iBemFinal) &&  !empty($oParam->iBemFinal)){
        $aWhere[] = "t52_bem <= '{$oParam->iBemFinal}'";
      }

      if ( isset($oParam->iPlacaInicial) &&  !empty($oParam->iPlacaInicial)){
        $aWhere[] = "t52_ident >= '{$oParam->iPlacaInicial}'";
      }

      if ( isset($oParam->iPlacaFinal) &&  !empty($oParam->iPlacaFinal)){
        $aWhere[] = "t52_ident <= '{$oParam->iPlacaFinal}'";
      }

      if ( isset($oParam->iConvenio) &&  !empty($oParam->iConvenio)){
        $aWhere[] = "t04_sequencial = '{$oParam->iConvenio}'";
      }

      if ( isset($oParam->nValorAquisicaoInicial) &&  !empty($oParam->nValorAquisicaoInicial)){
        $aWhere[] = "t52_valaqu >= '{$oParam->nValorAquisicaoInicial}'";
      }

      if ( isset($oParam->nValorAquisicaoFinal) &&  !empty($oParam->nValorAquisicaoFinal)){
        $aWhere[] = "t52_valaqu <= '{$oParam->nValorAquisicaoFinal}'";
      }

      if ( isset($oParam->dtAquisicaoInicial) &&  !empty($oParam->dtAquisicaoInicial)){
        $aWhere[] = "t52_dtaqu >= '" . db_formatar($oParam->dtAquisicaoInicial, "d") . "' ";
      }

      if ( isset($oParam->dtAquisicaoFinal) &&  !empty($oParam->dtAquisicaoFinal)){
        $aWhere[] = "t52_dtaqu <= '" . db_formatar($oParam->dtAquisicaoFinal, "d") . "' ";
      }

      if (isset($oParam->iTipoBem) && $oParam->iTipoBem != 1 && $oParam->iTipoBem != 4) {
      ...
      } else if (isset($oParam->iTipoBem) && $oParam->iTipoBem == 4) {
    ...
    case "desprocessaInventario":
      ...
      db_fim_transacao(false);
      $oRetorno->sMessage = _M('patrimonial.patrimonio.pat4_inventario.refine_pesquisa');
