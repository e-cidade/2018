     ...
     if (strstr($res_final," ")) {
       ...
       if ($ed60_c_concluida=="S") {
         ...
         if (strstr($res_final,"R")) {
           ...
           if (!empty($iCodigoEnsino)) {
             $aDadosTermo = DBEducacaoTermo::getTermoEncerramento($iCodigoEnsino, 'R');
       ...
       $resultado = "APROVADO COM PROGRESSAO PARCIAL /DEPENDÊNCIA";
         ...
         if (!empty($iCodigoEnsino)) {
           ...
           $aDadosTermo = DBEducacaoTermo::getTermoEncerramento($iCodigoEnsino, "P");
           if (isset($aDadosTermo[0])) {
             $resultado = $aDadosTermo[0]->sDescricao;
           }
         }
         ...
         if (!empty($iCodigoEnsino)) {
           ...
           $aDadosTermo = DBEducacaoTermo::getTermoEncerramento($iCodigoEnsino, "A");
           if (isset($aDadosTermo[0])) {
             $resultado = $aDadosTermo[0]->sDescricao;
           }
         }
...
function MatriculaPosterior($turma,$aluno) {
  ...
  $sql2                 = "SELECT ed60_i_codigo
                            ...
                            WHERE ed60_i_aluno = $aluno
                              ...
                              AND ed60_c_situacao != 'TROCA DE MODALIDADE'
                         ";
 ...
 if (pg_numrows($result)>0) {
  ...
  for($y=0;$y<pg_num_rows($result);$y++) {
   ...
   if (trim($periodos)=="") {
    ...
    echo '<td class="aluno1">'.(trim($frequencia)==""||trim($frequencia)=="0"?"&nbsp;":($mostraresfinal=="S"?trim($frequencia):"")).'</td>';
    ...
    if (!empty($iCodigoEnsino) && ($resfinal == 'Apr' || $resfinal == 'Rep')) {
      ...
      $aDadosTermo = DBEducacaoTermo::getTermoEncerramento($iCodigoEnsino, substr($resfinal, 0, 1));
      if (isset($aDadosTermo[0])) {
...
function GradeAproveitamentoBOLETIM($matricula, $largura, $pdf, $niveis, $orientacao, $seqatual, $ficha = "N", $sGeraResultado = "S") {
  ...
  if ($orientacao == "P") {
    ...
    $largura_disc = $largura*23/100;//coluna da descriçao da disciplina 23%
    $largura_meio = $largura*50/100;//coluna dos elementos da avaliaçao 50%
    ...
    for ($y = 0; $y < pg_num_rows($result); $y++) {
      ...
      if ($y == 0) {
        ...
        if (trim($periodos) == "") {
          ...
          $largura_ft         = ($largura_periodo*35)/100;
      ...
      if (trim($periodos) == "") {
        ...
        for ($x = 0; $x < count($array_peraval); $x++) {
          ...
          if (isset($arr_explode[5]) && trim(@$arr_explode[5]) == "AVA" && @$arr_explode[10] == "S") {
            ...
            $iInd             = count($aDados);
            $aDados[$iInd]    = substr(trim($arr_explode[3]), 0, 3);
            $aPosicao[$iInd]  = "C";
            ...
            $iInd             = count($aDados);
            $aDados[$iInd]    = trim($arr_explode[4]);
            $aNegrito[$iInd]  = false;
            $aPosicao[$iInd]  = "C";
        ...
        $iInd             = count($aDados);
        $aDados[$iInd]    = ( trim($frequencia) == "0"?"":trim($frequencia));
        $aPosicao[$iInd]  = "C";
...
function NotaParcial() {
 $sql = "case
         ...
         when procresultado.ed43_c_obtencao = 'ME' then
          (select sum(coalesce(diariorescomp.ed73_i_valornota,0)+coalesce(da3.ed72_i_valornota,0))
                    / (count(dr3.ed73_i_valornota) + count(da3.ed72_i_valornota))
          /*        /case when count(dr3.ed73_i_valornota) = 0
                    then count(da3.ed72_i_valornota)
                    else count(dr3.ed73_i_valornota)
                   end
...
function geraArquivoBPA($oDados, $rsCabecalho, $rsProducao, $lValidaCid = true, $sArquivo = "/tmp/arquivobpa.txt") {
    ...
    if (($lErro == true) && ($oDados->sTipo == "02")) {
      ...
      db_msgbox("Arquivo possui registros com ERRO. Verifique!!! ");
...
function getCotasAgendamento($iUpssolicitante, $iUpsprestadora=null, $sRh70_estrutural=null, $iAnocomp=null,
  ...
  if ($oDaoCotasAgendamento->numrows > 0) {
    ...
    $oResult->aCotasAgendamento = db_utils::getColectionByRecord($rs, false, false, true);
 ...
 */
function trocaTurma($iSeqMatricula, $iCodigoTurmaDestino, $lAproveitamento = false, $iMatriculaOrigem = null) {
   ...
   */
  $oDaoTurma             = db_utils::getDao("turma");
  $sSqlSerieTurmaDestino = $oDaoTurma->sql_query_turmaserie(null,
                                                            "distinct ed11_i_codigo,
                                                            turma.*,
                                                            (select max(ed60_i_numaluno)
                                                               from matricula
                                                              where ed60_i_turma = turma.ed57_i_codigo) as ultimo_numero",
                                                            null,
                                                            "ed57_i_codigo = {$iCodigoTurmaDestino}"
                                                           );
  ...
  for ($iSerie = 0; $iSerie < $iTotalSeriesTurma; $iSerie++) {
    ...
    $iTotalMatriculasTurma = $oDadosSerie->ed57_i_nummatr;
  }
  ...
  $oDaoTurma->ed57_i_numvagas          = $oDadosTurma->ed57_i_numvagas;
  ...
  $oDaoTurma->ed57_i_nummatr           = $iTotalMatriculasTurma + 1;
