...
function db_receitappa($anoini, $db_where = false, $retsql = false) {
  ...
  function db_dotacaosaldo($nivel = 8, $tipo_nivel = 1, $tipo_saldo = 2, $descr = true, $where = '', $anousu = null, $dataini = null, $datafim = null, $primeiro_fim = 8, $segundo_inicio = 0, $retsql = false, $tipo_balanco = 1, $desmembra_segundo_inicio = true, $subelemento = 'nao') {
   ...
   ";
    ...
    // $result1 = db_query("select * from work_dotacao");
...
//db_criatabela($result);exit;
 ...
 //   $result = db_query($sql);
  ...
  // aqui termina o codigo de 2007
    ...
    if ($descr == true) {
      if ($query == false) {
         $resultreceita = db_query(analiseQueryPlanoOrcamento($sql2,$anousu));
      } else {
         $resultreceita = analiseQueryPlanoOrcamento($sql2,$anousu);
      }   
    } else {
      if ($query == false) {
         $resultreceita = db_query(analiseQueryPlanoOrcamento($sql,$anousu));
      } else {
        $resultreceita = analiseQueryPlanoOrcamento($sql,$anousu);
      }
    }  
