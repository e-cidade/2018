  ...
  function sql_record() {
    ...
    for ($i = 0; $i < pg_numrows($result); $i ++) {
      ...
      if ($this->lPrevisaoCronograma) {
         ...
         db_query("update work_plano set 
      ...
      for ($ii = 1; $ii < 10; $ii ++) {
        ...
        if (pg_numrows($result_estrut) == 0) {
          $result_estrut = db_query("select o57_descr from orcfontes where o57_anousu = ".$this->anousu." and o57_fonte = '$estrutural'");
...
function db_planosissaldo($anousu, $dataini, $datafim, $retsql = false, $where = '') {
  ...
  db_query("create temporary table work_planosis(estrut_mae varchar(15), 
  estrut varchar(15),
  c61_reduz integer,
  ...
  for ($i = 0; $i < pg_numrows($result); $i ++) {
    //  for($i = 0;$i < 4;$i++){
      ...
      db_query("insert into work_planosis values('$estrut_mae',
      '$estrut',
      $c61_reduz,
      '$c60_descr',
      '$c60_finali',
      ...
      for ($ii = 1; $ii < 11; $ii ++) {
        ...
        if (@ pg_numrows($result_estrut) != true) {
          ...
          $result_1 = db_query("insert into work_planosis values('$estrutural',
          '$estrutural',
          0,
          '$c64_descr',
          '$c64_descr',
          ...
          db_query("update work_planosis set saldo_anterior = saldo_anterior + $tot_anterior ,
...
#echo "<pre>$sql</pre>";    
      ...
      //  for($i = 0;$i < 20;$i++){
        ...
        for ($ii = 1; $ii < 10; $ii ++) {
          ...
          if ($key === false) { // não achou  
            ...
            $res = db_query("select c60_descr,c60_finali,c60_codcon 
            from conplano
    ...
    */
    function db_planocontassaldo_desp_rec($anousu, $dataini, $datafim, $retsql = false, $where = '', $estrut_inicial = '', $encerramento = 'false') {
      ...
      ";
      db_query("create temporary table work_plano (
      estrut_mae varchar(15), 
      estrut varchar(15),
      c61_reduz integer,
      c61_codcon integer,
      ...
      for ($i = 0; $i < pg_numrows($result); $i ++) {
        ...
        for ($ii = 1; $ii < 10; $ii ++) {
          ...
          if ($key === false) { // não achou  
            // busca no banco e inclui
            $res = db_query("select c60_descr,c60_finali,c60_codcon from conplano where c60_anousu=".db_getsession("DB_anousu")." and c60_estrut = '$estrutural'");
            if ($res == false || pg_numrows($res) == 0) {
              db_redireciona("db_erros.php?fechar=true&db_erro=Está faltando cadastrar esse estrutural na contabilidade. Nível : $nivel  Estrutural : $estrutural");
              exit;
            }
    ...
    */
    ...
    function db_elementosaldo($tipo_agrupa = 0, $tipo_saldo = 2, $where = '', $anousu = null, $dataini = null, $datafim = null, $retsql = false) {
      ...
      db_query("create temporary table work_plano($agrupa1 
      codele integer, 
      elemento varchar(13),
      descr varchar(50),
      dot_ini float8,                  
      ...
      // for($i = 0;$i < 10;$i++){
        for ($i = 0; $i < pg_numrows($result); $i ++) {
          ...
          db_query("insert into work_plano values($agrupa2
          $codele, 
          '$elemento', 
          '$descr', 
          $dot_ini, 
          ...
          for ($ii = 1; $ii < 11; $ii ++) {
            ...
            //       db_criatabela($result_estrut);
            if (@ pg_numrows($result_estrut) != true) {
              $res = db_query("select o56_descr from orcelemento where o56_anousu = $anousu and o56_elemento = '$estrutural'");
              if (@ pg_numrows($res) != true)
              break;
              db_fieldsmemory($res, 0);
              
              $result_1 = db_query("insert into work_plano values($agrupa2
              $codele, 
              '$estrutural', 
              '$o56_descr', 
              $dot_ini, 
              ...
              db_query("update work_plano set dot_ini = dot_ini + $tot_dot_ini,
          ...
          for ($i = 0; $i < pg_numrows($result); $i ++) {
            ...
            for ($x = 0; $x < count($paramconta); $x ++) {
              if (in_array($estrutural, $paramconta[$x])) {
                $g = $x;
                db_query("insert into work_grupconta values (' ".$g." ',"."'".$estrutural."',"."'".$c60_descr."',"."$saldo_anterior,"."$saldo_anterior_debito,"."$saldo_anterior_credito,"."$saldo_final)");
              }
          ...
          for ($i = 0; $i < pg_numrows($result); $i ++) {
            ...
            for ($x = 0; $x < count($paramconta); $x ++) {
              if (in_array($estrutural, $paramconta[$x])) {
                ...
                db_query("insert into work_grupconta values (' ".$g." ',"."'".$estrutural."',"."'".$c60_descr."',"."$anterior,"."$inicial,"."$executado )");
              }
              if (substr($estrutural, 0, 1) == 'F') {
                $estrutural = substr($estrutural, 1);
                $g = 'F';
