...
////////// MODELO 6  -  NOTA DE EMPENHO
	...
	$this->objpdf->Setfont('Arial','',8);
	$this->objpdf->text($xcol+108,$xlin+26.5,'AUTORIZAÇÃO DE EMPENHO N'.chr(176).db_formatar($this->numaut,'s','0',5,'e'));
...
//	    $this->objpdf->text($xcol+4,$xlin+58,'ITEM');
        ...
        if ($pagina == 1){
           ...
           $this->objpdf->Roundedrect($xcol+120,$xlin+216,32,28,2,'DF','4');
...
////////// MODELO 7  -  ORDEM DE PAGAMENTO
	...
	$this->objpdf->text(40,$xlin-2,$this->emailpref);
        ...
        /// retangulo dos dados da dotação
	$this->objpdf->Roundedrect($xcol,$xlin+2,$xcol+100,35,2,'DF','1234');
	$this->objpdf->Setfont('Arial','B',8);
	$this->objpdf->text($xcol+2,$xlin+7,'Órgao');
	$this->objpdf->text($xcol+2,$xlin+11,'Unidade');
	$this->objpdf->text($xcol+2,$xlin+15,'Função');
	...
	$this->objpdf->text($xcol+17,$xlin+34,':  '.$this->recurso.' - '.$this->descr_recurso);
        ...
        //// retangulo dos dados do credor
	$this->objpdf->Roundedrect($xcol+106,$xlin+2,96,23,2,'DF','1234');
	$this->objpdf->Setfont('Arial','',6);
	$this->objpdf->text($xcol+108,$xlin+4,'Dados do Credor:');
	$this->objpdf->Setfont('Arial','B',8);
	$this->objpdf->text($xcol+107,$xlin+9,'Numcgm');
	...
	$this->objpdf->Roundedrect($xcol+106,$xlin+28,47,9,2,'DF','1234');
	$this->objpdf->roundedrect($xcol+155,$xlin+28,47,9,2,'DF','1234');
	
        ///// retangulo dos itens	
        $this->objpdf->Roundedrect($xcol+102,$xlin+ 98, 25, 7,2,'DF','');
        $this->objpdf->Roundedrect($xcol+127,$xlin+ 98, 25, 7,2,'DF','');
        $this->objpdf->Roundedrect($xcol+152,$xlin+ 98, 25, 7,2,'DF','');
        $this->objpdf->Roundedrect($xcol+177,$xlin+ 98, 25, 7,2,'DF','');
        $this->objpdf->Roundedrect($xcol+000,$xlin+ 98,102,24,2,'DF','34');
        $this->objpdf->Roundedrect($xcol+000,$xlin+ 48,102,50,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+102,$xlin+ 48, 25,50,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+127,$xlin+ 48, 25,50,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+152,$xlin+ 48, 25,50,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+177,$xlin+ 48, 25,50,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+102,$xlin+105, 75,17,2,'DF','34');
        $this->objpdf->Roundedrect($xcol+177,$xlin+105, 25,17,2,'DF','34');

        ///// retangulo das retenções
        $this->objpdf->Roundedrect($xcol+177,$xlin+179, 25, 8,2,'DF','34');
        $this->objpdf->Roundedrect($xcol+177,$xlin+171, 25, 8,2,'DF','');
        $this->objpdf->Roundedrect($xcol+000,$xlin+133, 75,46,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+000,$xlin+179, 75, 8,2,'DF','34');
        $this->objpdf->Roundedrect($xcol+75 ,$xlin+133, 25,46,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+75 ,$xlin+179, 25, 8,2,'DF','34');
        $this->objpdf->Roundedrect($xcol+102,$xlin+133, 75,38,2,'DF','12');
        $this->objpdf->Roundedrect($xcol+102,$xlin+171, 75, 8,2,'DF','');
        $this->objpdf->Roundedrect($xcol+102,$xlin+179, 75, 8,2,'DF','34');
        $this->objpdf->Roundedrect($xcol+177,$xlin+133, 25,38,2,'DF','12');
...
//		$this->objpdf->roundedrect(10,07,190,183,2,'DF','1234');
		$this->objpdf->Setfont('Arial','B',11);
		$this->objpdf->text(150,$xlin-13,'TRANSFERÊNCIA N'.chr(176).'  00001');
		$this->objpdf->text(159,$xlin-8,$this->datacalc);
		$this->objpdf->Image('imagens/files/'.$this->logo,15,$xlin-17,12);
		$this->objpdf->Setfont('Arial','B',9);
		$this->objpdf->text(40,$xlin-15,$this->prefeitura);
...
//		$this->objpdf->setfillcolor(245);
		...
		$this->objpdf->Setfont('Arial','',8);
		$this->objpdf->text($xcol+22,$xlin+9,':  '.$this->nome);
		$this->objpdf->Setfont('Arial','b',8);
		$this->objpdf->text($xcol+2,$xlin+16,'Usuario');
		$this->objpdf->Setfont('Arial','',8);
		$this->objpdf->text($xcol+22,$xlin+16,':  '.$this->ender);
		$this->objpdf->Setfont('Arial','',6);
		...
		$this->objpdf->Setfont('Arial','',8);
		$this->objpdf->text($xcol+128,$xlin+9,':  '.$this->nomepri);
...
//		$this->objpdf->setfillcolor(245);
		...
		$this->objpdf->text($xcol+2,$xlin+27,'Itens a Transmitir :');
	   	$this->objpdf->sety($xlin+28);
                $maiscol = 0;
                $yy = $this->objpdf->gety();
		$totlinhas = $this->linhasdadospagto;
		for($ii = 0;$ii < $totlinhas;$ii++) {
		   if($ii == 39){
		     $ii = 0;
		     $totlinhas = $totlinhas - 39;
		     $this->objpdf->addpage();
  		     $this->objpdf->setfillcolor(245);
		     $this->objpdf->roundedrect($xcol-2,$xlin-18,206,144.5,2,'DF','1234');
		     $this->objpdf->setfillcolor(255,255,255);
