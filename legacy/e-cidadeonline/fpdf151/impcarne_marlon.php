    ...
    if(($this->objpdf->gety() > $this->objpdf->h - 58 && $contapagina == 1 ) || ( $this->objpdf->gety() > $this->objpdf->h-30 && $contapagina != 1)){
	  ...
	  if(strtoupper(trim($this->municpref)) != 'SAPIRANGA'){
	    ...
	    $this->objpdf->text($xcol+20,$xlin+256,"AUTORIZO",0,4);
            ...
            $this->objpdf->text($xcol+27,$xlin+256,"AUTORIZO",0,4);
...
//      $this->objpdf->text(40,$xlin+2,'Continuação da Página '.($contapagina-1));
	...
	$this->objpdf->Setfont('Arial','B',8);
        ...
        // Caixas dos label's
	$this->objpdf->rect($xcol    ,$xlin+28,10,6,2,'DF','12');
	$this->objpdf->rect($xcol+ 10,$xlin+28,12,6,2,'DF','12');
	$this->objpdf->rect($xcol+ 22,$xlin+28,22,6,2,'DF','12');
	$this->objpdf->rect($xcol+ 44,$xlin+28,98,6,2,'DF','12');
	$this->objpdf->rect($xcol+142,$xlin+28,30,6,2,'DF','12');
	$this->objpdf->rect($xcol+172,$xlin+28,30,6,2,'DF','12');
	  ...
	  $xlin -= 10;
        ...
        // Caixa dos itens
	$this->objpdf->rect($xcol,    $xlin+30,10,258,2,'DF','34');
        // Caixa da quantidade
	$this->objpdf->rect($xcol+ 10,$xlin+30,12,258,2,'DF','34');
	
	$this->objpdf->rect($xcol+ 22,$xlin+30,22,258,2,'DF','34');
        // Caixa dos materiais ou serviços
	$this->objpdf->rect($xcol+ 44,$xlin+30,98,258,2,'DF','34');
        // Caixa dos valores unitário3
	$this->objpdf->rect($xcol+142,$xlin+30,30,258,2,'DF','');
        // Caixa dos valores totais dos iten
	$this->objpdf->rect($xcol+172,$xlin+30,30,258,2,'DF','34');
...
//                  $this->objpdf->Setfont('Arial','B',8);
		    ...
		    $this->objpdf->MultiCell(70,4,$ass_visto,0,"C",0);
	  	    $this->objpdf->setfillcolor(0,0,0);
                    if(strtoupper(trim($this->municpref)) == 'CHARQUEADAS'){
	...
	for($ii = 0;$ii < $this->linhasdositens ;$ii++) {
	  ...
	  $this->objpdf->Setfont('Arial','',8);
          $this->objpdf->Row(array(pg_result($this->recorddositens,$ii,$this->codmater),
				   ...
				   db_formatar(pg_result($this->recorddositens,$ii,$this->valoritem)/pg_result($this->recorddositens,$ii,$this->quantitem),'v'," ",4),
				   db_formatar(pg_result($this->recorddositens,$ii,$this->valoritem),'f')),3,false,4);
	  $xtotal += pg_result($this->recorddositens,$ii,$this->valoritem);
	  $item++;
	  $this->objpdf->Setfont('Arial','B',8);
...
//	    $this->objpdf->sety($this->objpdf->gety+4);
	  ...
	  $this->objpdf->Row(array($item,
			     ...
			     $unid,
	      		     ...
	      		     $valoritem,
	      		     $valimp),3,false,$distanciar,0,0,true);
...
//	$this->objpdf->text($xcol+ 23,$xlin+ 21,':  '.$this->Sdepart);
	...
	$this->objpdf->text($xcol+ 184,$xlin+$getdoy+4,'VALOR TOT.');
        $maiscol = 0;
	$this->objpdf->SetWidths(array(14,12,20,90,28,20,20));
	$this->objpdf->SetAligns(array('C','C','C','J','R','R','R'));
...
////////// MODELO 13  -  SOLICITAÇÃO DE ORÇAMENTO
	...
	$this->objpdf->text(185,$xlin-9,db_formatar($this->Snumero,'s','0',6,'e'));	
	$this->objpdf->Setfont('Arial','',7);
	$this->objpdf->text(130,$xlin-5,"Departamento");
	$this->objpdf->text(185,$xlin-5,": ".$this->Sdepart);	
	$this->objpdf->text(130,$xlin-2,"Fone / Ramal");
	$this->objpdf->text(185,$xlin-2,": ".$this->fonedepto." / ".$this->ramaldepto);	
	$this->objpdf->text(130,$xlin+1,"Fax");
	$this->objpdf->text(185,$xlin+1,": ".$this->faxdepto);	
	$this->objpdf->Setfont('Arial','B',9);
	$this->objpdf->text(40,$xlin-15,$this->prefeitura);
	$this->objpdf->Setfont('Arial','',7);
	$this->objpdf->text(40,$xlin-11,$this->enderpref);
...
//		$this->objpdf->roundedrect(10,07,190,183,2,'DF','1234');
		...
		$this->objpdf->Setfont('Arial','',7);
		$this->objpdf->text($xcol+2,$xlin+5,'Código');
		$this->objpdf->text($xcol+17,$xlin+5,'Nome');
		$this->objpdf->text($xcol+90,$xlin+5,'Função');
		$this->objpdf->text($xcol+120,$xlin+5,'Lotação');

		$this->objpdf->Setfont('Arial','B',8);
                $this->objpdf->text($xcol+2,$xlin+8,$this->registro);
                $this->objpdf->text($xcol+17,$xlin+8,$this->nome);
		$this->objpdf->text($xcol+90,$xlin+8,$this->descr_funcao);
		$this->objpdf->text($xcol+120,$xlin+8,$this->descr_lota);
...
//	db_msgbox($setaut.' -- '.$posini);
	...
	for($i=0;$i<$this->linhasdasdotac;$i++){
	  ...
	  $valimp        = $valtot;
	  $prazo         = pg_result($this->recorddasdotac,$i,$this->sprazo);
	  $pgto          = pg_result($this->recorddasdotac,$i,$this->spgto);
	  $resum         = pg_result($this->recorddasdotac,$i,$this->sresum);
	  $just          = pg_result($this->recorddasdotac,$i,$this->sjust);
...
//	  db_msgbox($dvalor.'   <-dval- '.$dvaltot.' <- dvltot- '.$dquant.'   <- dquant- '.$quantitem.'    <-qit- '.$valoritem.'  <-vlit');
          ...
          if(isset($dotacao) && trim($dotacao)!=""){
            ...
            if($clorcreservasol->numrows>0){
              ...
              if($valorreserva==$valimp){
                ...
                $ddvalor = "PARCIAL - R$ ". $valorreserva;
              }
            }
          }
	  ...
	  $valoritem = db_formatar($valoritem,'f');

          $this->objpdf->Setfont('Arial','B',7);
          /*
          $this->objpdf->ln(1.5);
          if(isset($selemento) && trim($selemento)!="" && $selemento!=$elementoant){
            $this->objpdf->ln(2);
            if($elementoant!=""){
              $this->objpdf->ln(0.3);
              $this->objpdf->rect(4,$this->objpdf->gety(),202,0,1,'DF','1234');
	  ...
	  $x = $this->muda_pag($pagina,$xlin,$xcol,"false",$contapagina);
          ...
          if(isset($ddvalor) && $ddvalor!=""){
            if((isset($prazo) && $prazo!="") || (isset($pgto) && $pgto!="") || (isset($resum) && $resum!="" && (isset($scodpcmater) && trim($scodpcmater)!="")) || (isset($just) && $just!="")){
              $this->objpdf->ln(2.5);
            }
...
////////// MODELO 18  -  REQUISIÇÃO DE SAÍDA DE MATERIAIS 
    ...
    */
	...
	$this->objpdf->Image('imagens/files/'.$this->logo,20,9,15);
	$this->objpdf->Setfont('Arial','B',9);
	...
	$this->objpdf->text(150,24,'Lei 010.2003');
//	$this->objpdf->text(40,28,$this->emailpref);
...
//	$this->objpdf->setfillcolor(245);
	...
	$this->objpdf->Setfont('Arial','',6);
	$this->objpdf->text(17,47,'Ente Federativo');
	$this->objpdf->text(97,47,'Código');
	$this->objpdf->text(130,47,'Operação');
	$this->objpdf->text(163,47,'Emissão');
	
	$this->objpdf->rect(15,55,80,10,'DF');
	$this->objpdf->rect(95,55,50,10,'DF');
	$this->objpdf->rect(145,55,50,10,'DF');
	
	$this->objpdf->Setfont('Arial','',6);
	$this->objpdf->text(17,57,'Fundo');
	$this->objpdf->text(97,57,'CNPJ do Fundo');
	$this->objpdf->text(147,57,'Início das Atividades do Fundo');
	
	$this->objpdf->Setfont('Arial','BI',8);
	$this->objpdf->text(17,77,'Administadora');
	
	$this->objpdf->rect(15,80,72.5,10,'DF');
	$this->objpdf->rect(87.5,80,72.5,10,'DF');
	$this->objpdf->rect(160,80,35,10,'DF');

	$this->objpdf->Setfont('Arial','',6);
	$this->objpdf->text(17,82,'Nome');
	$this->objpdf->text(89,82,'Endereço');
	$this->objpdf->text(162,82,'CNPJ da Administradora');

	$this->objpdf->Setfont('Arial','BI',8);
	$this->objpdf->text(17,102,'Contribuinte');
	
	$this->objpdf->rect(15,105,85,10,'DF');
	$this->objpdf->rect(100,105,30,10,'DF');
	$this->objpdf->rect(130,105,30,10,'DF');
	$this->objpdf->rect(160,105,18,10,'DF');
	$this->objpdf->rect(178,105,17,10,'DF');

	$this->objpdf->Setfont('Arial','',6);
	$this->objpdf->text(17,107,'Nome');
	$this->objpdf->text(102,107,'CPF/CNPJ');
	$this->objpdf->text(132,107,'Matrícula');
	$this->objpdf->text(162,107,'Ano');
	$this->objpdf->text(180,107,'Folha');
