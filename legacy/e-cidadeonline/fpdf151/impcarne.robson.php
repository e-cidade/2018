...
//        $this->objpdf->MultiCell(90,4,'..........................................................................................',0,"C",0);
	...
	$this->objpdf->Text($coluna + 60,$linha+63,($this->numero == ""?"":$this->numero));
        ...
        if ($this->compl != "") {
   	  $this->objpdf->SetFont('Arial','B',$fonte);
	  $this->objpdf->Text($coluna + 60 ,$linha+63,"COMPLEMENTO: "); // endereco
	  $this->objpdf->SetFont('Arial','',$fonte);
	  $this->objpdf->Text($coluna + 110,$linha+63,($this->compl == ""?"":$this->compl));
        }
	...
	if($this->q02_memo!=''){
	  ...
	  $this->objpdf->roundedrect($coluna-2,$linha-1,187,8,2,'1234');
	  $this->objpdf->SetFont('Arial','B',13);
  	  $this->objpdf->Ln(2);
	  $this->objpdf->setx(15);
	  $this->objpdf->Multicell(0,3,"ATIVIDADE PRINCIPAL: " . $this->descrativ) ; // descrição da atividade principal
	  ...
	  if(isset($this->q03_atmemo[$this->ativ])){
	    if ($this->q03_atmemo[$this->ativ] != '') {;
	      ...
	      $this->objpdf->setx(15);
	      $this->objpdf->SetFont('Arial','',12);
	      $this->objpdf->Multicell(0,5,$this->q03_atmemo[$this->ativ]); // texto
              $yyyatual = $this->objpdf->gety();
	      $this->objpdf->roundedrect($coluna-2,$linha-3,187,$yyyatual-$yyy,2,'1234'); // obs da atividade principal
	    }
	  }
...
//           $this->objpdf->roundedrect(42,$yyy-1,127,5,2,'1234'); // descricao da atividade secundaria
           ...
           $this->objpdf->roundedrect($coluna-2,$y,187,8,2,'1234'); // descricao da atividade secundaria

	   $this->objpdf->Ln(4);
	   for($i=0; $i<$num_outras; $i++){
             $yyy = $this->objpdf->gety();
	     $chave=key($this->outrasativs);
	     //$chave=key($this->outrasativs[$chave][$q07_datain][$q07_datafi][]);
	     $obs='';
	     if(isset($this->q03_atmemo[$chave])){
	       $obs = $this->q03_atmemo[$chave];
	     }
...
//	  }
	  ...
	  if(isset($prazo) && $prazo!=""){
	    $this->objpdf->Row(array('','',$unid,$prazo,'',''),3,false,$dist,0,0,true);
	    $x = $this->muda_pag($pagina,$xlin,$xcol,"false",$contapagina);
	  }
