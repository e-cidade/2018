...
then
  ...
  psql -p ${sPorta} -U ${sDbUser} -h ${sDbHost} ${sBaseName} -c "update db_config set db21_ativo = 3"; 
  ...
  # Ativando o dbportal  
  psql -p ${sPorta} -U ${sDbUser} -h ${sDbHost} ${sBaseName} -c "update db_config set db21_ativo = 1"; 
