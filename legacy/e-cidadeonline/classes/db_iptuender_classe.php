...
class cl_iptuender { 
   ...
   var $campos = "
                 ...
                 j43_iptuendergrupo = int4 = Grupo de Endereço de Entrerga 
                 ";
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->j43_iptuendergrupo = ($this->j43_iptuendergrupo == ""?@$GLOBALS["HTTP_POST_VARS"]["j43_iptuendergrupo"]:$this->j43_iptuendergrupo);
     }else{
       $this->j43_matric = ($this->j43_matric == ""?@$GLOBALS["HTTP_POST_VARS"]["j43_matric"]:$this->j43_matric);
     }
   }
   ...
   function incluir ($j43_matric){ 
     ...
     if($this->j43_iptuendergrupo == null ){ 
       $this->j43_iptuendergrupo = "0";
     }
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       ...
       $resac = db_query("insert into db_acount values($acount,32,19156,'','".AddSlashes(pg_result($resaco,0,'j43_iptuendergrupo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     }
     return true;
   } 
   ...
   function alterar ($j43_matric=null) { 
     ...
     if(trim($this->j43_iptuendergrupo)!="" || isset($GLOBALS["HTTP_POST_VARS"]["j43_iptuendergrupo"])){ 
        if(trim($this->j43_iptuendergrupo)=="" && isset($GLOBALS["HTTP_POST_VARS"]["j43_iptuendergrupo"])){ 
           $this->j43_iptuendergrupo = "0" ; 
        } 
       $sql  .= $virgula." j43_iptuendergrupo = $this->j43_iptuendergrupo ";
       $virgula = ",";
     }
     $sql .= " where ";
     if($j43_matric!=null){
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         if(isset($GLOBALS["HTTP_POST_VARS"]["j43_iptuendergrupo"]) || $this->j43_iptuendergrupo != "")
           $resac = db_query("insert into db_acount values($acount,32,19156,'".AddSlashes(pg_result($resaco,$conresaco,'j43_iptuendergrupo'))."','$this->j43_iptuendergrupo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     $result = db_query($sql);
     if($result==false){ 
   ...
   function excluir ($j43_matric=null,$dbwhere=null) { 
     ...
     if(($resaco!=false)||($this->numrows!=0)){
       for($iresaco=0;$iresaco<$this->numrows;$iresaco++){
         ...
         $resac = db_query("insert into db_acount values($acount,32,19156,'','".AddSlashes(pg_result($resaco,$iresaco,'j43_iptuendergrupo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
   ...
   function sql_query ( $j43_matric=null,$campos="*",$ordem=null,$dbwhere=""){ 
     ...
     $sql .= "      left  join iptuendergrupo  on  iptuendergrupo.j135_sequencial = iptuender.j43_iptuendergrupo";
