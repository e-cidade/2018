...
use ECidade\Saude\Agendamento\Exame\Cota\Mensal\Municipio\Repository;
use ECidade\Saude\Agendamento\Exame\Cota\Mensal\Municipio\Individual;
  ...
  switch ($oParametros->sExecucao) {
    ...
    case "salvar":
      ...
      foreach ($oParametros->aCompetencia as $iIndice => $oCompetencia) {
        ...
        $oCotaMensal->setProcedimento($oParametros->procedimento);
    ...
    case 'validar':
      ...
      foreach ($oParametros->aCompetencia as $oCompetencia) {
        $lValidado = $oRepository->checkByCompetenciaProcedimento($oCompetencia->mes,
                                                                  $oCompetencia->ano,
                                                                  $oParametros->procedimento);

        if ( !$lValidado ) {
          throw new BusinessException("Já existe uma Cota lançada para este(s) Exame(s) na Competência ".$oCompetencia->mes." / ".$oCompetencia->ano.".");
        }
      }
