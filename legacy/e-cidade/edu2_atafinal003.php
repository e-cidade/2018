...
for ($x = 0; $x < $linhas; $x ++) {
  ...
  $head5  = "Etapa:$ed11_c_descr  Ano:$ed52_i_ano  C.H. Total:$aulas Dias Letivos:$ed52_i_diasletivos";
  $head6  = "Turma: $ed57_c_descr  Turno: $ed15_c_nome";
  ...
  for ($z = 0; $z < $clmatricula->numrows; $z ++) {
    ...
    if ($cont4 == $limite && ($cont_geral + 1) < $clmatricula->numrows) {
	  ...
	  $pdf->cell(93, 4, "Convenções", 1, 1, "C", 0);
	  ...
	  if ($quebra == "1") {
		...
		$pdf->cell(96, 4, "", "R", 1, "L", 0);
		$cont5 ++;
		
	  }
	  
	  for ($y = ($cont5 / 2); $y < 12; $y ++) {
		$pdf->cell(96, 4, "", "L", 1, "L", 0);
	  }
	  ...
	  $pdf->setX(105);
	  $pdf->cell(48, 4, "SECRETÁRIO(A)", "L", 0, "C", 0);
	  $pdf->cell(48, 4, "DIRETOR(A)", "R", 1, "C", 0);
      $pdf->setX(105);
  ...
  $pdf->cell(100, 4, "Observações", 1, 0, "C", 0);
  $pdf->cell(96, 4, "Convenções", 1, 1, "C", 0);
  $sWhere   = " ed59_i_codigo in ($reg_pagina)  AND ed59_c_condicao = 'OB' AND ed59_i_serie = $ed223_i_serie";
  $sql2     = $clregencia->sql_query("",
                                     "ed59_i_codigo,ed232_c_abrev,ed232_c_descr,ed59_i_ordenacao", 
                                     "ed59_i_ordenacao", 
  ...
  $sql99    = $clregenciahorario->sql_query("", 
	...
	if ($clregenciahorario->numrows > 0) {
	  ...
	  for ($p = 0; $p < $linhas99; $p ++) {
		...
		$pdf->line(5, $pdf->getY(), 60, $pdf->getY());
		$pdf->cell(190, 4, "Professor", 0, 1, "L", 0);
		$pdf->cell(190, 4, $z01_nome . "-" . $ed20_i_codigo, 0, 1, "L", 0);
		...
		for ($w = 0; $w < $clregenciahorario->numrows; $w ++) {
		  ...
		  $pdf->cell(190, 4, $ed232_c_descr, 0, 1, "L", 0);
		  $pdf->cell(220, 2, "", 0, 1, "L", 0);
	  ...
	  $pdf->cell(220, 10, "Nenhum Professor Informado!", 0, 1, "L", 0);
	}
}
$pdf->Output();
?>