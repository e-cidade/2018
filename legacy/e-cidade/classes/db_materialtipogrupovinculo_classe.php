...
class cl_materialtipogrupovinculo { 
   ...
   function atualizacampos($exclusao=false) {
     if($exclusao==false){
       ...
       $this->m04_materialestoquegrupo = ($this->m04_materialestoquegrupo == ""?@$GLOBALS["HTTP_POST_VARS"]["m04_materialestoquegrupo"]:$this->m04_materialestoquegrupo);
     }else{
       $this->m04_sequencial = ($this->m04_sequencial == ""?@$GLOBALS["HTTP_POST_VARS"]["m04_sequencial"]:$this->m04_sequencial);
     }
   }
   ...
   function incluir ($m04_sequencial){ 
     ...
     if($this->m04_materialestoquegrupo == null ){ 
   ...
   public function alterar ($m04_sequencial=null) { 
      $this->atualizacampos();
   ...
   public function excluir ($m04_sequencial=null,$dbwhere=null) { 
     ...
     if ($result == false) { 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "Tipos de Grupo do Material Vinculo nao Excluído. Exclusão Abortada.\\n";
       ...
       if (pg_affected_rows($result) == 0) {
         $this->erro_banco = "";
         $this->erro_sql = "Tipos de Grupo do Material Vinculo nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$m04_sequencial;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   // funcao do sql 
   public function sql_query ($m04_sequencial = null,$campos = "*", $ordem = null, $dbwhere = "") { 

     $sql  = "select {$campos}";
     $sql .= "  from materialtipogrupovinculo ";
     $sql .= "      inner join materialestoquegrupo  on  materialestoquegrupo.m65_sequencial = materialtipogrupovinculo.m04_materialestoquegrupo";
   ...
   // funcao do sql 
   public function sql_query_file ($m04_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "") {
