...
class cl_cgsfatorderisco { 
   ...
   var $campos = "
                 s106_i_codigo = int4 = Código 
                 s106_i_cgs = int4 = Prontuário 
   ...
   function incluir ($s106_i_codigo){ 
     ...
     if($s106_i_codigo == "" || $s106_i_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $s106_i_codigo)){
         $this->erro_sql = " Campo s106_i_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "CGS Fator de Risco ($this->s106_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "CGS Fator de Risco já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "CGS Fator de Risco ($this->s106_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($s106_i_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "CGS Fator de Risco nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "CGS Fator de Risco nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$s106_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
   ...
   /*
   Query que faz a ligação com os fatores de risco da farmácia
   */
