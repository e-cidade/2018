...
//  $this->objpdf->Text($x+138, $y+30,str_pad($this->nosso_numero,17,"0",STR_PAD_LEFT));//   nosso numero
  ...
  $this->objpdf->Text($x+34,  $y+37,$this->carteira);                  // carteira interceptar variável
  ...
  if(@$this->desconto_abatimento!=""){
	...
	$this->objpdf->SetFont('Arial','',5);
  $this->objpdf->multicell(130, 2.3, $instrucao); // Instrução
  ...
  if ( $this->partilhaTipoLancamento != "") {
    ...
    $this->objpdf->SetFont('Arial','B',5);
    $this->objpdf->cell(20, 2,  "Situação das Custas: ",0,0,"L");
    $this->objpdf->SetFont('Arial','',5);
    ...
    $this->objpdf->setX($x+2);
    $this->objpdf->SetFont('Arial','B',5);
    $this->objpdf->cell(18, 2,  "Observação : ",0,1,"L");
