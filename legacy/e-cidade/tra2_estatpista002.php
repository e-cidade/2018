 ...
 */
 ...
 $head4 = 'Acidentes por Pista';
 $pdf->open();
 $pdf->addpage();
 $pdf->aliasNbPages();
 $pdf->setx(10);
 $pdf->setfillcolor(204);
 $pdf->Cell(75,5,"Tipo de Pistas",1,1,"C",1);
 $sql1 = "select tr03_id,
                 tr03_descr,
                 tr03_sigla
          from   tipo_pista
 ...
 while ($ln1 = pg_fetch_array($rs1)){
    $pdf->setx(10);
    $pdf->cell(50,5,$ln1["tr03_descr"],1,0,"L");
    $pdf->cell(25,5,$ln1["tr03_sigla"],1,1,"C");
 }
 $x = 0;
 $pdf->Cell(75,5,"Total",1,1,"C");
  for ($i = 1; $i <= 12 ;$i++){
      ...
      $sql2 = "SELECT tr03_id,
             ...
             from tipo_pista left join acidentes
                  on tr03_id = tr07_idpista and extract(year from tr07_data) = $ano
                                            and extract(month from tr07_data) = $i
             group by tr03_id order by tr03_id";
