...
try {
  ...
  switch ($oParam->exec) {
    ...
    case 'getBancosConfigurados':
      ...
      if (count($aBancos) == 0) {
        throw new BusinessException("Não existem configurações de consignações. Antes de importar configure em\nProcedimentos > Manutenção de Empréstimos Consignados > Parâmetros > Configuração Consignados.");
      }
    ...
    case 'importarArquivo':
      ...
      if (empty($oConfiguracao)) {
        throw new BusinessException('Não foram encotradas configurações para o banco informado');
      }
