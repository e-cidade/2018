...
class ZonasvalorIPTU implements iViradaIPTU {
  ...
  function __construct() {
    ...
    $sSqlIptuTabelasConfig  = $oDaoIptuTabelasConfig->sql_query(null, "iptutabelasconfig.j122_sequencial", 
                                                                null, "db_sysarquivo.nomearq = 'zonasvalor'");
    ...
    if ($oDaoIptuTabelasConfig->numrows > 0) {
      ...
      if ($oDaoIptuTabelasConfigVirada->numrows > 0) {
        ...
        $sMensagem = "ERRO: Tabela zonasvalor já foi feito virada anual para exercicío {$this->getAnoNovo()}!";
    ...
    $sWhere                           = "db_sysarquivo.nomearq = 'zonasvalor'";
  ...
  public function vira() {
    ...
    $oDaoZonasValor  = db_utils::getDao('zonasvalor');
    
    $oDaoZonasValor->excluir(null, null, " j51_anousu = {$this->getAnoNovo()} ");
    if ($oDaoZonasValor->erro_status == 0) {
      throw new Exception($oDaoZonasValor->erro_msg);
    }
    
    $sWhere       = " j51_anousu = {$this->getAnoAtual()} ";
    $sSqlZonasValor  = $oDaoZonasValor->sql_query_file(null, null, "*", null, $sWhere);
    $rsSqlZonasValor = $oDaoZonasValor->sql_record($sSqlZonasValor);
    $iNumRows = $oDaoZonasValor->numrows;
    if ($iNumRows > 0) {

    	for ( $iInd=0; $iInd < $iNumRows; $iInd++ ) {
    		
    		$oDadosZonasValor  = db_utils::fieldsMemory($rsSqlZonasValor, $iInd);
        $aCamposZonasValor = get_object_vars($oDadosZonasValor);
        
        foreach ($aCamposZonasValor as $sNomeCampoZonasValor => $sValorCampoZonasValor ) {
        	
          if (in_array($sNomeCampoZonasValor, $this->getCampoCorrecao())) {
            
            $nPercentual     = $this->getPercentual();
            $nSomaPercentual = ($oDadosZonasValor->$sNomeCampoZonasValor + ($oDadosZonasValor->$sNomeCampoZonasValor * ($nPercentual / 100)));
            ...
            $oDaoZonasValor->$sNomeCampoZonasValor = $oDadosZonasValor->$sNomeCampoZonasValor;
          }
        }
        
        $oDaoZonasValor->incluir($iZona, $iAnoUso);
        if ($oDaoZonasValor->erro_status == 0) {
          throw new Exception($oDaoZonasValor->erro_msg);
        }
    	}
