...
class ZonastaxaIPTU implements iViradaIPTU {
  ...
  function __construct() {
    ...
    $sSqlIptuTabelasConfig  = $oDaoIptuTabelasConfig->sql_query(null, "iptutabelasconfig.j122_sequencial", 
                                                                null, "db_sysarquivo.nomearq = 'zonastaxa'");
    ...
    if ($oDaoIptuTabelasConfig->numrows > 0) {
      ...
      if ($oDaoIptuTabelasConfigVirada->numrows > 0) {
        ...
        $sMensagem = "ERRO: Tabela zonastaxa já foi feito virada anual para exercicío {$this->getAnoNovo()}!";
    ...
    $sWhere                           = "db_sysarquivo.nomearq = 'zonastaxa'";
  ...
  public function vira() {
    ...
    $oDaoZonasTaxa  = db_utils::getDao('zonastaxa');
    
    $oDaoZonasTaxa->excluir(null, null, null, " j57_anousu = {$this->getAnoNovo()} ");
    if ($oDaoZonasTaxa->erro_status == 0) {
      throw new Exception($oDaoZonasTaxa->erro_msg);
    }
    
    $sWhere         = " j57_anousu = {$this->getAnoAtual()} ";
    $sSqlZonasTaxa  = $oDaoZonasTaxa->sql_query_file(null, null, null, "*", null, $sWhere);
    $rsSqlZonasTaxa = $oDaoZonasTaxa->sql_record($sSqlZonasTaxa);
    $iNumRows = $oDaoZonasTaxa->numrows;
    if ($iNumRows > 0) {

    	for ( $iInd=0; $iInd < $iNumRows; $iInd++ ) {
    		
    		$oDadosZonasTaxa  = db_utils::fieldsMemory($rsSqlZonasTaxa, $iInd);
        $aCamposZonasTaxa = get_object_vars($oDadosZonasTaxa);
        
        foreach ($aCamposZonasTaxa as $sNomeCampoZonasTaxa => $sValorCampoZonasTaxa ) {
        	
          if (in_array($sNomeCampoZonasTaxa, $this->getCampoCorrecao())) {
            
            $nPercentual     = $this->getPercentual();
            $nSomaPercentual = ($oDadosZonasTaxa->$sNomeCampoZonasTaxa + ($oDadosZonasTaxa->$sNomeCampoZonasTaxa * ($nPercentual / 100)));
            $oDaoZonasTaxa->$sNomeCampoZonasTaxa = "{$nSomaPercentual}";
          } else {
          	
            if (trim($sNomeCampoZonasTaxa) == 'j57_zona') {
              $iZona = $oDadosZonasTaxa->$sNomeCampoZonasTaxa;
            }
            
            if (trim($sNomeCampoZonasTaxa) == 'j57_receit') {
              $iReceita = $oDadosZonasTaxa->$sNomeCampoZonasTaxa;
            }
            
          	if (trim($sNomeCampoZonasTaxa) == 'j57_anousu') {
          		$iAnoUso = $this->getAnoNovo();
          	}
            
            $oDaoZonasTaxa->$sNomeCampoZonasTaxa = $oDadosZonasTaxa->$sNomeCampoZonasTaxa;
          }
        }
        
        $oDaoZonasTaxa->incluir($iZona, $iReceita, $iAnoUso);
        if ($oDaoZonasTaxa->erro_status == 0) {
          throw new Exception($oDaoZonasTaxa->erro_msg);
        }
    	}
