...
class AnexoXIRREO_2017 extends RelatoriosLegaisBase implements AnexoRREO
{
    ...
    const LINHA_FIM_DESPESAS = 10;
    const LINHA_SALDO_FINANCEIRO = 11;
    ...
    private function escreverReceitas()
    {
        ...
        for ($iLinha = self::LINHA_INICIO_RECEITAS; $iLinha <= self::LINHA_FIM_RECEITAS; $iLinha++) {
            ...
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->prevatu, 'f'), 'BRL', 0, "R",
                0);
            $this->oPdf->Cell($this->iLargura * 0.50, $this->iAltura, db_formatar($oLinha->recatebim, 'f'), 'BRL', 0,
                "R", 0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->saldo, 'f'), 'BL', 1, "R",
                0);
        }
        $this->oPdf->ln(4);
    }
    ...
    private function escreverDespesas()
    {
        ...
        $this->oPdf->MultiCell($this->iLargura * 0.10, $this->iAltura * 1.5, "DESPESAS EMPENHADAS\n(e)", 'TBRL', 'C',
            1);
        ...
        $this->oPdf->MultiCell($this->iLargura * 0.10, $this->iAltura * 0.75,
        ...
        for ($iLinha = self::LINHA_INICIO_DESPESAS; $iLinha <= self::LINHA_FIM_DESPESAS; $iLinha++) {
            ...
            $sDescricao = relatorioContabil::getIdentacao($oLinha->nivel) . $oLinha->descricao;
            $this->oPdf->Cell($this->iLargura * 0.30, $this->iAltura, $sDescricao, 'BR', 0, "L", 0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->dot_atual, 'f'), 'BRL', 0,
                "R", 0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->despemp, 'f'), 'BRL', 0, "R",
                0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->despliq, 'f'), 'BRL', 0, "R",
                0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->desppag, 'f'), 'BRL', 0, "R",
                0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->insc_rp_np, 'f'), 'BRL', 0,
                "R", 0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->rp_apagar, 'f'), 'BRL', 0,
                "R", 0);
            $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->saldo, 'f'), 'BL', 1, "R",
                0);
        }
    ...
    private function escreverSaldoFinanceiro()
    {
        ...
        $this->oPdf->MultiCell($this->iLargura * 0.50, $this->iAltura * 1.5,
            "{$this->iAnoUsu}\n(j) = (Ib - (IIf+ IIg))", 'TBRL', 'C', 1);
        $this->oPdf->setAutoNewLineMulticell(true);
        ...
        $this->oPdf->Cell($this->iLargura * 0.10, $this->iAltura, db_formatar($oLinha->vlrexanter, 'f'), 'TBRL', 0,
            'R');
        $this->oPdf->Cell($this->iLargura * 0.50, $this->iAltura, db_formatar($oLinha->vlrexatual, 'f'), 'TBRL', 0,
            'R');
    ...
    public function emitir()
    {
        ...
        $this->oPdf->Cell($this->iLargura * 0.50, $this->iAltura, 'RREO - ANEXO 11 (LRF, art. 53, § 1º, inciso III)',
            'B', 0, "L", 0);
    ...
    /**
     * Processa os dados da coluna de restos a pagar
     */
