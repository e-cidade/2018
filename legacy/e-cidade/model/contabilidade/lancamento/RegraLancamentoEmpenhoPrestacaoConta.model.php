...
class RegraLancamentoEmpenhoPrestacaoConta implements IRegraLancamentoContabil {
  ...
  public function getRegraLancamento($iCodigoDocumento, $iCodigoLancamento, ILancamentoAuxiliar $oLancamentoAuxiliar) {
    ...
    for ($i = 0; $i < $oDaoTransacao->numrows; $i++) {
      ...
      /**
       * Verificamos se necessitamos comparar a conta debito do lancamento
       * auxiliar com a conta debito da transacao
       * -> Processamento
       */
      if ($oLancamentoAuxiliar->getContaDebito() != "") {

        if ($oDadosTransacao->c47_debito == $oLancamentoAuxiliar->getContaDebito()) {
          return new RegraLancamentoContabil($oDadosTransacao->c47_seqtranslr);
        }
      }

      /**
       * Verificamos se necessitamos comparar a conta credito do lancamento
       * auxiliar com a conta credito da transacao
       * -> Estorno
       */
      if ($oLancamentoAuxiliar->getContaCredito() != "") {

        if ($oDadosTransacao->c47_credito == $oLancamentoAuxiliar->getContaCredito()) {
          return new RegraLancamentoContabil($oDadosTransacao->c47_seqtranslr);
        }
      }
    }
