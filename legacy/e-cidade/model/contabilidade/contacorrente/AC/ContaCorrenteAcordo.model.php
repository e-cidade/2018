...
/**
 * Model responsбvel pelos acordos
 * @package Contabilidade
 * @subpackage contacorrente
 * @version 1.0
 */
class ContaCorrenteAcordo extends ContacorrenteBase implements IContaCorrente {

  /**
   * Registro na tabela contacorrente
   * @var integer
   */
  const CONTA_CORRENTE = 108;
   ...
   * - se jб existir, simplesmente vincula com a tabela contacorrentedetalheconlancamval
   *
   * @throws DBException
   */
  public function salvar($dtLancamento = null) {
    ...
    $iAcordo        = $this->oContaCorrenteDetalhe->getAcordo()->getCodigo();
  ...
  private function validarInformacoes() {
    ...
    if ( !$this->oContaCorrenteDetalhe->getAcordo() instanceof Acordo) {
      throw new ParameterException("Acordo nгo informado para execuзгo do conta corrente {$this->oContaCorrente->getDescricao()}.");
    }
