...
abstract class AjusteFerias {
   ...
   */
  public static function lancarRegistrosPonto($iMatricula, $Ipessoal, $lFuncaoGerfsal = false) {

    global $pessoal, $subpes,$cadferia, $F016, $F017, $F018, $F019, $F020, $F021,
           $F023, $cfpess, $d08_carnes, $opcao_geral, $anousu, $mesusu, $DB_instit;
    ...
    $sWhereRubricasFerias  = "cfpess.r11_anousu = {$iAnoFolha} ";
    $sWhereRubricasFerias .= "and cfpess.r11_mesusu = {$iMesFolha} ";
    $sWhereRubricasFerias .= "and cfpess.r11_instit = {$iInstituicao} and (    r11_fer13 = rhhistoricoponto.rh144_rubrica ";
    $sWhereRubricasFerias .= "                                              or r11_ferabo = rhhistoricoponto.rh144_rubrica ";
    $sWhereRubricasFerias .= "                                              or r11_fer13a = rhhistoricoponto.rh144_rubrica ";
    $sWhereRubricasFerias .= "                                              or r11_feradi = rhhistoricoponto.rh144_rubrica ";
    $sWhereRubricasFerias .= "                                              or r11_fadiab = rhhistoricoponto.rh144_rubrica ";
    $sWhereRubricasFerias .= "                                            )";

    $sSqlRubricasFerias = $oDaoCfpess->sql_query_file (null,null,null, 1, null, $sWhereRubricasFerias);

    $sWhereRhHistoricoPonto  = "     rh141_anousu = {$iAnoFolha}";
    $sWhereRhHistoricoPonto .= " and rh141_mesusu = {$iMesFolha}";
    $sWhereRhHistoricoPonto .= " and rh141_instit = {$iInstituicao}";
    $sWhereRhHistoricoPonto .= " and rh141_tipofolha = " . FolhaPagamento::TIPO_FOLHA_COMPLEMENTAR;
    if (FolhaPagamentoComplementar::getFolhaAberta()) {
      $sWhereRhHistoricoPonto .= " and rh141_sequencial <> " . FolhaPagamentoComplementar::getFolhaAberta()->getSequencial();
    }
    $sWhereRhHistoricoPonto .= " and rh144_regist = {$iMatricula}";
    $sWhereRhHistoricoPonto .= " and exists ({$sSqlRubricasFerias})";

    $sSqlHistoricoPonto  = $oDaoRhHistoricoPonto->sql_query(null,"rh144_sequencial", null, $sWhereRhHistoricoPonto);
    $rsSqlHistoricoPonto = db_query($sSqlHistoricoPonto);

    $lCalculoFeriasEfetuado = false;

    if (pg_num_rows($rsSqlHistoricoPonto) > 0) {
      $lCalculoFeriasEfetuado = true;
    }
    ...
    if(!$lFuncaoGerfsal) {
       ...
       */
      $iTipoFolha = AjusteFerias::verificaPagamento($iMatricula);
       ...
       if ( !db_selectmax("pontofs", $sSqlValidacaoPontoSalario) ) {
        ...
        LogCalculoFolha::write("Numero de Dias a Pagar no mês..: {$F019}");
          ...
          if ($F019 < 30) {
            ...
            verifica_ferias_100($lCalculoFeriasEfetuado);
              ...
              if ( $opcao_geral == PONTO_SALARIO ) {
                ...
                if (!db_empty($pessoal[$Ipessoal]["r01_arredn"])) {
                  ...
                  if (db_selectmax("transacao", "select * from gerfsal ".bb_condicaosubpes("r14_" ).$condicaoaux )) {
                    ...
                    $sWhere     = bb_condicaosubpes("r14_")." and r14_regist = {$matriz2[1]} and r14_rubric = '{$matriz2[2]}'";

                    db_delete("gerfsal", $sWhere);
          ...
          LogCalculoFolha::write(" Valores das F's para a matricula {$iMatricula}: F019: {$F019} F020: {$F020}  F023: {$F023}");
