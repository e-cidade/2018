...
for( $iX = 0; $iX < $iTam; $iX++ ) {
  ...
  $sCamposProcedimentos  = "*";
  $sCamposProcedimentos .= " ,(select s115_c_cartaosus";
  $sCamposProcedimentos .= "      from cgs_cartaosus";
  $sCamposProcedimentos .= "     where s115_i_cgs = z01_i_cgsund";
  $sCamposProcedimentos .= "     order by s115_c_tipo, s115_i_codigo limit 1) as cartaosus";

  $sSql               = $clprontproced->sql_query_ext(null, $sCamposProcedimentos, null, "sd29_i_prontuario = {$aChaveProntuarios[$iX]}");
...
if( in_array( 0, $linhas ) ) {
  ...
  db_redireciona('db_erros.php?fechar=true&db_erro=' . urlencode("Nenhum Registro para o Relatório"));
  exit;
}
...
for ($iX = 0; $iX < $iTam; $iX++){
  ...
  for( $y=1; $y<=2; $y++ ){
    ...
    $pdf->rect( $pdf->getX()-0.5,   $pdf->getY(),   54, 16, "D" );
    $pdf->rect( $pdf->getX()+6, $pdf->getY()+1,  5, 14, "D" );
    $pdf->rect( $pdf->getX()+1, $pdf->getY()+5.5, 15,  5, "D" );
    ...
    $pdf->rect( $pdf->getX() - 0.5, $pdf->getY() + 18, 54, 48, "D" );
    $pdf->setfont( 'arial', 'b', 6 );
    ...
    $pdf->line( $pdf->getX() + 56     , $pdf->getY() + 24,
                $pdf->getX() + 56 + 18, $pdf->getY() + 24 );
    ...
    $pdf->line( $pdf->getX() + 76.5,  $pdf->getY(), $pdf->getX() + 76  + 34, $pdf->getY() );
    $pdf->line( $pdf->getX() + 112.5, $pdf->getY(), $pdf->getX() + 112 + 88, $pdf->getY() );
    ...
    for( $x = 0; $x < $linhas_proc[$iX]; $x++ ) {
      ...
      $pdf->line( $lar + 112.5,   $pdf->getY(), $lar + 116  + 84.5,  $pdf->getY() );
    }
    ...
    $pdf->setY( $alt + 76 );
    $pdf->setX( $lar );
    $pdf->text( $pdf->getX() + 1, $alt + 75, "CNS: {$s115_c_cartaosus}");
    $pdf->MultiCell( 53, 4, "NOME: " . $z01_i_numcgs . "-" . $z01_v_nome, 0, "L" );
    $pdf->setY( $alt );
    $pdf->setX( $lar );

    $alt = $pdf->getY() + 50 + 5;

    $pdf->text( $pdf->getX() + 1, $alt + 32, "ENDEREÇO: ".substr($z01_v_ender.", ".$z01_i_numero.", ".$z01_v_compl,0,21));
    $pdf->text( $pdf->getX() + 1, $alt + 32, substr($z01_v_ender.", ".$z01_i_numero.", ".$z01_v_compl,21,40));
    $pdf->text( $pdf->getX() + 1, $alt + 38, "BAIRRO: ".$z01_v_bairro);
    ...
    $pdf->rect( $pdf->getX() + 56.5, $pdf->getY() + 119, 123, 10, "D" );
    $pdf->setfont( 'arial', 'b', 5.5 );
    ...
    $pdf->rect( $pdf->getX() + 65.5 + 116, $pdf->getY() + 119, 19, 20, "D" );
    $pdf->setfont( 'arial', 'b', 5.2 );
    $pdf->text( $pdf->getX() + 65.5 + 116.5, $pdf->getY() + 122, "POLEGAR DIREITO:" );
    ...
    $pdf->rect( $pdf->getX() + 56.5, $pdf->getY() + 131, 123, 8, "D" );
    $pdf->setfont( 'arial', 'b', 6 );
