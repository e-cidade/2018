	  ...
	  <br>
		  <td colspan=2 align=center>
		   <input name="<?=($db_opcao==1?"incluir":($db_opcao==2?"alterar":"excluir"))?>" 
		          ...
		          <?=($db_botao==false?"disabled":"")?> <?=($db_opcao==22?"style='display:none;'":"")?> >
		   <input name="pesquisar" type="button" id="pesquisar" value="Pesquisar" onclick="js_pesquisa();" 
		          <?=(isset($opcao)?"style='display:none;'":"")?> >
		   <input name="novo" type="button" id="cancelar" value="Novo" onclick="js_cancelar();" 
		          <?=($db_opcao==1||$db_opcao==22||isset($db_opcaoal)?"style='visibility:hidden;'":"")?> >
		  </td>
	  </tr>
	</table>
 </fieldset>
  ...
  <tr>
	  <td valign="top"  align="center">  
	    <?
			  ...
			  $cliframe_alterar_excluir->legenda="Editais";
...
function js_pesquisa() {
  ...
  var sUrl = 'func_liclicita.php?funcao_js=parent.js_preenchepesquisa|l20_codigo';
  js_OpenJanelaIframe('', 'db_iframe_liclicita', sUrl, 'Pesquisa', true);
}
...
function js_valida() {
  ...
  var sNomeArquivo = document.form1.arquivoedital.value;
  var sOpcao       = document.form1.db_opcao.value;
  
  if (sOpcao != 'Excluir') {
  
	  if (sNomeArquivo == '') {
	  
		  alert('Arquivo do edital não informado!');
		  return false;
	  }
  }
}
