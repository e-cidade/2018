...
if ($oGet->emissao == 1) {
  ...
  $oPdf->Cell(135, $iAltCell, 'RGF - ANEXO I(LRF, art. 55, inciso I, alínea "a")', 'B', 0, "L", 0);
   ...
   */
  $oPdf->SetFont('arial', 'b', $iTamFonte);
  $oPdf->Cell(60, $iAltCell,'RGF - ANEXO I(LRF, art. 55, inciso I, alínea "a")', "b", 0, "L", 0);
  $oPdf->Cell(($iTamCell*14), $iAltCell, 'R$ 1,00', "b", 1, "R", 0);

  /**
   * Pega a possição inicial para ajustar o cabeçalhos dos meses de cada periodo.
   */
  $iPosXMes = $oPdf->GetX();
  $iPosYMes = $oPdf->GetY();
  ...
  $oPdf->Cell(($iTamCell*12), $iAltCell, "DESPESAS EXECUTADAS (Últimos 12 meses)", 'RTB', 0, "C", 0);

  /**
   * Pega a posição inicial para ajustar o cabeçalhos das totalizações do último mês.
   */
  $iPosX = $oPdf->GetX();
  $iPosY = $oPdf->GetY();

  /**
   * Monta cabeçalho de totalizações liquidadas(a)
   */
  $oPdf->MultiCell($iTamCell+5, $iAltCell*4, 'LIQUIDADAS(a)', 'TBR', 'C', 0, 0);

  /**
   * Monta totalizações inscritas em rp não processados
   */
  $oPdf->SetXY($iPosX+21, $iPosY);
  $oPdf->MultiCell($iTamCell+5, $iAltCell, "INSCRITAS EM RP NÃO PROCESSADOS\n(b)", 'TBL', 'C', 0, 0);
  $oPdf->Ln();
  ...
  foreach ($aDadosAnexoIRGF as $sIndice => $oDadoQuadroLinha) {
    ...
    if ($sIndice == 'quadrodespesanaocomputadas') {
      ...
      if ($sIndice == 'quadrodespesaliquida') {
        ...
        /**
         * Imprime linha quadro despesa líquida.
         */
        ...
        if ($sIndice == 'quadrodespesatotalcompessoal') {
          ...
          $oPdf->Cell(242, $iAltCell, "APURAÇÃO DO CUMPRIMENTO DO LIMITE LEGAL","RTB", 0, "C", 0);
