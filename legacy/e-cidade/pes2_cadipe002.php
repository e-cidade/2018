...
from rhipe 
     ...
     left join ipe on r36_sequencia = rh14_sequencia
                  and r36_anousu = $ano
		              and r36_mesusu = $mes
                  and r36_instit = rh14_instit  
		where rhipe.rh14_instit = ".db_getsession("DB_instit")."
$xordem
"; 
$result = pg_exec($sql);
...
for($x = 0; $x < pg_numrows($result);$x++){
   ...
   if ($pdf->gety() > $pdf->h - 30 || $troca != 0 ){
      ...
      $pdf->cell(25,$alt,'VLR INFORMADO',1,0,"C",1);
      $pdf->cell(25,$alt,'CONTRIBUICAO',1,1,"C",1);
      $troca = 0;
      $pre = 1;
   }
   if($pre == 1){
   ...
   $pdf->setfont('arial','',7);
   $pdf->cell(15,$alt,$rh63_numcgm,0,0,"R",$pre);
   $pdf->cell(75,$alt,$z01_nome,0,0,"L",$pre);
   $pdf->cell(15,$alt,$rh62_regist,0,0,"R",$pre);
   $pdf->cell(25,$alt,$rh14_matipe,0,0,"R",$pre);
   $pdf->cell(25,$alt,db_formatar($rh14_valor,'f'),0,0,"R",$pre);
   $pdf->cell(25,$alt,db_formatar($r36_contr1,'f'),0,1,"R",$pre);
   $total++;
}
$pdf->setfont('arial','b',8);
