...
$oDaoLabControleFisFin = db_utils::getdao('lab_controlefisicofinanceiro');
...
function novaLinha($oPdf, $oDados, $dDataIniDB, $dDataFimDB) {
  ...
  if ($oDados->iteto == 1) {
    ...
    $sSql = 'select sum(prc.sd63_f_sh+prc.sd63_f_sa+prc.sd63_f_sp+la53_n_acrescimo) as valor from lab_autoriza as lat ';
  }
  ...
  $rs                 = $oDaoLabConferencia->sql_record($sSql.$sJoin.$sWhere);
  
  //echo "<br><br> SQL Secundario: $sSql$sJoin$sWhere; ";
  
  $iLinhas            = $oDaoLabConferencia->numrows;
  if ($iLinhas > 0) {
    $oUtilizado = db_utils::fieldsmemory($rs, 0);
  } else {
    die(" $sSql$sJoin$sWhere <br><b>Erro ao selecionar procedimentos utilizados!</b>");
  }
...
//echo "<br><br> SQL principal: $sSql <br>";

$rs       = $oDaoLabControleFisFin->sql_record($sSql);
$iLinhas  = $oDaoLabControleFisFin->numrows;
...
//Cabeçalho
$head1 = "RELATÓRIO CONTROLE FINACEIRO DO LABORATORIO";
