  ...
  foreach ($oCursosSocial->getCidadaosMatriculados() as $oMatricula) {
    ...
    foreach ($aMesPagina as $oMes) {
      ...
      foreach ($oCursosSocial->getDiasDeAulaPorMesAno($oMes->iMes, $oMes->iAno) as $oDiaAula) {
        imprimePontoPresenca($oPdf, $oConfigRelatorio, $iColunaDia);
      }
    }
    $oPdf->ln();
 ...
 */
function imprimePontoPresenca (FPDF $oPdf, $oConfigRelatorio, $iColunaDia) {
  
  $iXInicio     = $oPdf->GetX();
  $iYInicio     = $oPdf->GetY();
  $oPdf->Rect($oPdf->GetX(), $oPdf->GetY(), $iColunaDia, $oConfigRelatorio->iAlturaLinha);
  $oPdf->SetFont("arial", "b", 9);
  $oPdf->Text(($iXInicio + ($iColunaDia / 2) - 0.5 ), ($iYInicio + ($oConfigRelatorio->iAlturaLinha / 2) + 0.5), ".");
