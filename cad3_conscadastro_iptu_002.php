...
$sql = "select      proprietario.*, j50_descr,
      ...
      left outer join cgm j on j44_numcgm = j.z01_numcgm              
       ...
       left join ruastipo  on ruastipo.j88_codigo      = ruas.j14_tipo
 			                      where $where limit 1";
$matriculaSelecionada = pg_exec($sql) or die($sql);
...
// comentado por evandro em 2008-09-22 por que estava duplicando as areas, e abaixo o select correto com distinct pelo idbql...
// $sqlareatotal = "
//					 select sum(j34_area) as areatotal
//					 from lote 
...
$sqlareatotal = "
 ...
 // die($sqlareatotal);
  ...
  $resultareatotal = pg_exec($sqlareatotal);
...
$sqlareaconst = "
	...
	where j39_matric = $cod_matricula
   	...
   	and j01_baixa is null";
//die($sqlareaconst);
$resultareaconst = pg_exec($sqlareaconst);
...
if ($linhasareaconst>0){
  ...
  /*
  $sqlareaconsttotal="
					  select sum(j39_area) as areaconttotal
					  from iptuconstr 
					          inner join iptubase on j01_matric = j39_matric 
					  ...
					  where j34_setor = '$j34_setor' 
					   ...
					   and j39_dtdemo is null 
					   and j01_baixa is null";
  //die($sqlareaconsttotal);

  $resultareaconsttotal = pg_exec($sqlareaconsttotal);
...
<table width="790" height="100%" border="0" align="center" cellpadding="0" cellspacing="2">
  ...
  <tr> 
    ...
    <?
    $rsSetfis = pg_query("select * from lotesetorfiscal inner join cadastro.setorfiscal on j90_codigo = j91_codigo inner join iptubase on j01_idbql = j91_idbql where j01_matric = $cod_matricula");
