...
$rsSqlParametro     = pg_exec($sSqlParametro);
...
/*if (pg_numrows($res) > 0 ){
	...
	// db_criatabela($result); exit;
 	if (pg_numrows($res) > 0) {
		for ($x = 0; $x < pg_numrows($result); $x ++) {
		     ...
		     if (pg_numrows($rr) > 0) {
				$sql = " update work_rel
   					        set saldo_extra = coalesce(".($saldo_final).",0)
					 where recurso = $o48_codrec
					  ";
			 	pg_exec($sql);
		     } else {
			  	$sql = " insert into work_rel  (recurso, recurso_descr, saldo_extra)
					 values ($o48_codrec,'".substr($o15_descr,0,30)."',".round($saldo_extra,2).")
				       ";
				pg_exec($sql);
		     }
	...
	$where = " c60_estrut like '211%'  ".' and c61_instit  in ('.str_replace('-', ', ', $db_selinstit).')  '.$where;
	...
	if (pg_numrows($result_bver) > 0) {
		...
		for ($x = 0; $x < pg_numrows($result_bver); $x ++) {
	         ...
	         $sql = "select * from work_rel where recurso = $livre";
		     $rr = pg_exec($sql);
		     if (pg_numrows($rr) > 0) {
				$sql = " update work_rel
							 set saldo_extra = coalesce(".($saldo_final).",0)
			 	...
			 	pg_exec($sql);
			  	...
			  	$sql = " insert into work_rel  (recurso, recurso_descr, saldo_extra)
							 values ($c61_codigo, '".substr($o15_descr,0,30)."',".round($saldo_final,2).")
						   ";
				pg_exec($sql);
		     }
