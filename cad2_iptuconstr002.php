...
$data = "";
/*
echo "<br> j39_dtlan = $j39_dtlan 
      <br> j39_dtlanfim = $j39_dtlanfim <br>";
exit;
*/
...
/*
  $sql = "select distinct on (iptuconstr.j39_matric,iptuconstr.j39_idcons)
                 ...
                 inner join (select distinct j48_matric, j48_idcons from carconstr where $where) as x on iptuconstr.j39_matric = x.j48_matric and iptuconstr.j39_idcons = x.j48_idcons 
         $sql_ruas 
         inner join proprietario_nome on proprietario_nome.j01_matric = j48_matric 
                 ".($areaini != "" || $areafim != "" || $habite != "" || $data != "" || $anoini !="" || $anofim != ""  || $setor != "" || $demo != "" || $origem != ""?"where":"")."
                 ".($areaini != ""?" j39_area >= $areaini":"")." ".($areafim != "" && $areaini != ""?"and":"")." ".($areafim != ""?" j39_area <= $areafim and ":"")." 
                 ".(($anoini != "" || $anofim != "") && ($areaini != "" || $areafim != "")?"":"")." ".($anoini != ""?" j39_ano >= $anoini":"")." ".($anofim != "" && $anoini != ""?"and":"")." ".($anofim != ""?" j39_ano <= $anofim and":"")." $habite $setorx $demo $origem $data";
*/
...
if(isset($j14_comruas) && $j14_comruas != "" && empty($j14_semruas)){
  ...
  $result1 = pg_exec($sSql) or die($sSql);
