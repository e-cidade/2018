...
global $sCepCxPostal,$sMunicipio;
...
function recibodesconto($numpre, $numpar, $tipo, $tipo_debito, $whereloteador, $totalregistrospassados, $totregistros,$dt_venc,$sTipoLista) {
  ...
  $sSqlTipoParc  = " select k40_codigo,                                                "; 
  $sSqlTipoParc .= "      k40_todasmarc,                                             "; 
  $sSqlTipoParc .= "        k40_forma,                                                 ";
  $sSqlTipoParc .= "        cadtipoparc                                                ";
  ...
  if (pg_numrows($rsTipoParc) == 0 or ($k40_todasmarc == 't'?$totalregistrospassados <> $totregistros:false) or $passar == false) {
    $desconto = 0;
  } else {
    $desconto = $k40_codigo;
  }
...
if (!isset($notifparc)) {
  ...
  if ($k60_tipo != 'M' ){
    ...
    $sql .= "        inner join debitos       on k22_data   = '$k60_datadeb'  ";
    $sql .= "                                and k22_numpre = k63_numpre      ";
    $sql .= "        $xxmatric                                                ";
    $sql .= "                                and $xxcodigo  = k63_notifica    ";
    $sql .= "        $xxmatric2                                               ";
    $sql .= "                                $limite                          ";
    $sql .= " $contr                                                          ";
    $sql .= "  where k60_codigo  = $lista                                     ";
    $sql .= "    and k22_dtvenc <= '$db_datausu'                              ";
    ...
    $sql .= "        $xcodigo1,                                                                ";
    $sql .= "        xvalor,                                                                   ";
    $sql .= "        substr(fc_iptuender,001,40)       as ender_entrega,                       ";
    $sql .= "        substr(fc_iptuender,159,08)       as cep_entrega,                         ";
    $sql .= "        substr(fc_iptuender,115,40)       as munic_entrega,                       ";
    $sql .= "        substr(fc_proprietario_nome,1,7)  as z01_numcgm,                          ";
    $sql .= "        substr(fc_proprietario_nome,8,40) as z01_nome                             ";
    $sql .= " from ( select $xxcodigo  as notifica,                                            ";
    $sql .= "               $xxcodigo1 as $xcodigo1,                                           ";
    $sql .= "               fc_iptuender($xxcodigo1),                                          ";
    $sql .= "               fc_proprietario_nome($xxcodigo1),                                  ";
    $sql .= "               sum(  k22_vlrcor +                                                 ";
    $sql .= "                     k22_juros  +                                                 ";
    $sql .= "                     k22_multa  -                                                 ";
    $sql .= "                     k22_desconto)        as xvalor                               ";
    $sql .= "          from lista                                                              ";
    $sql .= "             inner join listanotifica on k63_codigo         = k60_codigo          ";
    $sql .= "             inner join debitos       on k22_data           = '$k60_datadeb'      ";
    $sql .= "                                     and k22_numpre         = k63_numpre          ";
    $sql .= "             inner join arrecad       on k22_data           = '$k60_datadeb'      ";
    $sql .= "                                     and arrecad.k00_numpre = debitos.k22_numpre  ";
    $sql .= "                                     and arrecad.k00_numpar = debitos.k22_numpar  ";
    $sql .= "                                     and arrecad.k00_receit = debitos.k22_receit  ";
    $sql .= "                                     and arrecad.k00_tipo   = debitos.k22_tipo    ";
    $sql .= "             $xxmatric                                                            ";
    $sql .= "                                     and $xxcodigo = k63_notifica                 ";
    $sql .= "             $xxmatric2  $limite $contr                                           ";
    $sql .= " where k60_codigo = $lista                                                        ";
    $sql .= "   and k22_dtvenc <= '$db_datausu'                                                ";
    $sql .= "       group by $xxcodigo,                                                        ";
    $sql .= "                $xxcodigo1                                                        ";
...
//      $sqljapagou .= "  limit 1";
    ...
    if ($k60_tipo == 'M') {
      if ($tratamento > 1) {
        if (EnderecoNotif($tratamento, @$j01_matric, $z01_nome) == false and $imprimirmesmoembranco == "n") {
          continue;
        }
      }
    }
    ...
    global $variavel;
    $variavel="";
    if(($imprimirtimbre == 1) or ($imprimirtimbre == 2)){
    ...
    $sqlproced = " select distinct 
                          proced.v03_codigo, 
                          proced.v03_dcomp
                     from notidebitos
                          inner join divida on v01_numpre = k53_numpre 
                                           and v01_numpar = k53_numpar
    ...
    for ($doc = 0; $doc < pg_numrows($resultdocparagr); $doc++) {
      ...
      if (strtoupper($db02_descr) == "TOTALPORANO" and $somenteparc == false) {
        ...
        for ($ind = 0; $ind < $iLinhaOrigemNumpre; $ind ++ ) {
          ...
          $oEnvol = db_utils::getColectionByRecord($rsEnvol);
          
        } 
          
          foreach ($oEnvol as $iIndiceEnvol => $oEnvol){ //($i = 0; $i < $iLinhasEnvol; $i++){
      ...
      } else if (strtoupper($db02_descr) == "TOTALPORANOCORRIGIDO" and $somenteparc == false) {
        ...
        $aRegistros = db_utils::getColectionByRecord($rsQuery);
        ...
        if (isset($notifparc)){
          ...
          $sqltipodebito = "  select distinct 
                                     arretipo.k00_tipo as tipo_debito
                                from notidebitosreg
...
//          $sqlrecibo .=                    $sInnerArrecad;
        ...
        try {
           ...
           * utilizam este mesmo fonte, com uma chamada da new recibo. na planilha temos 3 opções de 
           * parametros para esse fonte, porem uma chamada da função, em conversa com o Robson, foi decidido passar o 27
           * (Notificações).
           */
        ...
        $sqlinstit = "select nomeinst,ender,numero,munic,cgc,email,telef,uf,logo,to_char(tx_banc,'9.99') as tx_banc,numbanco from db_config where codigo = ".db_getsession("DB_instit");
        ...
        if (isset($passou_por_aqui)&&$passou_por_aqui==true){        
          ...
          $pdf->cell(25,05,trim(db_formatar( ( @$totjuros1 /* - $tot_desc_jur */ ),'f')) ,1,0,"R",0);

          $pdf->cell(25,05,trim(db_formatar( ( @$totmulta1 /* - $tot_desc_mul */ ),'f')) ,1,0,"R",0);

          $pdf->cell(25,05,trim(db_formatar($tot_desc ,'f')) ,1,0,"R",0);

          @$total_correto = ( $tottotal1-$tot_desc /* -$tot_desc_jur-$tot_desc_mul */ );

          $pdf->cell(30,05,trim(db_formatar(( @$tottotal1-$tot_desc /* -$tot_desc_jur-$tot_desc_mul*/ ) ,'f')) ,1,1,"R",0);
...
//           $pdf->descr11_2           = trim(pg_result($Identificacao,0,"z01_ender"));
          ...
          $pdf->dtparapag          = $datavencimento; //date('d/m/Y',db_getsession('DB_datausu'));
          $pdf->descr10            = "1 / 1";
        ...
        if ($tipo_arrecadacao>0){ 
          ...
          if (strpos($db02_texto, "imprimirdesconto") >= 0 and gettype(strpos($db02_texto, "imprimirdesconto")) != "boolean") {
          ...
          $pdf->SetFont('Arial','',10);
          $pdf->Text($x+138, $y+15,@$pdf->descr10); // $pdf->parcela); // parcela
          $pdf->Text($x+158, $y+15,@$pdf->dtparapag); // $pdf->dtvenc);  // vencimento
          ...
          $pdf->Text($x+138, $y+30,str_pad(@$pdf->descr9,17,"0",STR_PAD_LEFT));//   $pdf->nosso_numero);      // nosso numero
          $pdf->Text($x+12, $y-7,str_pad(@$pdf->descr9,17,"0",STR_PAD_LEFT));//   $pdf->nosso_numero);      // nosso numero
        ...
        //$texto = trim($munic) .', '.date('d',strtotime($k60_datadeb)).' de '.db_mes(date('m',strtotime($k60_datadeb))).' de ' . date('Y',strtotime($k60_datadeb)) .'.';
      ...
      } elseif (strtoupper($db02_descr) == "SEEDPORMATRICULA") {
        $pdf->sety(190+35);
       ...
       // die('1');      
         /*
         * Tarefa 37315
         * Melhoramento na exibicao do endereco de entrega
         * se ele nao entra na funcao EnderecoNotif
         * aqui ele atribui as variaveis de municipio e cep com cx postal
         */
        
//        if ($z01_cxpostal == 0) {
//          $sCepCxPostal = $z01_cep;
//          $sMunicipio  = $z01_munic . " / " . $z01_uf;
//        } else {
...
//        }        
        ...
        $pdf->cell(40,5,"MUNICIPIO:",0,0,"L",0);
        $pdf->SetFont('Arial','B',12);
       // $pdf->cell(50,5,$z01_munic ."/".$z01_uf . " - " . substr($z01_cep,0,5)."-".substr($z01_cxpostal,5,3),0,1,"L",0);
        $pdf->cell(50,5,trim($z01_munic) ."/".$z01_uf ,0,1,"L",0);
        $pdf->SetFont('Arial','',12);
        // Campo Cep e Cx Postal , melhor separado abaixo do municipio
...
//        exit;
        ...
        for ($xxx=0; $xxx < sizeof($opcoesverso); $xxx++) {
          ...
          if ($dadosopcoes[0] == "parte1" and ( trim($dadosopcoes[1]) == "REMETENTE" ) ) {
            $remetente_parte1 = 1;
          }

          if ($dadosopcoes[0] == "parte3" and ( trim($dadosopcoes[1]) == "REMETENTE" ) ) {
            $remetente_parte3 = 1;
          }

        }

//        die("remetente_parte1: $remetente_parte1 === remetente_parte3: $remetente_parte3");

        for ($xxx=0; $xxx < sizeof($opcoesverso); $xxx++) {

          $dadosopcoes = split("=", $opcoesverso[$xxx]);

          if ($dadosopcoes[0] == "parte1" and ( $remetente_parte1 == 0 ) ) {
            //echo "aqui parte1<br>";
          } elseif ($dadosopcoes[0] == "parte2") {
            //echo "aqui parte2<br>";
            //$msgparte2 = split("=", $dadosopcoes[0]);
            //echo $dadosopcoes[1];
            // exit();
            if (trim($dadosopcoes[1]) == "ENDER_ENTREGA") {
              ...
              if(($imprimirtimbre == 1) or ($imprimirtimbre == 3)){
          ...
          } elseif ($dadosopcoes[0] == "parte3" or ( $remetente_parte3 == 1 ) ) {

            if (trim($dadosopcoes[1]) == "AR" and $dadosopcoes[0] == "parte3") {
        ...
        //EnderEntrega($pdf,$matric,$cgm,$k60_tipo,$tratamento,$z01_nome,$q02_inscr);
  ...
  function CabecNotif($pdf, $endereco, $central) {
    ...
    if ($endereco == 1) {
      //    $pdf->Image("imagens/files/".$logo,$posini,124,24);
      //    $pdf->Ln(115);
  ...
  function EnderecoNotif($tratamento, $matric, $z01_nome ) {
    ...
    global $z01_ender,$z01_bairro, $z01_numero, $z01_compl, $z01_munic, $z01_uf, $z01_cep, $z01_cxpostal, $z01_destinatario,$sCepCxPostal,$sMunicipio;

    $sqlsalva="select j18_ordendent from cfiptu where j18_anousu = " . db_getsession("DB_anousu");
    $resultsalva = db_query($sqlsalva) or die($sqlsalva);
    if (pg_numrows($resultsalva) == 0) {
      return false;
    }
    //echo $matric;
    //$sqltroca = "update cfiptu set j18_ordendent = " . ($tratamento - 10) . " where j18_anousu = " . db_getsession("DB_anousu");
    //$resulttroca = db_query($sqltroca) or die($sqltroca);
    ...
    $sqlendereco   .= "  from ( select fc_iptuender($matric) ) as x";
    $resultendereco = db_query($sqlendereco) or die($sqlendereco);     
    db_fieldsmemory($resultendereco,0);  
   
    if (pg_numrows($resultendereco) > 0 and pg_result($resultendereco,0,0) != "") {
      ...
      //db_msgbox("mat = $matric bairro = $z01_bairro  e ender = $z01_ender  e munic = $z01_munic dest = $z01_destinatario");
    } else {
