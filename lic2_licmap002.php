...
if ($modelo == 1) {
  ...
  for($x = 0; $x < $numrows_forne; $x ++) {
    ...
    if ($pdf->gety() > $pdf->h - 30 || $troca != 0) {
      ...
      $result_itens = $clpcorcamitem->sql_record($clpcorcamitem->sql_query_pcmaterlic(null, "distinct l21_ordem,
	                                          pc22_orcamitem,
	                                          pc01_descrmater", "l21_ordem", "pc22_codorc=$orcamento"));
    ...
    for($w = $quant_imp; $w < $max_itens; $w ++) {
      ...
      $result_valor = $clpcorcamval->sql_record($clpcorcamval->sql_query_julg(null, null, "pc23_valor,pc24_pontuacao", null, "pc23_orcamforne=$pc21_orcamforne and pc23_orcamitem=$pc22_orcamitem"));
  ...
  for($x = 0; $x < $numrows_itens; $x ++) {
    ...
    if ($pdf->gety() > $pdf->h - 30 || $troca != 0) {
      ...
      $pdf->cell(20, $alt, "SOLICITAÇÂO", 1, 0, "C", 1);
    ...
    $result_valor = $clpcorcamval->sql_record($clpcorcamval->sql_query_julg(null, null, "pc23_valor,pc24_pontuacao", null, "pc23_orcamitem=$pc22_orcamitem 
