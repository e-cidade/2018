...
include("classes/db_lab_resultadoitem_classe.php");
include("classes/db_lab_resultadoalfa_classe.php");
include("classes/db_lab_resultadonum_classe.php");
...
if ($iTipo == 2) { //Auto complete exames agendamento de exames
  ...
  $sSubSQL  = " select * from lab_setorexame where la09_i_exame=la08_i_codigo ";
  $sWhere   = "la08_c_descr like upper('".$sName."%') and EXISTS ($sSubSQL)";
  $sSql     = $oDaoLabExames->sql_query(null, $sCampos, 'label', $sWhere);
...
if ($objParam->exec == 'getProcedimento') {
  ...
  if ($clsau_proccbo->numrows > 0) {
    $objRetorno->itens   = db_utils::getColectionByRecord($res_sau_proccbo, true, false, true);
  } else {
...
if ($objParam->exec == 'getGridExames') {
  ...
  if ($oDaoLabRequiitem->numrows > 0) {
    $objRetorno->aItens = db_utils::getColectionByRecord($rsResult,true);
  } else {
...
if($objParam->exec == 'CarregaGridAutorizado'){
	...
	$sSql=$cllab_requisicao->sql_query_requiitem("","lab_requiitem.*,lab_exame.*,lab_laboratorio.*,lab_coletaitem.la32_i_codigo",""," la21_i_requisicao=$objParam->requisicao and la02_i_codigo=$objParam->iLaboratorioLogado ");
	$result = $cllab_requisicao->sql_record($sSql);
    $alinhasgrid=Array();
    $y=0;
    for($x=0;$x<$cllab_requisicao->numrows;$x++){
        ...
        if(($oExame->la21_c_situacao=="8 - Autorizado")||($oExame->la21_c_situacao=="f - falta material")){
          ...
          $alinhasgrid[$y]="$oExame->la21_i_setorexame#$oExame->la02_c_descr#$oExame->la08_c_descr#".$aData[2]."/".$aData[1]."/".$aData[0]."#$oExame->la21_c_hora#$oExame->la21_c_situacao#$oExame->la21_i_emergencia#$oExame->la21_i_codigo#".$aData2[2]."/".$aData2[1]."/".$aData2[0]."";
          $y++;
        }
    }
	$objRetorno->alinhasgrid=$alinhasgrid;
