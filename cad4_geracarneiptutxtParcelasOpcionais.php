...
// 			print_r($aUnicasComparativo);
			...
			for($iMes = $iMesVencimento; $iMes <= 12; $iMes++) {
				...
				$sDataVencimentoUnica      = "{$iDiaVencUnica}/".str_pad($iProximoMesUnica, 2, "0", STR_PAD_LEFT)."/{$iProximoAnoUnica}";
...
//        $sSqlCorrecao .= "                 where k00_numpre = $j20_numpre                                     \n";
        ...
        $sSqlCorrecao .= "group by k00_numpre                                                                 \n";
				...
				if ($gerar == "layout") {
					fputs($clabre_arquivo->arquivo, db_contador("VCTO_OPCAO_{$iIndicadorOpcao}_QUOTA_{$iOpcaoUnicas}","OPCAO DE VENC. {$iIndicadorOpcao} DA QUOTA ÚNICA {$iOpcaoUnicas}",$contador,10));
					fputs($clabre_arquivo->arquivo, db_contador("VALOR_{$iIndicadorOpcao}_QUOTA_{$iOpcaoUnicas}","Valor Opçao {$iIndicadorOpcao} da Quota ÚNICA {$iOpcaoUnicas}",$contador,15));
					$iIndicadorOpcao++;
	...
	$sDataVencParcela    = $aParecelasArrecad[0]->k00_dtvenc;
	$iTotalParcelas      = $aParecelasArrecad[0]->k00_numtot;
	...
	for ($iMesParcela = 1; $iMesParcela <= $iTotalParcelas; $iMesParcela ++) {
		...
		$oParcela           = $aParecelasArrecad[$iIndice];//dados das parcelas tipo, numpre, valor etc.
		...
		for ($iMesParcelaOpcao = $iMesPrimeiraParcela; $iMesParcelaOpcao <= 12 - $iIndice; $iMesParcelaOpcao ++) {
			...
			$rsDadosParcela           = debitos_numpre($j20_numpre, 0, 0,strtotime($dtVencimentoParcCorrecao), $anousu, $iMesParcela,'k00_numpre, k00_numpar');
			
			$aValoresParcela          = db_utils::getColectionByRecord($rsDadosParcela);
			...
			if ($gerar == "layout") {
				...
				fputs($clabre_arquivo->arquivo, db_contador("VCTO_OPCAO_{$iContadorParcela}_PARCELA_{$iMesParcela}","OPCAO DE VENC. {$iMesParcelaOpcao} DA COTA ÚNICA {$iMesParcela}",$contador,10));
				fputs($clabre_arquivo->arquivo, db_contador("VALOR_{$iContadorParcela}_PARCELA_{$iMesParcela}","VALOR OPCAO {$iMesParcelaOpcao} DA PARCELA  {$iMesParcela}",$contador,15));
