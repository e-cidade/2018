...
if ($oParam->acao == "incluir") {
	 ...
	 */
	if ($oParam->cidadao->ov02_cnpjcpf != '00000000000') {
  ...
  if (isset($oParam->cidadao->lTelaSocial) && $oParam->cidadao->lTelaSocial && !$lerro) {
	...
	if ($iNumRows>0){
	  ...
	  $aCidadao = array();
	  foreach (db_utils::getColectionByRecord($rsCidadao,false,false,true) as $oDadosCidadao) {

	    $oCidadao = new stdClass();

	    $oCidadao->ov02_sequencial     = $oDadosCidadao->ov02_sequencial;
	    $oCidadao->ov02_seq            = $oDadosCidadao->ov02_seq;
	    $oCidadao->ov02_nome           = db_stdClass::normalizeStringJson($oDadosCidadao->ov02_nome);
	  ...
	  $oRetorno->cidadao     = $aCidadao;
		...
		if ($iNumRows > 0){
		  ...
		  $oRetorno->tiporetorno = db_utils::getColectionByRecord($rsTipoRetorno,false,false,true);
		  $oRetorno->status	     = 0;
		}
		...
		if ($iNumRows > 0) {
			...
			$oRetorno->cidadaotelefones = db_utils::getColectionByRecord($rsCidadaoTelefones,false,false,true);
			$oRetorno->status	          = 0;
		}else {
		...
		if ($iNumRows>0){
			$oRetorno->cidadaoemails = db_utils::getColectionByRecord($rsCidadaoEmails,false,false,true);
			$oRetorno->status	       = 0;
		}else {
