...
void *fazAmao(void *arg) {
  ...
  if((fp = fopen(est->saida,"w")) == NULL) {
    fprintf(stderr,"Erro abrindo dispositivo %s: %s\n",est->saida,strerror(errno));
    exit(-1);
  }
  fprintf(fp,"%s",buf);
  fclose(fp);
...
int main(void) {
  ...
  if((fp = fopen("servidor.conf","r")) == NULL) {
  ...
  while(1) {  // loop principal accept()
    ...
    printf("servidor: recebeu conex?o de %s\n",inet_ntoa(outro_end.sin_addr));
    if(send(nova_fd, "Ol? , Beleza!\n", 14,0) == -1) {
      perror("send");
      exit(-1);
    }
    estFD->saida = saida;
    estFD->fd = nova_fd;
    if((sin_size = pthread_create(&thread_id, NULL, fazAmao,(void *)estFD)) != 0) {
