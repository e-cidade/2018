  ...
  if ( $xmatric > 0 ) {
    ...
    for ($i=0; $i < $iLinhasEnvol; $i++) {
      ...
      if ($clcgm->numrows > 0) {
        ...
        $xender	.= $sExpFalecido.trim($oDadosEnvol->z01_nome).(strlen(trim($oDadosEnvol->z01_cgccpf))==11?", BRASILEIRO":"").($oDadosEnvol->z01_cgccpf!=''?", {$sCgcCpf}":"");
    ...
    for ($xi = 0; $xi < pg_num_rows($rsDadosEnvol); $xi++) {
      ...
      $xender .= " CGM: " . $z01_numcgm . " - " . $sExpFalecido . trim($z01_nome) . (strlen(trim($z01_cgccpf)) == 11 ? ", BRASILEIRO" : "") . ($z01_cgccpf != '' ? ", {$sCgcCpf}" : "");
