...
class cl_configdbpref { 
   ...
   var $campos = "
                 ...
                 w13_uploadarquivos = varchar(50) = Caminho da Pasta de Upload de Arquivos 
                 ";
   ...
   function incluir ($w13_instit){ 
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if(($resaco!=false)||($this->numrows!=0)){
         ...
         $resac = db_query("insert into db_acount values($acount,1383,1009244,'','".AddSlashes(pg_result($resaco,0,'w13_uploadarquivos'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
       }
     }
     return true;
   } 
       ...
       if($this->numrows>0){
         ...
         for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
           ...
           if(isset($GLOBALS["HTTP_POST_VARS"]["w13_uploadarquivos"]) || $this->w13_uploadarquivos != "")
             $resac = db_query("insert into db_acount values($acount,1383,1009244,'".AddSlashes(pg_result($resaco,$conresaco,'w13_uploadarquivos'))."','$this->w13_uploadarquivos',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
         }
       }
     }
     $result = db_query($sql);
