   ...
   function alterar ($m84_sequencial=null, $sWhereAlterar) { 
     ...
     if($this->numrows>0){
       for($conresaco=0;$conresaco<$this->numrows;$conresaco++){
         ...
         $acount = pg_result($resac,0,0);
         $resac = db_query("insert into db_acountacesso values($acount,".db_getsession("DB_acessado").")");
         $resac = db_query("insert into db_acountkey values($acount,19818,'$this->m84_sequencial','A')");
         if(isset($GLOBALS["HTTP_POST_VARS"]["m84_sequencial"]) || $this->m84_sequencial != "")
           $resac = db_query("insert into db_acount values($acount,3553,19818,'".AddSlashes(pg_result($resaco,$conresaco,'m84_sequencial'))."','$this->m84_sequencial',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
