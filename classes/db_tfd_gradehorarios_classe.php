...
class cl_tfd_gradehorarios { 
   ...
   var $campos = "
                 tf02_i_codigo = int4 = Código 
                 tf02_i_destino = int4 = Destino 
                 tf02_i_diasemana = int4 = Dia Semana 
                 tf02_i_lotacao = int4 = Lotação 
                 tf02_c_horario = char(5) = Horário 
                 tf02_d_validadeini = date = Início 
                 tf02_d_validadefim = date = Fim 
                 tf02_c_localsaida = varchar(50) = Local da Saída 
                 ";
   ...
   function incluir ($tf02_i_codigo){ 
     ...
     if($tf02_i_codigo == "" || $tf02_i_codigo == null ){
       ...
       if(($result != false) && (pg_result($result,0,0) < $tf02_i_codigo)){
         $this->erro_sql = " Campo tf02_i_codigo maior que último número da sequencia.";
     ...
     if($result==false){ 
       ...
       if( strpos(strtolower($this->erro_banco),"duplicate key") != 0 ){
         $this->erro_sql   = "tfd_gradehorarios ($this->tf02_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_banco = "tfd_gradehorarios já Cadastrado";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
       }else{
         $this->erro_sql   = "tfd_gradehorarios ($this->tf02_i_codigo) nao Incluído. Inclusao Abortada.";
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
   ...
   function excluir ($tf02_i_codigo=null,$dbwhere=null) { 
     ...
     if($result==false){ 
       $this->erro_banco = str_replace("\n","",@pg_last_error());
       $this->erro_sql   = "tfd_gradehorarios nao Excluído. Exclusão Abortada.\\n";
       ...
       if(pg_affected_rows($result)==0){
         $this->erro_banco = "";
         $this->erro_sql = "tfd_gradehorarios nao Encontrado. Exclusão não Efetuada.\\n";
         $this->erro_sql .= "Valores : ".$tf02_i_codigo;
         $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
         $this->erro_msg   .=  str_replace('"',"",str_replace("'","",  "Administrador: \\n\\n ".$this->erro_banco." \\n"));
         $this->erro_status = "1";
