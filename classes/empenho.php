...
//                $oEmpItem->saldovalor -= $aItensNota[$iItens]->e72_valor;
      ...
      if (!$this->lSqlErro){
        ...
        if (count($aNotas) > 0){
          ...
          for ($iNotas = 0; $iNotas < count($aNotas); $iNotas++) {
            ...
            $oEmpNotaEle->e70_vlrliq  = "{$nValorLiquidado}";
  ...
  public function getEmpenhosLiberados($oFiltro) {
    ...
    $sWhere .= "                    and m53_codordem is null)                                                         ";
	  ...
	  $aItens          = db_utils::getColectionByRecord($rsSqlEmpEmpenho, true, false, true);
  ...
  function getLancamentosContabeis($sWhere='') {
    ...
    $aLancamentos     = db_utils::getColectionByRecord($rsLancamentos, false,false, true);
    return $aLancamentos;
    ...
    switch ($oDadosLancamento->tipoevento) {
      ...
      case 10:
        ...
        if (pg_num_rows($rsOrdemCompra) > 0 ) {
          ...
          if (db_strtotime($dtValidar) > db_strtotime($oOrdemCompra->m51_data)) {
            ...
            $sOrdens  = "";
            $sVirgula = "";
            $aOrdens  = db_utils::getColectionByRecord($rsOrdemCompra);
      ...
      case 11:
        ...
        lancamentoContabil::alterarDataLancamento($iCodigoLancamento, $dtValidar);
          ...
          $aAnulacoes        = db_utils::getColectionByRecord($rsDadosAnulacao);
   ...
   if (!db_utils::inTransaction()) {
    ...
    switch ($oDadosLancamento->tipoevento) {
      ...
      case 10:
        ...
        $oDaoEmpPresta->excluir($this->numemp);
  ...
  protected function excluirLancamentoEstornoLiquidacao($oDadosLancamento) {
    ...
    $aOrdemPagamentos = db_utils::getColectionByRecord($rsOrdens);
    $oOrdem           = null;
    ...
    if (pg_num_rows($rsOrdem) > 0) {
      ...
      $aOrdemCompras  = db_utils::getColectionByRecord($rsOrdem);
     ...
     if ($oDadosCorrente->k12_empen != "") {
       ...
       if ($iTipo == 1 || $iTipo == 2) {
         ...
         if ($oDadosCorrente->corlancconta != "") {
           ...
           $oDaoCorlanc->incluir($oDadosCorrente->k12_id,$oDadosCorrente->k12_data, $oDadosCorrente->k12_autent);
           if ($oDaoCorlanc->erro_status == 0) {
