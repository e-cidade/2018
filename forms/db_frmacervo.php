...
<input name="<?=($db_opcao==1?"incluir":($db_opcao==2||$db_opcao==22?"alterar":"excluir"))?>" type="submit" id="db_opcao" value="<?=($db_opcao==1?"Incluir":($db_opcao==2||$db_opcao==22?"Alterar":"Excluir"))?>" <?=($db_botao==false?"disabled":"")?> <?=$db_opcao==3?"onclick=\"return confirm('Serão apagados todos registros (Autores,Assuntos,Exemplares,Localização) referentes a este acervo! Caso exista algum registro de empréstimo ou reserva para algum exemplar deste acervo a exclusão não será permitida. Confirma exclusão?')\"":""?>>
