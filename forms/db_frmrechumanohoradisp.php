...
<table border="0">
 <tr>
  <td nowrap title="<?=@$Ted33_i_rechumano?>">
   <?db_ancora(@$Led33_i_rechumano,"",3);?>
  </td>
  <td>
   <?db_input('ed33_i_rechumano',10,$Ied33_i_rechumano,true,'text',3,"")?>
...
<table cellspacing="0" cellpading="0" border="1" bordercolor="#000000">
  ...
  $result2 = $clrechumanoativ->sql_record($clrechumanoativ->sql_query("","ed01_c_regencia as regencia",""," ed75_i_rechumano = $ed33_i_rechumano GROUP BY regencia"));
  ...
  db_fieldsmemory($result1,$z);
   ...
   for($x=0;$x<$cldiasemana->numrows;$x++){
    $quadro = "Q".$z.$x;
    db_fieldsmemory($result,$x);
    $sql2 = $clrechumanohoradisp->sql_query("","ed33_i_codigo,ed33_i_periodo,ed33_i_diasemana,ed17_i_escola as escolamarcada,ed18_c_nome as nomeescola",""," ed33_i_rechumano = $ed33_i_rechumano AND ed32_i_codigo = $ed32_i_codigo AND ed08_i_codigo = $ed08_i_codigo AND ed15_i_codigo = $ed15_i_codigo");
