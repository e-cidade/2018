...
?>
  ...
  <tr>
   <td nowrap>
    <?db_ancora(@$Lla31_i_cgs,"js_pesquisala31_i_cgs(true);",$db_opcao);?></td>
   <td nowrap>
...
?>
  ...
  <tr>
       ...
       <?
		db_ancora ( '<b>Exame</>', "js_pesquisala21_i_codigo(true);", "" );
	   ?>
    </td>
		<td> 
<?
...
db_ancora ( @$Lla31_i_tipodocumento, "js_pesquisala31_i_tipodocumento(true);", "" );
       ?>
		...
		<td> 
<?
db_input ( 'la31_i_tipodocumento', 10, @$Ila31_i_tipodocumento, true, 'text',"", " onchange='js_pesquisala31_i_tipodocumento(false);'" );
...
function js_limpaCamposTrocaReq() {
	...
	function js_pesquisala22_i_codigo(mostra){
     ...
     function js_pesquisala21_i_codigo(mostra){
	    ...
	    if(mostra==true){
	       js_OpenJanelaIframe('','db_iframe_requiitem','func_lab_requiitem.php?'+sPesq+'funcao_js=parent.js_mostrarequiitem1|la21_i_codigo|la08_c_descr','Pesquisa',true);
	    }else{
	       if(document.form1.la21_i_codigo.value != ''){ 
	          js_OpenJanelaIframe('','db_iframe_requiitem','func_lab_requiitem.php?'+sPesq+'pesquisa_chave='+document.form1.la21_i_codigo.value+'&funcao_js=parent.js_mostrarequiitem','Pesquisa',false);
	       }else{
	          document.form1.la08_c_descr.value = ''; 
	       }
	    }
	 ...
	 function js_pesquisala31_i_tipodocumento(mostra){
		  if(mostra==true){
	          js_OpenJanelaIframe('','db_iframe_tipodocumento','func_lab_tipodocumento.php?&funcao_js=parent.js_mostratipodocumento1|la33_i_codigo|la33_c_descr','Pesquisa',true);
		  }else{
		      if(document.form1.la31_i_tipodocumento.value != ''){ 
		         js_OpenJanelaIframe('','db_iframe_tipodocumento','func_lab_tipodocumento.php?&pesquisa_chave='+document.form1.la31_i_tipodocumento.value+'&funcao_js=parent.js_mostratipodocumento','Pesquisa',false);
		      }else{
	 ...
	 function js_pesquisala31_i_cgs(mostra){
		  if(mostra==true){
		    ...
		    if(document.form1.la31_i_cgs.value != ''){ 
		       js_OpenJanelaIframe('','db_iframe_cgs_und','func_cgs_und.php?pesquisa_chave='+document.form1.la31_i_cgs.value+'&funcao_js=parent.js_mostracgs_und','Pesquisa',false);
		    }else{
     ...
     function js_pesquisa(){
        js_OpenJanelaIframe('','db_iframe_lab_tipodocumento','func_lab_tipodocumento.php?funcao_js=parent.js_preenchepesquisa|la33_i_codigo','Pesquisa',true);
     }
