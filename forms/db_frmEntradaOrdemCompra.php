...
<table>
  ...
  <tr>
    ...
    <td  style='display: <?=$iControlaPit==1?"":"none"?>'><?
      ...
      ?>
      <a href='#' onclick='js_abreNotaExtra()' style='display: none'
      id='dadosnotacomplementar'>Outros Dados</a> </td>
    ...
    <table>
      ...
      <tr>
        ...
        <td>
            ...
            $rsUnidades = $oDaoUnid->sql_record($oDaoUnid->sql_query(null, "*", "m61_codmatunid"));
            for($i = 0; $i < $oDaoUnid->numrows; $i ++) {

              $oItens = db_utils::fieldsMemory($rsUnidades, $i);
              $aItens [$oItens->m61_codmatunid] = $oItens->m61_descr;

            }
            db_select('unidadeentrada', $aItens, true, 1, "style='width:200pt'");
            ?>
          </td>
      </tr>
      <tr>
...
function js_pesquisa_matordem(mostra){
    js_OpenJanelaIframe('top.corpo','db_iframe_matordem','func_matordement.php?funcao_js=parent.js_mostramatordem|m51_codordem','Pesquisa',true);
}
...
function js_confirmaEntrada() {
  ...
  if (confirm('Confirma a entrada dos Itens Selecionados no estoque?')) {
   ...
   if ($F('e69_numero').trim() == '') {
    ...
    sJson     += '"sNumero":"'+sNumero+'","dtDataNota":"'+$F('e69_dtnota')+'",';
