...
foreach ($aTurmas as $oTurma) {
  ...
  $oDadosTurma->sEscola         = $oTurma->getEscola()->getNome();
...
foreach ($aTurmaProcessada as $oTurma) {
  ...
  foreach ($oTurma->aAlunos as $oAluno) {
    ...
    if (!empty($oAluno->nPercentualFaltas) && $oAluno->nPercentualFaltas != 100) {
      $nPercentualFaltas = $oAluno->nPercentualFaltas. "%";
    }
    if (!empty($oAluno->nPercentualFrequencia) && $oAluno->nPercentualFrequencia != 100) {
